...
class FrontierExecutionReceiptQueueWritebackHandoffBridgeChecklistWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_csv_json_and_markdown(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch(
                "src.frontier_execution_receipt_queue_writeback_handoff_bridge_checklist.PROJECT_ROOT",
                root,
            ):
            ...
            self.assertIn(
                "Writeback Handoff Bridge Checklist",
