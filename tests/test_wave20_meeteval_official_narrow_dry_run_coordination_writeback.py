...
from src.wave20_meeteval_official_narrow_dry_run_coordination_writeback import (
...
class Wave20MeetevalOfficialNarrowDryRunCoordinationWritebackTest(unittest.TestCase):
    def test_build_coordination_rows_has_five_sections(self) -> None:
        self.assertEqual(len(build_coordination_rows()), 5)

    def test_build_fill_row_marks_wave20_official_coordination_complete(self) -> None:
    ...
    def test_run_coordination_writeback_requires_wave20_closure(self) -> None:
        with patch(
            "src.wave20_meeteval_official_narrow_dry_run_coordination_writeback.load_json_dict",
            side_effect=[
