...
class SpeakerProfileEmbeddingTrialExecutionReceiptMilestoneBridgeChecklistWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_csv_json_and_markdown(self) -> None:
        rows = build_bridge_checklist_rows(
            {
                "next_milestone": "speaker_profile_receipt_readiness_reopen_ready",
                "unlocks": "Reopen readiness target after checkpoint.",
            }
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch(
                "src.speaker_profile_embedding_trial_execution_receipt_milestone_bridge_checklist.PROJECT_ROOT",
                root,
            ):
