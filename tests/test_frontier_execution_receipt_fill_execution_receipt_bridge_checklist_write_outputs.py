...
class FrontierExecutionReceiptFillExecutionReceiptBridgeChecklistWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_csv_json_and_markdown(self) -> None:
        rows = build_bridge_checklist_rows(
            {
                ...
                "bridge_note": "Verify operator brief before receipt update.",
            }
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch(
                "src.frontier_execution_receipt_fill_execution_receipt_bridge_checklist.PROJECT_ROOT",
                root,
            ):
