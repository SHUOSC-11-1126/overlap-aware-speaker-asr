...
from src.external_validation_narrow_slice_coordination_writeback import (
...
class ExternalValidationNarrowSliceCoordinationWritebackTest(unittest.TestCase):
    def test_build_coordination_rows_has_five_sections(self) -> None:
        self.assertEqual(len(build_coordination_rows()), 5)

    def test_build_fill_row_records_external_blocker(self) -> None:
        ...
        self.assertEqual(row["blocker"], "gold_benchmark_claims_still_blocked")

    def test_run_coordination_writeback_requires_wave11_closure(self) -> None:
        with patch(
            "src.external_validation_narrow_slice_coordination_writeback.load_json_dict",
            side_effect=[
