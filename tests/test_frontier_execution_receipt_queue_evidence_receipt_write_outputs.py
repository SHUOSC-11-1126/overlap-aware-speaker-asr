...
from src.frontier_execution_receipt_queue_evidence_receipt import (
...
class FrontierExecutionReceiptQueueEvidenceReceiptWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_csv_json_and_markdown(self) -> None:
        row = build_evidence_receipt_row(
            {
                "operator_frontier": "meeteval_compatibility",
                "operator_action": "Fill execution receipt after real run.",
                "operator_evidence": "results/figures/frontier_execution_receipt_queue_handoff.md",
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch(
                "src.frontier_execution_receipt_queue_evidence_receipt.PROJECT_ROOT",
                root,
            ):
