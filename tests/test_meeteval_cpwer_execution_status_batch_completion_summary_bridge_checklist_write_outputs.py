...
class MeetEvalCpwerExecutionStatusBatchCompletionSummaryBridgeChecklistWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_csv_json_and_markdown(self) -> None:
        rows = build_bridge_checklist_rows(
            {"queue_status": "queue_ready_to_execute", "ready_chain_count": "5", "total_chain_count": "5"}
        )

        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch(
                "src.meeteval_cpwer_execution_status_batch_completion_summary_bridge_checklist.PROJECT_ROOT",
                root,
            ):
