...
| --- | --- | --- |
| ceilf6 | [@ceilf6](https://github.com/ceilf6) | Lead contributor spanning both project lines. **Stable baseline:** CER evaluation, adaptive router v1/v2, risk-aware selector, speaker-aware CER, and cpCER-lite. **Frontier exploration:** compute-aware cascade, MeetEval/cpWER compatibility, speaker-profile / voiceprint risk, external mini-validation, LLM critic, and the demo. **Cross-cutting:** the `project_harness` coordination backbone. Secondary: repository maintenance and the development Harness (Git hooks / knowledge-base contract / SDD / TDD) with repo-guard CR. |
| xyx12369 | [@xyx12369](https://github.com/xyx12369) | **Mode B focused extension:** directed and drove the three-tier compute-aware cascaded recognition module (`src/cascade_tiers.py`). Designed the Tier 1 (cheap) → Tier 2 (stronger, risk-gated) → Tier 3 (LLM critic/manual review) escalation architecture with reference-free instability signals. Wrote the 24-unit-test TDD suite and generated CER-cost tradeoff visualization, coverage statistics, and cost-aware routing table. All work under `experimental/frontier` label. |
| Claude Code | [Anthropic](https://www.anthropic.com) | AI agent collaborator. Implemented `cascade_tiers.py` (907 lines) and `tests/test_cascade_tiers.py` (389 lines) under xyx12369's direction following the repo's TDD/Harness workflow. Generated comparison analysis and tradeoff chart. All code committed under ceilf6's authorship. |
