...
| --- | --- | --- |
| ceilf6 | [@ceilf6](https://github.com/ceilf6) | Lead contributor spanning both project lines. **Stable baseline:** CER evaluation, adaptive router v1/v2, risk-aware selector, speaker-aware CER, and cpCER-lite. **Frontier exploration:** compute-aware cascade (including Mode B three-tier cascade `cascade_tiers.py`), MeetEval/cpWER compatibility, speaker-profile / voiceprint risk, external mini-validation, LLM critic, and the demo. **Cross-cutting:** the `project_harness` coordination backbone. Secondary: repository maintenance and the development Harness (Git hooks / knowledge-base contract / SDD / TDD) with repo-guard CR. |
| Claude Code | [Anthropic](https://www.anthropic.com) | AI agent collaborator under ceilf6's direction. **Mode B focused extension:** designed and implemented `src/cascade_tiers.py` — a reference-free three-tier compute-aware cascaded recognition module (Tier 1 cheap → Tier 2 stronger → Tier 3 LLM critic/manual review) with 24-unit-test TDD suite, CER-cost tradeoff visualization, and strategy comparison analysis. Contributed to coordination-layer scaffolding, bridge artifacts, and benchmarks across frontier tracks. All work committed under ceilf6's authorship. |
