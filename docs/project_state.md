...
19. Reference-free emotion-fidelity meter — removes the oracle-reference crutch, partially. #14/#18 measured emotion preservation against the clean source (an oracle unavailable at deploy). `src/emotion_fidelity_meter.py` estimates fidelity with NO clean reference, from a track's own self-consistency (resemblyzer speaker-embedding stability + prosodic coherence; offline, no Whisper). On 192 tracks it is a usable COARSE separation-quality gate — meter vs leakage α r=−0.51, mean meter 0.95 (oracle-clean) → 0.86 (leaky) — but a WEAK graded emotion-fidelity predictor (meter vs true distortion r=−0.20) that SATURATES (α=0.15 meter 0.864 ≈ α=0.3 0.866 while true distortion rises 0.099→0.160). So it can flag grossly contaminated tracks (a deployable confidence gate for the #18 router) but cannot replace the oracle reference for fine fidelity ranking — an honest partial completion of the reference-free separator-quality item. `experimental/frontier`; see `results/frontier/emotion_fidelity_meter/FINDINGS.md`.
20. The CER-tuned hallucination-cure gates are objective-blind too — but the speaker gate damages emotion least (closing loop). `src/gate_emotion_cost.py` (128 babble tracks) measures whether the #11/#12 gates damage emotional prosody while curing CER. Both do: flatness gate CER benefit +0.397 / emotion cost +0.057; speaker gate CER benefit +0.460 / emotion cost +0.023 (emotion cost = prosody distortion added vs raw separation; >0 = damage). So curing the transcript moves prosody away from the clean source — the cures are objective-blind, extending #14's thesis to the cures themselves. But the speaker gate (#13's winner) DOMINATES on both axes (cures more, damages less), reinforcing #13's "use the speaker gate" from the emotion side. The emotion cost (+0.02–0.06) is second-order vs the separate-vs-mixed decision (#14: +0.1–0.3), so the deploy ordering is: first decide separate-vs-mixed (decoupled per #18), then prefer the speaker gate (least emotion-damaging cure), then gate confidence with the #19 meter. `experimental/frontier`; see `results/frontier/gate_emotion_cost/FINDINGS.md`.
21. The separation-tax hallucination is a CONFIDENT, two-mode, causally-early-detectable phenomenon — the detector that closes the loop on #11/separation_tax is an OUTPUT signal (compression-ratio, fires at ~20% of the stream, AFTER the repetition is emitted); the internal-state trip-wire fires ~10× earlier. `src/causal_hallucination_probe.py` (issue #855) looks INSIDE Whisper for the first time in this repo. A 3-second tone-in-silence smoke test reproduces the loop (token ×224) with avg_logprob=−0.065 (highly confident) — the loop is a confident attractor, not a confidence collapse. A two-phase case-control study (exhaustive discovery: 14/165 con×pro pairings catastrophic at r=0.1; analysis: 26 catastrophic vs 40 clean conditions) finds: (a) H-M SUPPORTED — catastrophic routes decode with HIGHER confidence (avg_logprob −0.335 vs −0.739) and LOWER token-id entropy (1.49 vs 2.33); honest refinement: `no_speech_prob` is anti-correlated (AUC 0.33), NOT the signal — the confident-loop signature is decoder-confidence + low entropy + high dominant-token fraction, and the smoke-test "encoder says silent" was near-pure-silence-specific; extends the 2025-26 confident-attractor line (Aparin/Waldendorf/Calm-Whisper/Viakhirev) to the separation-tax regime. (b) TWO MODES — Mode R (repetition, con_001/002/010-driven, 11/26) vs Mode N (non-repetition, pro_006-driven, 15/26); explains why no single reference-free detector dominates. (c) H-D SUPPORTED for Mode R — the novel token-id repetition lock-in trip-wire (any period-p loop of ≥3 reps; p=1 single-token … p=6 phrase) fires at ~2% of the stream vs CR at ~20% (~10× earlier); AUC-broad CR (0.996) still the best single detector. (d) H-C scoped — at tight causal caps (streaming-realistic 0.05–0.15) causal-internal beats causal-CR (catches the repetition tail CR cannot see early); at loose caps CR's two-mode coverage wins; neither alone dominates → deployable design is a union. Litreview (`docs/frontier/causal_hallucination_probe_litreview.md`, 6-agent sweep) grounded novelty: token-id lock-in + offline-CR-router gain-decay-under-prefix-forcing are the unoccupied slots; the confident-loop mechanism and separation-crossover are cited bedrock, not our discovery. `experimental/frontier`; see `results/frontier/causal_hallucination_probe/FINDINGS.md`.

22. STATISTICAL ROBUSTNESS AUDIT — under Benjamini-Hochberg correction at q=0.05, only **6 of 17** directional frontier findings survive (F01, F10, F14, F18, F19, F21); including the F15 null (consistent with H0), 7 of 21 are supported. The pre-registered H3 threshold (≥15 survive) is **NOT SUPPORTED**. **11 claims are downgraded** from "demonstrates" to "suggests": F02 (gold_benefit_separation, n=3 underpowered), F03 (repetition_hallucination_mechanism), F05 (router_v1_fails_synthetic), F06 (router_v2_improves_synthetic), F07 (risk_aware_not_best_cer, n=5 underpowered), F11 (noise_robust_gate_cure), F12 (speaker_gate_moderate_babble), F13 (gate_selector_falsified — non-sig is consistent with the falsification), F16 (lexical_tax_cer_reproduction), F17 (llm_repair_net_harm), F20 (gate_emotion_cost_speaker_least). **9 of the 11 are additionally underpowered** (observed |effect| < MDE at 80% power), so their BH failure is ambiguous, not disconfirming. The frontier's quantitative backbone is real but narrow: a few large, well-powered effects carry it; the rest are suggestive. REPORT.md §18 cites this audit and downgrades claim language throughout. `experimental/frontier`; reanalysis only, no gold tables touched; see `results/frontier/statistical_robustness/FINDINGS.md`. Reproduce: `python3 results/frontier/statistical_robustness/bh_correction.py`.
23. AISHELL-4 EXTERNAL VALIDATION — the first external benchmark test of router v2. On a real AISHELL-4 meeting (M_R003S02C01, 38.5 min, 6 speakers, 77×30s windows, cpWER via MeetEval): **H1a NOT SUPPORTED** — router v2 cpWER 1.206 is slightly worse than always-mixed 1.173 (paired bootstrap 95% CI [−0.152, +0.186] crosses zero); the router's NoOverlap rule does not transfer because oracle-TextGrid separated tracks carry silence that triggers Whisper hallucination. **H1b SUPPORTED (stronger than gold)** — mixed < separated at ALL overlap levels (NoOverlap Δ=+0.20, LightOverlap Δ=+0.64, MidOverlap Δ=+0.72), not just low overlap. **Boundary condition (most important):** the gold "separated wins at HeavyOverlap" does NOT transfer to AISHELL-4 (cpWER 1.667 vs 1.000) — the routing boundary depends on separation quality (silence gaps), not just overlap ratio. The H1a falsification is a boundary condition, not a disproof of the core phenomenon (the separation tax replicates). Honest scope: single meeting, oracle separation, Whisper-tiny only. `external/sanity-check`; does NOT overwrite gold references; see `results/external_sanity_check/aishell4/FINDINGS.md`.
24. POMDP DECISION-THEORETIC FRAMEWORK — closes Gap T1 (no formal theoretical framework for the routing decision). A first-principles POMDP (states = overlap × noise × objective; actions = {mixed, separated, gate_flatness, gate_speaker}; reward = −(text_regret + λ·emotion_regret); all rewards estimated from existing #11/#14/#18/#20 data, no new data) **P1 SUPPORTED**: the POMDP-optimal text route matches router v2 at all 5 overlap strata; crossover divergence 0.03 (POMDP 0.20 vs router v2 r\*=0.17, well inside the ±0.1 threshold). This elevates the routing boundary from "the data says so" to "a decision-theoretic model predicts it." The POMDP also **predicts #18's decoupling**: text and emotion routes disagree at low overlap (ov 0.1, the largest coupling cost +0.119) and agree at the extremes (ov 0.0/0.9, zero cost) — the model independently rediscovers #18's "always read emotion from the separated track" design rule from the reward structure. Noise-type gate selection also emerges (flatness for white/pink, speaker for babble). Honest limitation: stratum-level discretization captures the sign pattern and largest-cost stratum, not exact mid-overlap magnitudes. `experimental/frontier`; see `results/frontier/decision_theoretic_routing/FINDINGS.md`.
25. EMOTION-ASR ASYMMETRY MECHANISM — explains *why* the #14 asymmetry exists (separation preserves prosody but injects text hallucination). **P2 SUPPORTED (moderate regime) / BOUNDED (catastrophic regime):** the dimensionality gap between prosody (~3 dims) and text (~50k tokens) is ~4 orders of magnitude. At low/mid overlap where separation HURTS high-dim text (mean CER benefit −1.207), low-dim features are preserved (emotion prosody 93%, speaker count 100%, binary usability 64%) — the "low-dim preserved, high-dim hurt" pattern P2 predicts. Bound: in the catastrophic confident-attractor regime (#21, 26 conditions), even 1-dim utterance-length is NOT preserved (compression_ratio 17.9× vs 0.81×) — dimensionality explains the moderate asymmetry but not the extreme collapse. **P3 BORDERLINE (weakly supported):** best pre-decode LOO AUC = 0.623 (no_speech_prob alone, 0.023 above threshold); combined pre-decode model below threshold (0.588); post-decode detection far stronger (compression_ratio LOO AUC 0.846). The attractor is marginally predictable pre-decode but overwhelmingly a post-decode phenomenon. Operational implication: P2 explains *why* #18's decoupled design works (prosody is low-dim and robust to artifacts that break high-dim text decode); pre-decode prevention is weakly supported, post-decode detection (#21) remains the deployable cure. `experimental/frontier`; reanalysis of #14/#18/#21, no new data; see `results/frontier/emotion_asr_asymmetry/FINDINGS.md`.
26. VENUE ANALYSIS — Interspeech 2026 is the primary target venue (theme "Speaking Together" is a near-perfect match for the overlap-aware routing thesis; double-blind review; long-paper 8+2 page track fits the multi-finding story; ~48% acceptance). Critical evidence path for submission: RQ1 (AISHELL-4, done — §19/finding #23) + RQ2 (one realistic-separator condition, open) + RQ3 (statistical robustness, done — §18/finding #22). The H1a falsification is an honest boundary condition a double-blind reviewer will credit, not a blocker. Backup: ICASSP 2027 (4+1 pages, confident-attractor focus). Journal extension: IEEE TASLP (full 21-finding story; requires RQ1–RQ4; the POMDP framework #24 and emotion mechanism #25 are the differentiators). `experimental/frontier`; see `RESEARCH/overlap-aware-speaker-asr/framing/venue_analysis.md`.
