...
Stage 30 adds a risk-guarded policy sweep for that calibrated gate. The guard uses only mixed-audio AudioDepth summary proxies: uncertainty mean and spread, overlap mean, and their overlap-uncertainty product. It allows direct `easy_mixed` and `likely_separation_helpful` bypass only when confidence and risk constraints pass; otherwise it falls back to Stage-2 text routing or marks review-like samples. On controlled_v2, the balanced policy reaches CER `0.529082`, route accuracy `0.833333`, direct-bypass false-safe rate `0.000000`, and text-probe reduction `0.416667`. The aggressive policy reaches CER `0.537082` with text-probe reduction `0.650000`, also with direct-bypass false-safe `0.000000`. This is a stronger deployable AudioDepth triage result than the raw calibrated sweep, but it is still not a stable replacement claim: `12` high-error mixed selections remain after Stage-2 fallback/review handling, the references remain `silver_plus_unverified`, and the Stage 27 route-winner/oracle row remains lower.

Stage 32 tests a Generative AudioDepth formulation inspired by perception-as-generation: instead of asking a small classifier to pick a route directly, it builds a promptable acoustic map dataset with task tokens for overlap maps, dominance maps, uncertainty maps, route-regret vectors, and review-risk maps. The dataset currently contains `60` source-disjoint controlled_v2 samples and `300` task rows. Teacher maps may use source tracks only for analysis; deployable student inputs are restricted to mixed-audio / mixed-logmel metadata, and the split audit keeps source utterance IDs plus counterfactual families disjoint.

The first-pass result is useful but deliberately modest. The promptable prototype improves map MAE over an unconditioned map baseline (`0.241263` versus `0.246685`), and sample-level route-regret selection improves fixed mixed CER on the source-disjoint test split (`0.671608` versus `0.739509`). However, it remains worse than the stronger Stage 30/31 risk-guarded AudioDepth gate, leaves `4` false-safe mixed selections, and has only proxy counterfactual pairs rather than exact same-source overlap edits. The correct interpretation is therefore not "new main router"; it is "a promising auxiliary frontier for interpretable acoustic map generation and route-regret diagnostics."

Stage 33 audits that claim more strictly. The first important finding is negative: the Stage 32 split is clean by source pair and counterfactual family, but not by individual source utterance. Because the controlled_v2 source-token graph is connected, a fully strict source-token-disjoint evaluation must drop cross-partition samples. The resulting reliability subset has train `16`, validation `7`, and test `9` samples, with zero source-utterance, source-pair, counterfactual-family, or mixed-wav leakage. On that stricter test, generated maps beat shuffled/zero maps and add route-gap-bucket information over logmel (`1.000000` versus `0.888889`), but they do not add review-risk accuracy beyond logmel on this tiny slice.

The strongest Stage 33 result is safety-oriented regret ranking rather than map generation. The R0 Stage 32-style regret regression has selected CER `0.593715` and false-safe count `4`; adding pairwise route ranking reduces false-safe count to `0` and selected CER to `0.519641`. The calibrated review-head and safe-fusion variants also keep false-safe at `0`, but they do so with high abstention/review rates (`0.777778` or higher on the strict test split). Counterfactual reliability is encouraging but still map-level: monotonic consistency is `0.966667`, with a dominance/SDR failure case. The final decision remains conservative: Generative AudioDepth should be kept as a safety/interpretable auxiliary module, not promoted to a standalone router.
