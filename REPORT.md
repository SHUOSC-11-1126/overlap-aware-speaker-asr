...
Stage 30 adds a risk-guarded version of that calibrated gate. The guard uses only mixed-audio AudioDepth summary proxies: uncertainty mean and spread, overlap mean, and their overlap-uncertainty product. It keeps confident `easy_mixed` and `likely_separation_helpful` direct decisions, but if the Stage-2 fallback would choose mixed on a high-risk AudioDepth map, it overrides that fallback to separated. On controlled_v2, the selected setting uses confidence `0.30` and risk quantile `0.45`, reaches CER `0.525160`, keeps text-probe reduction at `0.716667`, and reduces false-safe rate to `0.050000` with `9` mixed-fallback overrides. This is a stronger deployable AudioDepth frontier result than the raw calibrated sweep, but it is still not a stable replacement claim because the references remain `silver_plus_unverified` and the Stage 27 route-winner/oracle row remains lower.
