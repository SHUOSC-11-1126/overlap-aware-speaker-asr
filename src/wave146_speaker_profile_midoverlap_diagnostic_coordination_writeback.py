...
RECEIPT_COLUMNS = [
    "execution_status",
    "coordination_scope",
    "wave146_closure_status",
    "midoverlap_prior_coordination_status",
    "wave140_refresh_status",
...
def assert_writeback_preconditions(
    wave146_receipt: dict[str, Any],
    demo_wave146_fill: dict[str, Any],
    midoverlap_receipt: dict[str, Any],
    wave140_refresh_receipt: dict[str, Any],
    lightoverlap_refresh_receipt: dict[str, Any],
) -> None:
    if str(wave146_receipt.get("execution_status", "")) != "wave146_exploration_baseline_closure_complete":
        raise RuntimeError("Wave146 closure must be complete before MidOverlap diagnostic refresh coordination")
    if str(demo_wave146_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave146 presentation writeback must be filled before MidOverlap coordination")
    if str(demo_wave146_fill.get("storyboard_receipt_status", "")) != "wave146_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave146 storyboard receipt must be wave146_presentation_extension_complete before MidOverlap coordination"
        )
    if str(midoverlap_receipt.get("execution_status", "")) != "speaker_profile_midoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Speaker profile MidOverlap diagnostic coordination must be complete before Wave146 refresh"
        )
    if (
        str(wave140_refresh_receipt.get("execution_status", ""))
        != "wave140_speaker_profile_midoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave140 MidOverlap diagnostic refresh must be complete before Wave146 refresh coordination"
        )
    if (
        ...
        raise RuntimeError(
            "Wave145 LightOverlap diagnostic refresh must be complete before MidOverlap refresh coordination"
        )
    ...
    return [
        {
            ...
            "headline": "MidOverlap separation-harm diagnostic scope refresh after Wave146 closure",
        ...
        {
            "section_id": "wave140_refresh",
            "headline": "Wave140 MidOverlap diagnostic refresh precedes Wave146 refresh",
        ...
        },
        {
            ...
            "headline": "Wave145 LightOverlap diagnostic refresh precedes MidOverlap refresh",
            "artifact_anchor": "results/figures/wave145_speaker_profile_lightoverlap_diagnostic_coordination_card.md",
            "coordination_note": "Overlap diagnostic chain progresses LightOverlap → MidOverlap through coordinated refreshes.",
            "result_label": "experimental/frontier",
        },
        {
            "section_id": "wave146_boundary",
            "headline": "Wave146 closure defers overlap embedding execution to diagnostic boundaries only",
            "artifact_anchor": "results/figures/wave146_exploration_baseline_closure_card.md",
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave146_speaker_profile_midoverlap_diagnostic_coordination_card",
        "coordination_section_count": str(len(rows)),
        "diagnostic_case_scope": "MidOverlap",
        "candidate_case_scope": "HeavyOverlap",
        "execution_receipt_status": "wave146_speaker_profile_midoverlap_diagnostic_coordination_complete",
        "blocker": "attribution_claims_still_blocked",
        "fill_note": (
            "Documented MidOverlap diagnostic refresh after Wave146 closure; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave146_speaker_profile_midoverlap_diagnostic_coordination_complete",
        "coordination_scope": "wave146_speaker_profile_midoverlap_diagnostic",
        "wave146_closure_status": str(wave146_receipt.get("execution_status", "")),
        "midoverlap_prior_coordination_status": str(midoverlap_receipt.get("execution_status", "")),
        "wave140_refresh_status": str(wave140_refresh_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave146 closure, demo wave146, Wave9 MidOverlap, Wave140 refresh, and Wave145 LightOverlap refresh."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave146 Speaker Profile MidOverlap Diagnostic Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave146 Speaker Profile MidOverlap Diagnostic Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave146 Speaker Profile MidOverlap Diagnostic Coordination Receipt",
        "",
        "| execution_status | wave140_refresh_status | lightoverlap_refresh_status |",
        "| --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['wave140_refresh_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave146_speaker_profile_midoverlap_diagnostic_coordination_card.csv"
    card_json = tables_dir / "wave146_speaker_profile_midoverlap_diagnostic_coordination_card.json"
    card_md = figures_dir / "wave146_speaker_profile_midoverlap_diagnostic_coordination_card.md"
    fill_csv = tables_dir / "wave146_speaker_profile_midoverlap_diagnostic_coordination_writeback.csv"
    fill_json = tables_dir / "wave146_speaker_profile_midoverlap_diagnostic_coordination_writeback.json"
    fill_md = figures_dir / "wave146_speaker_profile_midoverlap_diagnostic_coordination_writeback.md"
    receipt_json = tables_dir / "wave146_speaker_profile_midoverlap_diagnostic_coordination_receipt.json"
    receipt_md = figures_dir / "wave146_speaker_profile_midoverlap_diagnostic_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    wave146_receipt = load_json_dict("results/tables/wave146_exploration_baseline_closure_receipt.json")
    demo_wave146_fill = load_json_dict("results/tables/demo_wave146_presentation_writeback.json")
    midoverlap_receipt = load_json_dict(
        "results/tables/speaker_profile_midoverlap_diagnostic_coordination_receipt.json"
    )
    wave140_refresh_receipt = load_json_dict(
    ...
    assert_writeback_preconditions(
        ...
        demo_wave146_fill,
        midoverlap_receipt,
        wave140_refresh_receipt,
        lightoverlap_refresh_receipt,
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave146_speaker_profile_midoverlap_diagnostic_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict(
            "results/tables/wave146_speaker_profile_midoverlap_diagnostic_coordination_receipt.json"
        )
        if str(existing.get("execution_status", "")) == "wave146_speaker_profile_midoverlap_diagnostic_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave146_receipt, midoverlap_receipt, wave140_refresh_receipt, lightoverlap_refresh_receipt
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave146 speaker profile MidOverlap diagnostic refresh after Wave146 closure."
    )
