...
from .demo_wave17_presentation_writeback import build_extended_polish_rows as build_wave17_polish_rows
...
def assert_extension_preconditions(
    ...
    wave17_fill: dict[str, Any],
) -> None:
    if str(wave18_receipt.get("execution_status", "")) != "wave18_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave18 exploration baseline closure must be complete before wave18 demo extension"
        )
    if str(wave17_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave17 presentation writeback must be filled before wave18 extension")
    if str(wave17_fill.get("storyboard_receipt_status", "")) != "wave17_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave17 storyboard receipt must be wave17_presentation_extension_complete before wave18 extension"
        )


def build_wave18_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave18",
        "headline": "Wave18 exploration+baseline closure and HeavyOverlap diagnostic refresh boundary",
        "artifact_anchor": "results/figures/wave18_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave18 closure card only; HeavyOverlap diagnostic and LLM critic dry-run remain "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave17_polish_rows()
    if any(row["section_id"] == "frontier_wave18" for row in rows):
        return rows
    return rows + [build_wave18_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave18_presentation_extension_complete",
        "storyboard_scope": "wave18_storyboard_extension",
        "expected_inputs": "Wave18 exploration+baseline closure card.",
        "expected_outputs": "Wave18 storyboard extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave18 storyboard extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave18_presentation_extension_complete",
        "walkthrough_scope": "wave18_walkthrough_extension",
        "expected_inputs": "Wave18 exploration+baseline closure card.",
        "expected_outputs": "Wave18 walkthrough extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave18 walkthrough extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave18_presentation_polish_extension",
        "storyboard_receipt_status": "wave18_presentation_extension_complete",
        "walkthrough_receipt_status": "wave18_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave18 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave18_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave18_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave18_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave18 Presentation Writeback",
                "",
...
def run_wave18_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave18_receipt = load_json_dict("results/tables/wave18_exploration_baseline_closure_receipt.json")
    wave17_fill = load_json_dict("results/tables/demo_wave17_presentation_writeback.json")
    assert_extension_preconditions(wave18_receipt, wave17_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave18_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave18_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave18 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave18 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave18_presentation_writeback(force=args.force)
