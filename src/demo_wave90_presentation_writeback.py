...
from .demo_wave89_presentation_writeback import build_extended_polish_rows as build_wave89_polish_rows
...
def assert_extension_preconditions(
    ...
    wave89_fill: dict[str, Any],
) -> None:
    if str(wave90_receipt.get("execution_status", "")) != "wave90_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave90 exploration baseline closure must be complete before wave90 demo extension"
        )
    if str(wave89_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave89 presentation writeback must be filled before wave90 extension")
    if str(wave89_fill.get("storyboard_receipt_status", "")) != "wave89_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave89 storyboard receipt must be wave89_presentation_extension_complete before wave90 extension"
        )


def build_wave90_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave90",
        "headline": "Wave90 exploration+baseline closure and MeetEval speaker-profile diagnostic refresh speaker-profile diagnostic refresh boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave89_polish_rows()
    if any(row["section_id"] == "frontier_wave90" for row in rows):
        return rows
    return rows + [build_wave90_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave90_presentation_extension_complete",
        "storyboard_scope": "wave90_storyboard_extension",
        "expected_inputs": "Wave90 exploration+baseline closure card.",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave90_presentation_extension_complete",
        "walkthrough_scope": "wave90_walkthrough_extension",
        "expected_inputs": "Wave90 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave90_presentation_polish_extension",
        "storyboard_receipt_status": "wave90_presentation_extension_complete",
        "walkthrough_receipt_status": "wave90_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave90_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave90_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave90_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave90 Presentation Writeback",
                "",
...
def run_wave90_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave90_receipt = load_json_dict("results/tables/wave90_exploration_baseline_closure_receipt.json")
    wave89_fill = load_json_dict("results/tables/demo_wave89_presentation_writeback.json")
    assert_extension_preconditions(wave90_receipt, wave89_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave90_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave90_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave90 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave90 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave90_presentation_writeback(force=args.force)
