...
def assert_writeback_preconditions(
    ...
    if str(wave47_receipt.get("execution_status", "")) != "wave47_exploration_baseline_closure_complete":
        raise RuntimeError("Wave47 closure must be complete before external validation narrow slice coordination")
    if (
        str(oppositeoverlap_receipt.get("execution_status", ""))
        != "wave46_speaker_profile_oppositeoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave46 OppositeOverlap diagnostic coordination must be complete before external validation coordination"
        )
    if str(demo_wave47_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave47 presentation writeback must be filled before external validation coordination")
    if str(demo_wave47_fill.get("storyboard_receipt_status", "")) != "wave47_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave47 storyboard receipt must be wave47_presentation_extension_complete before external coordination"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Wave41 external validation narrow slice coordination must be complete before Wave47 refresh"
        )
...
def build_coordination_rows() -> list[dict[str, str]]:
    return [
        ...
        {
            "section_id": "wave41_prior_coordination",
            "headline": "Wave41 external validation narrow slice coordination precedes Wave47 refresh",
            "artifact_anchor": "results/figures/wave41_external_validation_narrow_slice_coordination_card.md",
            "coordination_note": "Wave47 refresh does not replace Wave41 external/sanity-check boundary.",
            "result_label": "external/sanity-check",
        },
        {
            "section_id": "oppositeoverlap_prior",
            "headline": "Wave46 OppositeOverlap diagnostic coordination precedes external validation refresh",
        ...
        },
        {
            ...
            "headline": "Wave47 closure keeps external validation separate from gold baseline CER",
...
def build_fill_row(rows: list[dict[str, str]], slice_id: str) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave47_external_validation_narrow_slice_coordination_card",
        "coordination_section_count": str(len(rows)),
        "narrow_slice_id": slice_id,
        "execution_receipt_status": "wave47_external_validation_narrow_slice_coordination_complete",
        "blocker": "gold_benchmark_claims_still_blocked",
        "fill_note": (
            "Documented external validation narrow slice refresh boundary after Wave46 OppositeOverlap chain; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave47_external_validation_narrow_slice_coordination_complete",
        "coordination_scope": "wave47_external_validation_narrow_slice",
        ...
        "wave41_refresh_status": str(prior_coord_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave47 closure, OppositeOverlap diagnostic, demo wave47, narrow ASR eval receipt, external board."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave47 External Validation Narrow Slice Coordination Card (external/sanity-check)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave47 External Validation Narrow Slice Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave47 External Validation Narrow Slice Coordination Receipt",
        "",
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave47_external_validation_narrow_slice_coordination_card.csv"
    card_json = tables_dir / "wave47_external_validation_narrow_slice_coordination_card.json"
    card_md = figures_dir / "wave47_external_validation_narrow_slice_coordination_card.md"
    fill_csv = tables_dir / "wave47_external_validation_narrow_slice_coordination_writeback.csv"
    fill_json = tables_dir / "wave47_external_validation_narrow_slice_coordination_writeback.json"
    fill_md = figures_dir / "wave47_external_validation_narrow_slice_coordination_writeback.md"
    receipt_json = tables_dir / "wave47_external_validation_narrow_slice_coordination_receipt.json"
    receipt_md = figures_dir / "wave47_external_validation_narrow_slice_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    prior_coord_receipt = load_json_dict(
        "results/tables/wave41_external_validation_narrow_slice_coordination_receipt.json"
    )
    ...
    receipt_path = PROJECT_ROOT / "results/tables/wave47_external_validation_narrow_slice_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave47_external_validation_narrow_slice_coordination_receipt.json")
        if str(existing.get("execution_status", "")) == "wave47_external_validation_narrow_slice_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave47_receipt, oppositeoverlap_receipt, prior_coord_receipt, narrow_eval_receipt, external_summary
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave47 external validation narrow slice coordination after OppositeOverlap chain."
    )
