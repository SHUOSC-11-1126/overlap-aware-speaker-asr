...
from .demo_wave143_presentation_writeback import build_extended_polish_rows as build_wave144_polish_rows
...
def assert_extension_preconditions(
    wave144_receipt: dict[str, Any],
    wave143_fill: dict[str, Any],
) -> None:
    if str(wave144_receipt.get("execution_status", "")) != "wave144_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave143 exploration baseline closure must be complete before wave144 demo extension"
        )
    if str(wave143_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave143 presentation writeback must be filled before wave144 extension")
    if str(wave143_fill.get("storyboard_receipt_status", "")) != "wave143_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave143 storyboard receipt must be wave143_presentation_extension_complete before wave144 extension"
        )


def build_wave144_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave144",
        "headline": "Wave144 exploration+baseline closure after Wave143 external validation chain boundary",
        "artifact_anchor": "results/figures/wave144_exploration_baseline_closure_card.md",
        "presentation_note": (
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave144_polish_rows()
    if any(row["section_id"] == "frontier_wave144" for row in rows):
        return rows
    return rows + [build_wave144_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave144_presentation_extension_complete",
        "storyboard_scope": "wave144_storyboard_extension",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave144_presentation_extension_complete",
        "walkthrough_scope": "wave144_walkthrough_extension",
        "expected_inputs": "Wave104 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave144_presentation_polish_extension",
        "storyboard_receipt_status": "wave144_presentation_extension_complete",
        "walkthrough_receipt_status": "wave144_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave144 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave144_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave144_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave144_presentation_writeback.md"
...
def run_wave144_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave144_receipt = load_json_dict("results/tables/wave144_exploration_baseline_closure_receipt.json")
    wave143_fill = load_json_dict("results/tables/demo_wave143_presentation_writeback.json")
    assert_extension_preconditions(wave144_receipt, wave143_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave144_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave144_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave144 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave144 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave144_presentation_writeback(force=args.force)
