...
from .demo_wave147_presentation_writeback import build_extended_polish_rows as build_wave148_polish_rows
...
def assert_extension_preconditions(
    ...
    wave147_fill: dict[str, Any],
) -> None:
    if str(wave148_receipt.get("execution_status", "")) != "wave148_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave147 exploration baseline closure must be complete before wave148 demo extension"
        )
    if str(wave147_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave147 presentation writeback must be filled before wave148 extension")
    if str(wave147_fill.get("storyboard_receipt_status", "")) != "wave147_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave147 storyboard receipt must be wave147_presentation_extension_complete before wave148 extension"
        )


def build_wave148_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave148",
        "headline": "Wave148 exploration+baseline closure after Wave147 HeavyOverlap chain boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave148_polish_rows()
    if any(row["section_id"] == "frontier_wave148" for row in rows):
        return rows
    return rows + [build_wave148_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave148_presentation_extension_complete",
        "storyboard_scope": "wave148_storyboard_extension",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave148_presentation_extension_complete",
        "walkthrough_scope": "wave148_walkthrough_extension",
        "expected_inputs": "Wave104 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave148_presentation_polish_extension",
        "storyboard_receipt_status": "wave148_presentation_extension_complete",
        "walkthrough_receipt_status": "wave148_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave148 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave148_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave148_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave148_presentation_writeback.md"
...
def run_wave148_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave148_receipt = load_json_dict("results/tables/wave148_exploration_baseline_closure_receipt.json")
    wave147_fill = load_json_dict("results/tables/demo_wave147_presentation_writeback.json")
    assert_extension_preconditions(wave148_receipt, wave147_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave148_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave148_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave148 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave148 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave148_presentation_writeback(force=args.force)
