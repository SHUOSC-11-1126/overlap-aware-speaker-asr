...
def build_status_row(
    ...
    return {
        "scope": "meeteval_cpwer_execution_chain",
        "case_id": case_id,
        ...
        "status_note": (
            "experimental/frontier execution-chain rollup for one verified gold case; "
...
def build_status_lines(row: dict[str, str]) -> list[str]:
    lines = [
        ...
        "",
        "This generated note rolls up the cpWER execution chain status for one verified gold case. "
...
def write_outputs(status_row: dict[str, str]) -> tuple[Path, Path, Path]:
    ...
    csv_path = tables_dir / "meeteval_cpwer_execution_status.csv"
    json_path = tables_dir / "meeteval_cpwer_execution_status.json"
    md_path = figures_dir / "meeteval_cpwer_execution_status.md"
...
def main() -> None:
    preflight = load_json_dict("results/tables/meeteval_cpwer_execution_preflight.json")
    receipt_scaffold = load_json_dict("results/tables/meeteval_cpwer_execution_receipt_scaffold.json")
    ...
    print(f"Wrote MeetEval cpWER execution status CSV: {csv_path.relative_to(PROJECT_ROOT)}")
    print(f"Wrote MeetEval cpWER execution status JSON: {json_path.relative_to(PROJECT_ROOT)}")
    print(f"Wrote MeetEval cpWER execution status note: {md_path.relative_to(PROJECT_ROOT)}")
