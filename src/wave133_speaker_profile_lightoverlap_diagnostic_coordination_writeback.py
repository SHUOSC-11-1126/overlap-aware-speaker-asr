...
RECEIPT_COLUMNS = [
    "execution_status",
    "coordination_scope",
    "wave133_closure_status",
    "wave127_lightoverlap_refresh_status",
...
def assert_writeback_preconditions(
    wave133_receipt: dict[str, Any],
    demo_wave133_fill: dict[str, Any],
    wave127_lightoverlap_receipt: dict[str, Any],
    lightoverlap_receipt: dict[str, Any],
) -> None:
    if str(wave133_receipt.get("execution_status", "")) != "wave133_exploration_baseline_closure_complete":
        raise RuntimeError("Wave133 closure must be complete before LightOverlap diagnostic refresh coordination")
    if str(demo_wave133_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave133 presentation writeback must be filled before LightOverlap coordination")
    if str(demo_wave133_fill.get("storyboard_receipt_status", "")) != "wave133_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave133 storyboard receipt must be wave133_presentation_extension_complete before LightOverlap coordination"
        )
    if (
        str(wave127_lightoverlap_receipt.get("execution_status", ""))
        != "wave127_speaker_profile_lightoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave127 LightOverlap diagnostic refresh must be complete before Wave133 refresh coordination"
        )
    if str(lightoverlap_receipt.get("execution_status", "")) != "speaker_profile_lightoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Speaker profile LightOverlap diagnostic coordination must be complete before Wave133 refresh"
        )
    ...
    return [
        {
            ...
            "headline": "LightOverlap separation-harm diagnostic scope refresh after Wave133 closure",
        ...
        {
            "section_id": "wave127_refresh",
            "headline": "Wave127 LightOverlap diagnostic refresh precedes Wave133 refresh",
        ...
        },
        {
            ...
            "headline": "Wave133 closure defers overlap embedding execution to diagnostic boundaries only",
            "artifact_anchor": "results/figures/wave133_exploration_baseline_closure_card.md",
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave133_speaker_profile_lightoverlap_diagnostic_coordination_card",
        "coordination_section_count": str(len(rows)),
        "diagnostic_case_scope": "LightOverlap",
        "candidate_case_scope": "MidOverlap",
        "execution_receipt_status": "wave133_speaker_profile_lightoverlap_diagnostic_coordination_complete",
        "blocker": "attribution_claims_still_blocked",
        "fill_note": (
            "Documented LightOverlap diagnostic refresh after Wave133 closure; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave133_speaker_profile_lightoverlap_diagnostic_coordination_complete",
        "coordination_scope": "wave133_speaker_profile_lightoverlap_diagnostic",
        "wave133_closure_status": str(wave133_receipt.get("execution_status", "")),
        "wave127_lightoverlap_refresh_status": str(wave127_lightoverlap_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave133 closure, demo wave133 writeback, Wave127 LightOverlap refresh, and Wave8 LightOverlap coordination."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave133 Speaker Profile LightOverlap Diagnostic Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave133 Speaker Profile LightOverlap Diagnostic Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave133 Speaker Profile LightOverlap Diagnostic Coordination Receipt",
        "",
        "| execution_status | wave127_lightoverlap_refresh_status | separation_harm_signal |",
        "| --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['wave127_lightoverlap_refresh_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave133_speaker_profile_lightoverlap_diagnostic_coordination_card.csv"
    card_json = tables_dir / "wave133_speaker_profile_lightoverlap_diagnostic_coordination_card.json"
    card_md = figures_dir / "wave133_speaker_profile_lightoverlap_diagnostic_coordination_card.md"
    fill_csv = tables_dir / "wave133_speaker_profile_lightoverlap_diagnostic_coordination_writeback.csv"
    fill_json = tables_dir / "wave133_speaker_profile_lightoverlap_diagnostic_coordination_writeback.json"
    fill_md = figures_dir / "wave133_speaker_profile_lightoverlap_diagnostic_coordination_writeback.md"
    receipt_json = tables_dir / "wave133_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    receipt_md = figures_dir / "wave133_speaker_profile_lightoverlap_diagnostic_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    wave133_receipt = load_json_dict("results/tables/wave133_exploration_baseline_closure_receipt.json")
    demo_wave133_fill = load_json_dict("results/tables/demo_wave133_presentation_writeback.json")
    wave127_lightoverlap_receipt = load_json_dict(
    ...
    assert_writeback_preconditions(
        wave133_receipt, demo_wave133_fill, wave127_lightoverlap_receipt, lightoverlap_receipt
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave133_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict(
            "results/tables/wave133_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
        )
        if str(existing.get("execution_status", "")) == "wave133_speaker_profile_lightoverlap_diagnostic_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave133_receipt, wave127_lightoverlap_receipt, lightoverlap_receipt)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave133 speaker profile LightOverlap diagnostic refresh after Wave133 closure."
    )
