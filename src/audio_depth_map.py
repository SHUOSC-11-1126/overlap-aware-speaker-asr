...
def generate_one(row: dict[str, Any], mode: str, preview: bool = True) -> dict[str, Any]:
    ...
    if mode == "analysis":
        ...
        if not spk1_path.exists() or not spk2_path.exists():
            raise FileNotFoundError(f"Analysis mode requires separated tracks for {row['sample_id']}")
    ...
    return {
        ...
        "channels": "log_mel|overlap_proxy|uncertainty_proxy"
        if mode == "deployable"
        else "mixed_log_mel|simultaneous_activity|speaker_dominance",
...
def main() -> None:
    ...
    metadata = [generate_one(row, args.mode, preview=args.preview) for row in rows]
    write_csv(
        METADATA_PATH,
        metadata,
        ["sample_id", "mode", "audio_path", "spk1_path", "spk2_path", "map_path", "preview_path", "channels", "label_type"],
    )
