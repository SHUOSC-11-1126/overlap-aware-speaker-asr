...
RECEIPT_COLUMNS = [
    "execution_status",
    "coordination_scope",
    "wave131_closure_status",
    "oppositeoverlap_coordination_status",
    "wave125_refresh_status",
...
def assert_writeback_preconditions(
    wave131_receipt: dict[str, Any],
    oppositeoverlap_receipt: dict[str, Any],
    demo_wave131_fill: dict[str, Any],
    narrow_eval_receipt: dict[str, Any],
    external_summary: dict[str, Any],
    prior_coord_receipt: dict[str, Any],
) -> None:
    if str(wave131_receipt.get("execution_status", "")) != "wave131_exploration_baseline_closure_complete":
        raise RuntimeError("Wave131 closure must be complete before external validation narrow slice coordination")
    if (
        str(oppositeoverlap_receipt.get("execution_status", ""))
        != "wave130_speaker_profile_oppositeoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave130 OppositeOverlap diagnostic coordination must be complete before external validation coordination"
        )
    if str(demo_wave131_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave131 presentation writeback must be filled before external validation coordination")
    if str(demo_wave131_fill.get("storyboard_receipt_status", "")) != "wave131_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave131 storyboard receipt must be wave131_presentation_extension_complete before external coordination"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Wave119 external validation narrow slice coordination must be complete before Wave131 refresh"
        )
...
def build_coordination_rows() -> list[dict[str, str]]:
    return [
        ...
        {
            "section_id": "wave125_prior_coordination",
            "headline": "Wave119 external validation narrow slice coordination precedes Wave131 refresh",
            "artifact_anchor": "results/figures/wave125_external_validation_narrow_slice_coordination_card.md",
        ...
        },
        {
            ...
            "headline": "Wave130 OppositeOverlap diagnostic coordination precedes external validation refresh",
        ...
        {
            "section_id": "wave131_boundary",
            "headline": "Wave131 closure keeps external validation separate from gold baseline CER",
            "artifact_anchor": "results/figures/wave131_exploration_baseline_closure_card.md",
...
def build_fill_row(rows: list[dict[str, str]], slice_id: str) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave131_external_validation_narrow_slice_coordination_card",
        "coordination_section_count": str(len(rows)),
        "narrow_slice_id": slice_id,
        "execution_receipt_status": "wave131_external_validation_narrow_slice_coordination_complete",
        "blocker": "gold_benchmark_claims_still_blocked",
        "fill_note": (
            "Documented external validation narrow slice refresh boundary after Wave130 OppositeOverlap chain; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave131_external_validation_narrow_slice_coordination_complete",
        "coordination_scope": "wave131_external_validation_narrow_slice",
        "wave131_closure_status": str(wave131_receipt.get("execution_status", "")),
        "oppositeoverlap_coordination_status": str(oppositeoverlap_receipt.get("execution_status", "")),
        "wave125_refresh_status": str(prior_coord_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave131 closure, OppositeOverlap diagnostic, demo wave131, narrow ASR eval receipt, external board."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave131 External Validation Narrow Slice Coordination Card (external/sanity-check)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave131 External Validation Narrow Slice Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave131 External Validation Narrow Slice Coordination Receipt",
        "",
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave131_external_validation_narrow_slice_coordination_card.csv"
    card_json = tables_dir / "wave131_external_validation_narrow_slice_coordination_card.json"
    card_md = figures_dir / "wave131_external_validation_narrow_slice_coordination_card.md"
    fill_csv = tables_dir / "wave131_external_validation_narrow_slice_coordination_writeback.csv"
    fill_json = tables_dir / "wave131_external_validation_narrow_slice_coordination_writeback.json"
    fill_md = figures_dir / "wave131_external_validation_narrow_slice_coordination_writeback.md"
    receipt_json = tables_dir / "wave131_external_validation_narrow_slice_coordination_receipt.json"
    receipt_md = figures_dir / "wave131_external_validation_narrow_slice_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    wave131_receipt = load_json_dict("results/tables/wave131_exploration_baseline_closure_receipt.json")
    ...
    demo_wave131_fill = load_json_dict("results/tables/demo_wave131_presentation_writeback.json")
    ...
    prior_coord_receipt = load_json_dict(
        "results/tables/wave125_external_validation_narrow_slice_coordination_receipt.json"
    )
    ...
    receipt_path = PROJECT_ROOT / "results/tables/wave131_external_validation_narrow_slice_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave131_external_validation_narrow_slice_coordination_receipt.json")
        if str(existing.get("execution_status", "")) == "wave131_external_validation_narrow_slice_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave131_receipt, oppositeoverlap_receipt, prior_coord_receipt, narrow_eval_receipt, external_summary
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave131 external validation narrow slice coordination after Wave130 OppositeOverlap chain."
    )
