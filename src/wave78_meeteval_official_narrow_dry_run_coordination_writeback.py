...
def assert_writeback_preconditions(
    ...
    if str(wave78_receipt.get("execution_status", "")) != "wave78_exploration_baseline_closure_complete":
        raise RuntimeError("Wave78 closure must be complete before MeetEval official narrow dry-run refresh coordination")
    if str(demo_wave78_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave78 presentation writeback must be filled before MeetEval official coordination")
    if str(demo_wave78_fill.get("storyboard_receipt_status", "")) != "wave78_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave78 storyboard receipt must be wave78_presentation_extension_complete before MeetEval official coordination"
        )
    if (
        ...
        raise RuntimeError(
            "Wave72 MeetEval official narrow dry-run coordination must be complete before Wave78 refresh"
        )
...
def build_coordination_rows() -> list[dict[str, str]]:
    return [
        ...
        {
            "section_id": "wave72_prior_coordination",
            "headline": "Wave72 official MeetEval narrow dry-run refresh remains the prerequisite chain",
            "artifact_anchor": "results/figures/wave72_meeteval_official_narrow_dry_run_coordination_card.md",
            "coordination_note": "Wave78 refresh does not replace Wave72 official MeetEval boundary documentation.",
            "result_label": "experimental/frontier",
        },
        {
        ...
        {
            "section_id": "wave78_boundary",
            "headline": "Wave78 closure keeps official MeetEval coordination separate from gold baseline CER",
...
def build_fill_row(rows: list[dict[str, str]], official_dry_run_case_count: str) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave78_meeteval_official_narrow_dry_run_coordination_card",
        "coordination_section_count": str(len(rows)),
        "official_dry_run_case_count": official_dry_run_case_count,
        "execution_receipt_status": "wave78_meeteval_official_narrow_dry_run_coordination_complete",
        "blocker": "official_benchmark_claims_still_blocked",
        "fill_note": (
            "Documented MeetEval official narrow dry-run refresh boundary after Wave78 closure; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave78_meeteval_official_narrow_dry_run_coordination_complete",
        "coordination_scope": "wave78_meeteval_official_narrow_dry_run",
        ...
        "expected_inputs": (
            "Wave78 closure, demo wave78, Wave72 official coordination, Wave17 character-level refresh, official execution."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave78 MeetEval Official Narrow Dry-Run Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave78 MeetEval Official Narrow Dry-Run Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave78 MeetEval Official Narrow Dry-Run Coordination Receipt",
        "",
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave78_meeteval_official_narrow_dry_run_coordination_card.csv"
    card_json = tables_dir / "wave78_meeteval_official_narrow_dry_run_coordination_card.json"
    card_md = figures_dir / "wave78_meeteval_official_narrow_dry_run_coordination_card.md"
    fill_csv = tables_dir / "wave78_meeteval_official_narrow_dry_run_coordination_writeback.csv"
    fill_json = tables_dir / "wave78_meeteval_official_narrow_dry_run_coordination_writeback.json"
    fill_md = figures_dir / "wave78_meeteval_official_narrow_dry_run_coordination_writeback.md"
    receipt_json = tables_dir / "wave78_meeteval_official_narrow_dry_run_coordination_receipt.json"
    receipt_md = figures_dir / "wave78_meeteval_official_narrow_dry_run_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    prior_official_receipt = load_json_dict(
        "results/tables/wave72_meeteval_official_narrow_dry_run_coordination_receipt.json"
    )
    ...
    assert_writeback_preconditions(
        wave78_receipt, demo_wave78_fill, prior_official_receipt, character_refresh_receipt
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave78_meeteval_official_narrow_dry_run_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave78_meeteval_official_narrow_dry_run_coordination_receipt.json")
        if str(existing.get("execution_status", "")) == "wave78_meeteval_official_narrow_dry_run_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave78_receipt, prior_official_receipt, character_refresh_receipt, official_count
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave78 MeetEval official narrow dry-run refresh after Wave78 closure."
    )
