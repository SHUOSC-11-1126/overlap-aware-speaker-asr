...
from .demo_wave91_presentation_writeback import build_extended_polish_rows as build_wave91_polish_rows
...
def assert_extension_preconditions(
    wave92_receipt: dict[str, Any],
    wave91_fill: dict[str, Any],
) -> None:
    if str(wave92_receipt.get("execution_status", "")) != "wave92_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave92 exploration baseline closure must be complete before wave92 demo extension"
        )
    if str(wave91_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave91 presentation writeback must be filled before wave92 extension")
    if str(wave91_fill.get("storyboard_receipt_status", "")) != "wave91_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave91 storyboard receipt must be wave91_presentation_extension_complete before wave92 extension"
        )


def build_wave92_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave92",
        "headline": "Wave92 exploration+baseline closure and MidOverlap speaker-profile diagnostic refresh speaker-profile diagnostic refresh boundary",
        "artifact_anchor": "results/figures/wave92_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave81 closure card only; MidOverlap speaker-profile diagnostic refresh remains "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave91_polish_rows()
    if any(row["section_id"] == "frontier_wave92" for row in rows):
        return rows
    return rows + [build_wave92_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave92_presentation_extension_complete",
        "storyboard_scope": "wave92_storyboard_extension",
        "expected_inputs": "Wave92 exploration+baseline closure card.",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave92_presentation_extension_complete",
        "walkthrough_scope": "wave92_walkthrough_extension",
        "expected_inputs": "Wave92 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave92_presentation_polish_extension",
        "storyboard_receipt_status": "wave92_presentation_extension_complete",
        "walkthrough_receipt_status": "wave92_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave92_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave92_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave92_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave92 Presentation Writeback",
                "",
...
def run_wave92_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave92_receipt = load_json_dict("results/tables/wave92_exploration_baseline_closure_receipt.json")
    wave91_fill = load_json_dict("results/tables/demo_wave91_presentation_writeback.json")
    assert_extension_preconditions(wave92_receipt, wave91_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave92_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave92_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave92 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave92 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave92_presentation_writeback(force=args.force)
