...
def assert_writeback_preconditions(
    ...
    if str(wave15_receipt.get("execution_status", "")) != "wave15_exploration_baseline_closure_complete":
        raise RuntimeError("Wave15 closure must be complete before external validation narrow slice coordination")
    if str(demo_wave15_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave15 presentation writeback must be filled before external validation coordination")
    if str(demo_wave15_fill.get("storyboard_receipt_status", "")) != "wave15_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave15 storyboard receipt must be wave15_presentation_extension_complete before external coordination"
        )
    ...
    if str(prior_coord_receipt.get("execution_status", "")) != "external_validation_narrow_slice_coordination_complete":
        raise RuntimeError(
            "Wave11 external validation narrow slice coordination must be complete before Wave15 refresh"
        )
...
def build_coordination_rows() -> list[dict[str, str]]:
    return [
        ...
        {
            "section_id": "wave11_prior_coordination",
            "headline": "Wave11 external validation narrow slice coordination remains the prerequisite chain",
            "artifact_anchor": "results/figures/external_validation_narrow_slice_coordination_card.md",
            "coordination_note": "Wave15 refresh does not replace Wave11 external/sanity-check boundary.",
            "result_label": "external/sanity-check",
        },
        {
            "section_id": "wave15_boundary",
            "headline": "Wave15 closure keeps external validation separate from gold baseline CER",
        ...
        {
            "section_id": "gold_claim_boundary",
            "headline": "Narrow slice refresh coordination does not upgrade external sanity-check to gold evaluation",
            "artifact_anchor": "results/figures/external_validation_narrow_audio_eval.md",
            "coordination_note": "README mentions must keep external/sanity-check labeling.",
            "result_label": "external/sanity-check",
        },
    ]
...
def build_fill_row(rows: list[dict[str, str]], slice_id: str) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave15_external_validation_narrow_slice_coordination_card",
        "coordination_section_count": str(len(rows)),
        "narrow_slice_id": slice_id,
        "execution_receipt_status": "wave15_external_validation_narrow_slice_coordination_complete",
        "blocker": "gold_benchmark_claims_still_blocked",
        "fill_note": (
            "Documented external validation narrow slice refresh boundary after Wave15 closure; "
...
def build_receipt_row(
    ...
    return {
        ...
        "coordination_scope": "wave15_external_validation_narrow_slice",
        ...
        "expected_inputs": (
            "Wave15 closure, demo wave15 writeback, narrow ASR eval receipt, and external validation board."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave15 External Validation Narrow Slice Coordination Card (external/sanity-check)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave15 External Validation Narrow Slice Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave15 External Validation Narrow Slice Coordination Receipt",
        "",
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave15_external_validation_narrow_slice_coordination_card.csv"
    card_json = tables_dir / "wave15_external_validation_narrow_slice_coordination_card.json"
    card_md = figures_dir / "wave15_external_validation_narrow_slice_coordination_card.md"
    fill_csv = tables_dir / "wave15_external_validation_narrow_slice_coordination_writeback.csv"
    fill_json = tables_dir / "wave15_external_validation_narrow_slice_coordination_writeback.json"
    fill_md = figures_dir / "wave15_external_validation_narrow_slice_coordination_writeback.md"
    receipt_json = tables_dir / "wave15_external_validation_narrow_slice_coordination_receipt.json"
    receipt_md = figures_dir / "wave15_external_validation_narrow_slice_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    prior_coord_receipt = load_json_dict(
        "results/tables/external_validation_narrow_slice_coordination_receipt.json"
    )
    assert_writeback_preconditions(
        wave15_receipt, demo_wave15_fill, narrow_eval_receipt, external_summary, prior_coord_receipt
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave15_external_validation_narrow_slice_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave15_external_validation_narrow_slice_coordination_receipt.json")
        if str(existing.get("execution_status", "")) == "wave15_external_validation_narrow_slice_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave15_receipt, narrow_eval_receipt, external_summary)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave15 external validation narrow slice coordination after Wave15 closure."
    )
