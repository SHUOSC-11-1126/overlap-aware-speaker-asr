...
RECEIPT_COLUMNS = [
    ...
    "lightoverlap_diagnostic_status",
    "frontier_go_count",
    "expected_inputs",
    "writeback_note",
]
...
def assert_closure_preconditions(
    ...
    demo_wave8_fill: dict[str, Any],
    frontier_summary: dict[str, Any],
) -> None:
    if str(wave8_receipt.get("execution_status", "")) != "wave8_exploration_baseline_closure_complete":
        raise RuntimeError("Wave8 closure receipt must be complete before Wave9 exploration baseline closure")
    if str(evidence_receipt.get("execution_status", "")) != "cascade_benchmark_evidence_receipt_coordination_complete":
        raise RuntimeError("Cascade benchmark evidence receipt coordination must be complete before Wave9 closure")
    if str(lightoverlap_receipt.get("execution_status", "")) != "speaker_profile_lightoverlap_diagnostic_coordination_complete":
        raise RuntimeError("Speaker profile LightOverlap diagnostic coordination must be complete before Wave9 closure")
    if str(demo_wave8_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave8 presentation writeback must be filled before Wave9 closure")
...
def build_closure_rows() -> list[dict[str, str]]:
    return [
        {
            "section_id": "wave8_rollup",
            "headline": "Wave8 closed: LightOverlap diagnostic + evidence receipt + demo + exploration closure",
        ...
        },
        {
            ...
            "headline": "LightOverlap diagnostic scope coordinated; MidOverlap deferred",
            "artifact_anchor": "results/figures/speaker_profile_lightoverlap_diagnostic_coordination_card.md",
            "coordination_note": "Diagnostic coordination only; no overlap embedding execution claimed.",
            "result_label": "experimental/frontier",
        },
        {
            "section_id": "evidence_receipt_boundary",
            "headline": "Benchmark evidence receipt scaffold coordinated; phase1 not executed",
            "artifact_anchor": "results/figures/cascade_benchmark_evidence_receipt_coordination_card.md",
            "coordination_note": "template_only_not_executed; controlled timing still pending.",
            "result_label": "experimental/frontier",
        },
        {
        ...
        },
        {
            ...
            "coordination_note": "Wave9 closure does not record controlled hardware benchmark sessions.",
            "result_label": "experimental/frontier",
        },
    ]
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave9_exploration_baseline_closure_card",
        "coordination_section_count": str(len(rows)),
        "execution_receipt_status": "wave9_exploration_baseline_closure_complete",
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": (
            "Wave9 exploration+baseline closure rollup after Wave8 chain; stable gold baseline preserved."
        ),
    }
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave9_exploration_baseline_closure_complete",
        "coordination_scope": "wave9_exploration_baseline",
        ...
        "lightoverlap_diagnostic_status": str(lightoverlap_receipt.get("execution_status", "")),
        "frontier_go_count": str(frontier_summary.get("go_count", "")),
        "expected_inputs": (
            "Wave8 closure, evidence receipt coordination, LightOverlap diagnostic, demo wave8, frontier summary."
        ),
        "writeback_note": (
            "探索+基线 Wave9 closure writeback; qualitative/demo and experimental/frontier labels preserved."
        ),
    }
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave9 Exploration+Baseline Closure Card",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave9 Exploration+Baseline Closure Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave9 Exploration+Baseline Closure Receipt",
        "",
        "| execution_status | wave8_closure_status | frontier_go_count |",
        "| --- | --- | ---: |",
        (
            f"| {row['execution_status']} | {row['wave8_closure_status']} | {row['frontier_go_count']} |"
        ),
    ]
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave9_exploration_baseline_closure_card.csv"
    card_json = tables_dir / "wave9_exploration_baseline_closure_card.json"
    card_md = figures_dir / "wave9_exploration_baseline_closure_card.md"
    fill_csv = tables_dir / "wave9_exploration_baseline_closure_writeback.csv"
    fill_json = tables_dir / "wave9_exploration_baseline_closure_writeback.json"
    fill_md = figures_dir / "wave9_exploration_baseline_closure_writeback.md"
    receipt_json = tables_dir / "wave9_exploration_baseline_closure_receipt.json"
    receipt_md = figures_dir / "wave9_exploration_baseline_closure_receipt.md"
...
def run_closure_writeback(force: bool = False) -> dict[str, str]:
    ...
    demo_wave8_fill = load_json_dict("results/tables/demo_wave8_presentation_writeback.json")
    frontier_summary = load_json_dict("results/tables/frontier_go_no_go_summary.json")
    assert_closure_preconditions(
        wave8_receipt, evidence_receipt, lightoverlap_receipt, demo_wave8_fill, frontier_summary
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave9_exploration_baseline_closure_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave9_exploration_baseline_closure_receipt.json")
        if str(existing.get("execution_status", "")) == "wave9_exploration_baseline_closure_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave8_receipt, evidence_receipt, lightoverlap_receipt, frontier_summary)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Write Wave9 exploration+baseline closure after Wave8 chain.")
