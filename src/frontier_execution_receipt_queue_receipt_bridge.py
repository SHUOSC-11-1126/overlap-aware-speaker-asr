...
def build_receipt_bridge_row(operator_brief: dict[str, str]) -> dict[str, str]:
    ...
    return {
        ...
        "bridge_note": (
            f"Open the receipt queue operator brief first, then write back through {receipt_target} "
...
def build_receipt_bridge_lines(row: dict[str, str]) -> list[str]:
    return [
        ...
        "",
        "This generated bridge connects the receipt queue operator brief to the current execution receipt target. "
...
def write_outputs(row: dict[str, str]) -> tuple[Path, Path, Path]:
    ...
    csv_path = tables_dir / "frontier_execution_receipt_queue_receipt_bridge.csv"
    json_path = tables_dir / "frontier_execution_receipt_queue_receipt_bridge.json"
    md_path = figures_dir / "frontier_execution_receipt_queue_receipt_bridge.md"
...
def main() -> None:
    ...
    if not row:
        print("Receipt queue operator brief not found; receipt bridge not written.")
        return
    csv_path, json_path, md_path = write_outputs(row)
    print(
        "Wrote frontier execution receipt queue receipt bridge CSV: "
        f"{csv_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote frontier execution receipt queue receipt bridge JSON: "
        f"{json_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote frontier execution receipt queue receipt bridge note: "
