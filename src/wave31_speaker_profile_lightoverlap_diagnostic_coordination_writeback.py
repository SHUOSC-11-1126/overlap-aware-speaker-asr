...
RECEIPT_COLUMNS = [
    "execution_status",
    "coordination_scope",
    "wave31_closure_status",
    "lightoverlap_prior_coordination_status",
    "wave27_refresh_status",
...
def assert_writeback_preconditions(
    wave31_receipt: dict[str, Any],
    demo_wave31_fill: dict[str, Any],
    lightoverlap_receipt: dict[str, Any],
    wave27_refresh_receipt: dict[str, Any],
) -> None:
    if str(wave31_receipt.get("execution_status", "")) != "wave31_exploration_baseline_closure_complete":
        raise RuntimeError("Wave31 closure must be complete before LightOverlap diagnostic refresh coordination")
    if str(demo_wave31_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave31 presentation writeback must be filled before LightOverlap coordination")
    if str(demo_wave31_fill.get("storyboard_receipt_status", "")) != "wave31_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave31 storyboard receipt must be wave31_presentation_extension_complete before LightOverlap coordination"
        )
    if str(lightoverlap_receipt.get("execution_status", "")) != "speaker_profile_lightoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Speaker profile LightOverlap diagnostic coordination must be complete before Wave31 refresh"
        )
    if (
        str(wave27_refresh_receipt.get("execution_status", ""))
        != "wave27_speaker_profile_lightoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave27 LightOverlap diagnostic refresh must be complete before Wave31 refresh coordination"
        )
    ...
    return [
        {
            ...
            "headline": "LightOverlap separation-harm diagnostic scope refresh after Wave31 closure",
        ...
        },
        {
            ...
            "coordination_note": "Wave31 refresh does not replace original LightOverlap boundary documentation.",
            "result_label": "experimental/frontier",
        },
        {
            "section_id": "wave27_refresh",
            "headline": "Wave27 LightOverlap diagnostic refresh precedes Wave31 refresh",
        ...
        {
            "section_id": "wave31_boundary",
            "headline": "Wave31 closure defers overlap embedding execution to diagnostic boundaries only",
            "artifact_anchor": "results/figures/wave31_exploration_baseline_closure_card.md",
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave31_speaker_profile_lightoverlap_diagnostic_coordination_card",
        "coordination_section_count": str(len(rows)),
        "diagnostic_case_scope": "LightOverlap",
        "candidate_case_scope": "MidOverlap",
        "execution_receipt_status": "wave31_speaker_profile_lightoverlap_diagnostic_coordination_complete",
        "blocker": "attribution_claims_still_blocked",
        "fill_note": (
            "Documented LightOverlap diagnostic refresh after Wave31 closure; "
...
def build_receipt_row(
    ...
    return {
        ...
        "coordination_scope": "wave31_speaker_profile_lightoverlap_diagnostic",
        "wave31_closure_status": str(wave31_receipt.get("execution_status", "")),
        "lightoverlap_prior_coordination_status": str(lightoverlap_receipt.get("execution_status", "")),
        "wave27_refresh_status": str(wave27_refresh_receipt.get("execution_status", "")),
        "separation_harm_signal": "separation_hurts_on_lightoverlap",
        "expected_inputs": (
            "Wave31 closure, demo wave31 writeback, Wave8 LightOverlap coordination, and Wave27 refresh."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave31 Speaker Profile LightOverlap Diagnostic Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave31 Speaker Profile LightOverlap Diagnostic Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave31 Speaker Profile LightOverlap Diagnostic Coordination Receipt",
        "",
        "| execution_status | wave27_refresh_status | separation_harm_signal |",
        "| --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['wave27_refresh_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave31_speaker_profile_lightoverlap_diagnostic_coordination_card.csv"
    card_json = tables_dir / "wave31_speaker_profile_lightoverlap_diagnostic_coordination_card.json"
    card_md = figures_dir / "wave31_speaker_profile_lightoverlap_diagnostic_coordination_card.md"
    fill_csv = tables_dir / "wave31_speaker_profile_lightoverlap_diagnostic_coordination_writeback.csv"
    fill_json = tables_dir / "wave31_speaker_profile_lightoverlap_diagnostic_coordination_writeback.json"
    fill_md = figures_dir / "wave31_speaker_profile_lightoverlap_diagnostic_coordination_writeback.md"
    receipt_json = tables_dir / "wave31_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    receipt_md = figures_dir / "wave31_speaker_profile_lightoverlap_diagnostic_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    wave31_receipt = load_json_dict("results/tables/wave31_exploration_baseline_closure_receipt.json")
    demo_wave31_fill = load_json_dict("results/tables/demo_wave31_presentation_writeback.json")
    lightoverlap_receipt = load_json_dict(
        "results/tables/speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    )
    wave27_refresh_receipt = load_json_dict(
        "results/tables/wave27_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    )
    assert_writeback_preconditions(
        wave31_receipt, demo_wave31_fill, lightoverlap_receipt, wave27_refresh_receipt
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave31_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict(
            "results/tables/wave31_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
        )
        if str(existing.get("execution_status", "")) == "wave31_speaker_profile_lightoverlap_diagnostic_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave31_receipt, lightoverlap_receipt, wave27_refresh_receipt)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave31 speaker profile LightOverlap diagnostic refresh after Wave31 closure."
    )
