...
def build_summary_row(rows: list[dict[str, str]]) -> dict[str, str]:
    ...
    if no_go_count == 0 and "wave126_presentation_extension_complete" in receipt_statuses:
        overall_state = "presentation_wave126_extension_complete"
        recommended_next_action = (
            "Wave126 presentation extension complete; README/UI refresh remains qualitative/demo only."
        )
        primary_boundary = "none_documented"
    elif no_go_count == 0 and "wave125_presentation_extension_complete" in receipt_statuses:
