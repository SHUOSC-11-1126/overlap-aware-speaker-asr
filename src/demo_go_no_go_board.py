...
def build_summary_row(rows: list[dict[str, str]]) -> dict[str, str]:
    ...
    wave_receipt_re = re.compile(r"^wave(\d+)_presentation_extension_complete$")
    if no_go_count == 0:
        matched_wave = None
        for status in receipt_statuses:
            m = wave_receipt_re.match(status)
            if m:
                matched_wave = m.group(1)
                break
        if matched_wave is not None:
            overall_state = f"presentation_wave{matched_wave}_extension_complete"
            recommended_next_action = (
                f"Wave{matched_wave} presentation extension complete; README/UI refresh remains qualitative/demo only."
            )
