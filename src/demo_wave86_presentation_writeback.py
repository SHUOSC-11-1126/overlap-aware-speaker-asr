...
from .demo_wave85_presentation_writeback import build_extended_polish_rows as build_wave85_polish_rows
...
def assert_extension_preconditions(
    ...
    wave85_fill: dict[str, Any],
) -> None:
    if str(wave86_receipt.get("execution_status", "")) != "wave86_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave86 exploration baseline closure must be complete before wave86 demo extension"
        )
    if str(wave85_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave85 presentation writeback must be filled before wave86 extension")
    if str(wave85_fill.get("storyboard_receipt_status", "")) != "wave85_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave85 storyboard receipt must be wave85_presentation_extension_complete before wave86 extension"
        )


def build_wave86_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave86",
        "headline": "Wave86 exploration+baseline closure and MidOverlap speaker-profile diagnostic refresh speaker-profile diagnostic refresh boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave85_polish_rows()
    if any(row["section_id"] == "frontier_wave86" for row in rows):
        return rows
    return rows + [build_wave86_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave86_presentation_extension_complete",
        "storyboard_scope": "wave86_storyboard_extension",
        "expected_inputs": "Wave86 exploration+baseline closure card.",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave86_presentation_extension_complete",
        "walkthrough_scope": "wave86_walkthrough_extension",
        "expected_inputs": "Wave86 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave86_presentation_polish_extension",
        "storyboard_receipt_status": "wave86_presentation_extension_complete",
        "walkthrough_receipt_status": "wave86_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave86_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave86_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave86_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave86 Presentation Writeback",
                "",
...
def run_wave86_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave86_receipt = load_json_dict("results/tables/wave86_exploration_baseline_closure_receipt.json")
    wave85_fill = load_json_dict("results/tables/demo_wave85_presentation_writeback.json")
    assert_extension_preconditions(wave86_receipt, wave85_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave86_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave86_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave86 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave86 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave86_presentation_writeback(force=args.force)
