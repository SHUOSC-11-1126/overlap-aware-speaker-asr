...
from .demo_wave99_presentation_writeback import build_extended_polish_rows as build_wave99_polish_rows
...
def assert_extension_preconditions(
    ...
    wave99_fill: dict[str, Any],
) -> None:
    if str(wave100_receipt.get("execution_status", "")) != "wave100_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave100 exploration baseline closure must be complete before wave100 demo extension"
        )
    if str(wave99_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave99 presentation writeback must be filled before wave100 extension")
    if str(wave99_fill.get("storyboard_receipt_status", "")) != "wave99_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave99 storyboard receipt must be wave99_presentation_extension_complete before wave100 extension"
        )


def build_wave100_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave100",
        "headline": "Wave100 exploration+baseline closure after Wave99 HeavyOverlap chain boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave99_polish_rows()
    if any(row["section_id"] == "frontier_wave100" for row in rows):
        return rows
    return rows + [build_wave100_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave100_presentation_extension_complete",
        "storyboard_scope": "wave100_storyboard_extension",
        "expected_inputs": "Wave100 exploration+baseline closure card.",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave100_presentation_extension_complete",
        "walkthrough_scope": "wave100_walkthrough_extension",
        "expected_inputs": "Wave100 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave100_presentation_polish_extension",
        "storyboard_receipt_status": "wave100_presentation_extension_complete",
        "walkthrough_receipt_status": "wave100_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave100_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave100_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave100_presentation_writeback.md"
...
def run_wave100_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave100_receipt = load_json_dict("results/tables/wave100_exploration_baseline_closure_receipt.json")
    wave99_fill = load_json_dict("results/tables/demo_wave99_presentation_writeback.json")
    assert_extension_preconditions(wave100_receipt, wave99_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave100_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave100_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave100 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave100 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave100_presentation_writeback(force=args.force)
