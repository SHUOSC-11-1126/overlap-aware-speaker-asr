...
def assert_writeback_preconditions(
    ...
    if str(wave19_receipt.get("execution_status", "")) != "wave19_exploration_baseline_closure_complete":
        raise RuntimeError("Wave19 closure must be complete before OppositeOverlap diagnostic refresh coordination")
    if str(demo_wave19_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave19 presentation writeback must be filled before OppositeOverlap coordination")
    if str(demo_wave19_fill.get("storyboard_receipt_status", "")) != "wave19_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave19 storyboard receipt must be wave19_presentation_extension_complete before OppositeOverlap coordination"
        )
    if str(oppositeoverlap_receipt.get("execution_status", "")) != "speaker_profile_oppositeoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Speaker profile OppositeOverlap diagnostic coordination must be complete before Wave19 refresh"
        )
    if str(heavyoverlap_refresh_receipt.get("execution_status", "")) != "wave18_speaker_profile_heavyoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Wave18 HeavyOverlap diagnostic refresh must be complete before OppositeOverlap refresh coordination"
        )
    ...
    return [
        {
            ...
            "headline": "OppositeOverlap separation-benefit diagnostic scope refresh after Wave19 closure",
        ...
        },
        {
            ...
            "coordination_note": "Wave19 refresh does not replace original OppositeOverlap boundary documentation.",
            "result_label": "experimental/frontier",
        },
        {
            "section_id": "heavyoverlap_refresh",
            "headline": "Wave18 HeavyOverlap diagnostic refresh precedes OppositeOverlap refresh",
        ...
        },
        {
            ...
            "headline": "Wave19 closure defers overlap embedding execution to diagnostic boundaries only",
        ...
        },
        {
            ...
            "headline": "OppositeOverlap refresh does not upgrade diagnostic scope to attribution claims",
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave19_speaker_profile_oppositeoverlap_diagnostic_coordination_card",
        "coordination_section_count": str(len(rows)),
        "diagnostic_case_scope": "OppositeOverlap",
        "candidate_case_scope": "none_remaining",
        "execution_receipt_status": "wave19_speaker_profile_oppositeoverlap_diagnostic_coordination_complete",
        "blocker": "attribution_claims_still_blocked",
        "fill_note": (
            "Documented OppositeOverlap diagnostic refresh after Wave19 closure; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave19_speaker_profile_oppositeoverlap_diagnostic_coordination_complete",
        "coordination_scope": "wave19_speaker_profile_oppositeoverlap_diagnostic",
        ...
        "expected_inputs": (
            "Wave19 closure, demo wave19 writeback, Wave13 OppositeOverlap diagnostic, and Wave18 HeavyOverlap refresh."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave19 Speaker Profile OppositeOverlap Diagnostic Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave19 Speaker Profile OppositeOverlap Diagnostic Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave19 Speaker Profile OppositeOverlap Diagnostic Coordination Receipt",
        "",
        "| execution_status | oppositeoverlap_prior_coordination_status | separation_benefit_signal |",
        "| --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['oppositeoverlap_prior_coordination_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave19_speaker_profile_oppositeoverlap_diagnostic_coordination_card.csv"
    card_json = tables_dir / "wave19_speaker_profile_oppositeoverlap_diagnostic_coordination_card.json"
    card_md = figures_dir / "wave19_speaker_profile_oppositeoverlap_diagnostic_coordination_card.md"
    fill_csv = tables_dir / "wave19_speaker_profile_oppositeoverlap_diagnostic_coordination_writeback.csv"
    fill_json = tables_dir / "wave19_speaker_profile_oppositeoverlap_diagnostic_coordination_writeback.json"
    fill_md = figures_dir / "wave19_speaker_profile_oppositeoverlap_diagnostic_coordination_writeback.md"
    receipt_json = tables_dir / "wave19_speaker_profile_oppositeoverlap_diagnostic_coordination_receipt.json"
    receipt_md = figures_dir / "wave19_speaker_profile_oppositeoverlap_diagnostic_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    assert_writeback_preconditions(
        wave19_receipt, demo_wave19_fill, oppositeoverlap_receipt, heavyoverlap_refresh_receipt
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave19_speaker_profile_oppositeoverlap_diagnostic_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict(
            "results/tables/wave19_speaker_profile_oppositeoverlap_diagnostic_coordination_receipt.json"
        )
        if str(existing.get("execution_status", "")) == "wave19_speaker_profile_oppositeoverlap_diagnostic_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave19_receipt, oppositeoverlap_receipt, heavyoverlap_refresh_receipt)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave19 speaker profile OppositeOverlap diagnostic refresh after Wave19 closure."
    )
