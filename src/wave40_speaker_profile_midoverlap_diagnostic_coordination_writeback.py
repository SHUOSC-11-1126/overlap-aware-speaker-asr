...
RECEIPT_COLUMNS = [
    "execution_status",
    "coordination_scope",
    "wave40_closure_status",
    "midoverlap_prior_coordination_status",
    "wave36_refresh_status",
...
def assert_writeback_preconditions(
    wave40_receipt: dict[str, Any],
    demo_wave40_fill: dict[str, Any],
    midoverlap_receipt: dict[str, Any],
    wave36_refresh_receipt: dict[str, Any],
    lightoverlap_refresh_receipt: dict[str, Any],
) -> None:
    if str(wave40_receipt.get("execution_status", "")) != "wave40_exploration_baseline_closure_complete":
        raise RuntimeError("Wave40 closure must be complete before MidOverlap diagnostic refresh coordination")
    if str(demo_wave40_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave40 presentation writeback must be filled before MidOverlap coordination")
    if str(demo_wave40_fill.get("storyboard_receipt_status", "")) != "wave40_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave40 storyboard receipt must be wave40_presentation_extension_complete before MidOverlap coordination"
        )
    if str(midoverlap_receipt.get("execution_status", "")) != "speaker_profile_midoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Speaker profile MidOverlap diagnostic coordination must be complete before Wave40 refresh"
        )
    if (
        str(wave36_refresh_receipt.get("execution_status", ""))
        != "wave36_speaker_profile_midoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave36 MidOverlap diagnostic refresh must be complete before Wave40 refresh coordination"
        )
    if (
        ...
        raise RuntimeError(
            "Wave39 LightOverlap diagnostic refresh must be complete before MidOverlap refresh coordination"
        )
    ...
    return [
        {
            ...
            "headline": "MidOverlap separation-harm diagnostic scope refresh after Wave40 closure",
        ...
        },
        {
            ...
            "coordination_note": "Wave40 refresh does not replace original MidOverlap boundary documentation.",
            "result_label": "experimental/frontier",
        },
        {
            "section_id": "wave36_refresh",
            "headline": "Wave36 MidOverlap diagnostic refresh precedes Wave40 refresh",
            "artifact_anchor": "results/figures/wave36_speaker_profile_midoverlap_diagnostic_coordination_card.md",
        ...
        },
        {
            ...
            "headline": "Wave39 LightOverlap diagnostic refresh precedes MidOverlap refresh",
        ...
        {
            "section_id": "wave40_boundary",
            "headline": "Wave40 closure defers overlap embedding execution to diagnostic boundaries only",
            "artifact_anchor": "results/figures/wave40_exploration_baseline_closure_card.md",
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave40_speaker_profile_midoverlap_diagnostic_coordination_card",
        "coordination_section_count": str(len(rows)),
        "diagnostic_case_scope": "MidOverlap",
        "candidate_case_scope": "HeavyOverlap",
        "execution_receipt_status": "wave40_speaker_profile_midoverlap_diagnostic_coordination_complete",
        "blocker": "attribution_claims_still_blocked",
        "fill_note": (
            "Documented MidOverlap diagnostic refresh after Wave40 closure; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave40_speaker_profile_midoverlap_diagnostic_coordination_complete",
        "coordination_scope": "wave40_speaker_profile_midoverlap_diagnostic",
        "wave40_closure_status": str(wave40_receipt.get("execution_status", "")),
        "midoverlap_prior_coordination_status": str(midoverlap_receipt.get("execution_status", "")),
        "wave36_refresh_status": str(wave36_refresh_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave40 closure, demo wave40, Wave9 MidOverlap, Wave36 refresh, and Wave39 LightOverlap refresh."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave40 Speaker Profile MidOverlap Diagnostic Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave40 Speaker Profile MidOverlap Diagnostic Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave40 Speaker Profile MidOverlap Diagnostic Coordination Receipt",
        "",
        "| execution_status | wave36_refresh_status | lightoverlap_refresh_status |",
        "| --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['wave36_refresh_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave40_speaker_profile_midoverlap_diagnostic_coordination_card.csv"
    card_json = tables_dir / "wave40_speaker_profile_midoverlap_diagnostic_coordination_card.json"
    card_md = figures_dir / "wave40_speaker_profile_midoverlap_diagnostic_coordination_card.md"
    fill_csv = tables_dir / "wave40_speaker_profile_midoverlap_diagnostic_coordination_writeback.csv"
    fill_json = tables_dir / "wave40_speaker_profile_midoverlap_diagnostic_coordination_writeback.json"
    fill_md = figures_dir / "wave40_speaker_profile_midoverlap_diagnostic_coordination_writeback.md"
    receipt_json = tables_dir / "wave40_speaker_profile_midoverlap_diagnostic_coordination_receipt.json"
    receipt_md = figures_dir / "wave40_speaker_profile_midoverlap_diagnostic_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    wave40_receipt = load_json_dict("results/tables/wave40_exploration_baseline_closure_receipt.json")
    demo_wave40_fill = load_json_dict("results/tables/demo_wave40_presentation_writeback.json")
    midoverlap_receipt = load_json_dict(
        "results/tables/speaker_profile_midoverlap_diagnostic_coordination_receipt.json"
    )
    wave36_refresh_receipt = load_json_dict(
    ...
    assert_writeback_preconditions(
        ...
        demo_wave40_fill,
        midoverlap_receipt,
        wave36_refresh_receipt,
        lightoverlap_refresh_receipt,
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave40_speaker_profile_midoverlap_diagnostic_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict(
            "results/tables/wave40_speaker_profile_midoverlap_diagnostic_coordination_receipt.json"
        )
        if str(existing.get("execution_status", "")) == "wave40_speaker_profile_midoverlap_diagnostic_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave40_receipt, midoverlap_receipt, wave36_refresh_receipt, lightoverlap_refresh_receipt
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave40 speaker profile MidOverlap diagnostic refresh after Wave40 closure."
    )
