...
def assert_writeback_preconditions(
    ...
    if str(wave95_receipt.get("execution_status", "")) != "wave95_exploration_baseline_closure_complete":
        raise RuntimeError("Wave95 closure must be complete before external validation narrow slice coordination")
    if (
        str(oppositeoverlap_receipt.get("execution_status", ""))
        != "wave94_speaker_profile_oppositeoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave94 OppositeOverlap diagnostic coordination must be complete before external validation coordination"
        )
    if str(demo_wave95_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave95 presentation writeback must be filled before external validation coordination")
    if str(demo_wave95_fill.get("storyboard_receipt_status", "")) != "wave95_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave95 storyboard receipt must be wave95_presentation_extension_complete before external coordination"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Wave89 external validation narrow slice coordination must be complete before Wave95 refresh"
        )
...
def build_fill_row(rows: list[dict[str, str]], slice_id: str) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave95_external_validation_narrow_slice_coordination_card",
        "coordination_section_count": str(len(rows)),
        "narrow_slice_id": slice_id,
        "execution_receipt_status": "wave95_external_validation_narrow_slice_coordination_complete",
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave95_external_validation_narrow_slice_coordination_complete",
        "coordination_scope": "wave89_external_validation_narrow_slice",
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave95_external_validation_narrow_slice_coordination_card.csv"
    card_json = tables_dir / "wave95_external_validation_narrow_slice_coordination_card.json"
    card_md = figures_dir / "wave95_external_validation_narrow_slice_coordination_card.md"
    fill_csv = tables_dir / "wave95_external_validation_narrow_slice_coordination_writeback.csv"
    fill_json = tables_dir / "wave95_external_validation_narrow_slice_coordination_writeback.json"
    fill_md = figures_dir / "wave95_external_validation_narrow_slice_coordination_writeback.md"
    receipt_json = tables_dir / "wave95_external_validation_narrow_slice_coordination_receipt.json"
    receipt_md = figures_dir / "wave95_external_validation_narrow_slice_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    receipt_path = PROJECT_ROOT / "results/tables/wave95_external_validation_narrow_slice_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave95_external_validation_narrow_slice_coordination_receipt.json")
        if str(existing.get("execution_status", "")) == "wave95_external_validation_narrow_slice_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave95_receipt, oppositeoverlap_receipt, prior_coord_receipt, narrow_eval_receipt, external_summary
    )
