...
RECEIPT_COLUMNS = [
    "execution_status",
    "coordination_scope",
    "wave101_closure_status",
    "oppositeoverlap_coordination_status",
    "wave89_refresh_status",
...
def assert_writeback_preconditions(
    wave101_receipt: dict[str, Any],
    oppositeoverlap_receipt: dict[str, Any],
    demo_wave101_fill: dict[str, Any],
    narrow_eval_receipt: dict[str, Any],
    external_summary: dict[str, Any],
    prior_coord_receipt: dict[str, Any],
) -> None:
    if str(wave101_receipt.get("execution_status", "")) != "wave101_exploration_baseline_closure_complete":
        raise RuntimeError("Wave101 closure must be complete before external validation narrow slice coordination")
    if (
        str(oppositeoverlap_receipt.get("execution_status", ""))
        != "wave100_speaker_profile_oppositeoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave100 OppositeOverlap diagnostic coordination must be complete before external validation coordination"
        )
    if str(demo_wave101_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave101 presentation writeback must be filled before external validation coordination")
    if str(demo_wave101_fill.get("storyboard_receipt_status", "")) != "wave101_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave95 storyboard receipt must be wave101_presentation_extension_complete before external coordination"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Wave95 external validation narrow slice coordination must be complete before Wave101 refresh"
        )
...
def build_coordination_rows() -> list[dict[str, str]]:
    return [
        ...
        {
            "section_id": "wave89_prior_coordination",
            "headline": "Wave89 external validation narrow slice coordination precedes Wave89 refresh",
            "artifact_anchor": "results/figures/wave95_external_validation_narrow_slice_coordination_card.md",
            "coordination_note": "Wave89 refresh does not replace Wave71 external/sanity-check boundary.",
            "result_label": "external/sanity-check",
        },
        {
            "section_id": "oppositeoverlap_prior",
            "headline": "Wave94 OppositeOverlap diagnostic coordination precedes external validation refresh",
        ...
        },
        {
            ...
            "headline": "Wave95 closure keeps external validation separate from gold baseline CER",
            "artifact_anchor": "results/figures/wave101_exploration_baseline_closure_card.md",
...
def build_fill_row(rows: list[dict[str, str]], slice_id: str) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave101_external_validation_narrow_slice_coordination_card",
        "coordination_section_count": str(len(rows)),
        "narrow_slice_id": slice_id,
        "execution_receipt_status": "wave101_external_validation_narrow_slice_coordination_complete",
        "blocker": "gold_benchmark_claims_still_blocked",
        "fill_note": (
            "Documented external validation narrow slice refresh boundary after Wave94 OppositeOverlap chain; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave101_external_validation_narrow_slice_coordination_complete",
        "coordination_scope": "wave101_external_validation_narrow_slice",
        "wave101_closure_status": str(wave101_receipt.get("execution_status", "")),
        "oppositeoverlap_coordination_status": str(oppositeoverlap_receipt.get("execution_status", "")),
        "wave89_refresh_status": str(prior_coord_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave95 closure, OppositeOverlap diagnostic, demo wave83, narrow ASR eval receipt, external board."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave95 External Validation Narrow Slice Coordination Card (external/sanity-check)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave95 External Validation Narrow Slice Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave95 External Validation Narrow Slice Coordination Receipt",
        "",
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave101_external_validation_narrow_slice_coordination_card.csv"
    card_json = tables_dir / "wave101_external_validation_narrow_slice_coordination_card.json"
    card_md = figures_dir / "wave101_external_validation_narrow_slice_coordination_card.md"
    fill_csv = tables_dir / "wave101_external_validation_narrow_slice_coordination_writeback.csv"
    fill_json = tables_dir / "wave101_external_validation_narrow_slice_coordination_writeback.json"
    fill_md = figures_dir / "wave101_external_validation_narrow_slice_coordination_writeback.md"
    receipt_json = tables_dir / "wave101_external_validation_narrow_slice_coordination_receipt.json"
    receipt_md = figures_dir / "wave101_external_validation_narrow_slice_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    wave101_receipt = load_json_dict("results/tables/wave101_exploration_baseline_closure_receipt.json")
    ...
    demo_wave101_fill = load_json_dict("results/tables/demo_wave101_presentation_writeback.json")
    ...
    receipt_path = PROJECT_ROOT / "results/tables/wave101_external_validation_narrow_slice_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave101_external_validation_narrow_slice_coordination_receipt.json")
        if str(existing.get("execution_status", "")) == "wave101_external_validation_narrow_slice_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave101_receipt, oppositeoverlap_receipt, prior_coord_receipt, narrow_eval_receipt, external_summary
    )
