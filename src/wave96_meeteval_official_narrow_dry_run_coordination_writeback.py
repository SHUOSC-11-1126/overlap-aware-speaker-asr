...
def assert_writeback_preconditions(
    wave96_receipt: dict[str, Any],
    demo_wave96_fill: dict[str, Any],
    prior_official_receipt: dict[str, Any],
    character_refresh_receipt: dict[str, Any],
) -> None:
    if str(wave96_receipt.get("execution_status", "")) != "wave96_exploration_baseline_closure_complete":
        raise RuntimeError("Wave96 closure must be complete before MeetEval official narrow dry-run refresh coordination")
    if str(demo_wave96_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave96 presentation writeback must be filled before MeetEval official coordination")
    if str(demo_wave96_fill.get("storyboard_receipt_status", "")) != "wave96_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave96 storyboard receipt must be wave96_presentation_extension_complete before MeetEval official coordination"
        )
    if (
        ...
        raise RuntimeError(
            "Wave90 MeetEval official narrow dry-run coordination must be complete before Wave96 refresh"
        )
...
def build_coordination_rows() -> list[dict[str, str]]:
    return [
        ...
        {
            "section_id": "wave84_prior_coordination",
            "headline": "Wave90 official MeetEval narrow dry-run refresh remains the prerequisite chain",
            "artifact_anchor": "results/figures/wave90_meeteval_official_narrow_dry_run_coordination_card.md",
            "coordination_note": "Wave90 refresh does not replace Wave90 official MeetEval boundary documentation.",
            "result_label": "experimental/frontier",
        },
        {
        ...
        },
        {
            ...
            "headline": "Wave96 closure keeps official MeetEval coordination separate from gold baseline CER",
            "artifact_anchor": "results/figures/wave96_exploration_baseline_closure_card.md",
...
def build_fill_row(rows: list[dict[str, str]], official_dry_run_case_count: str) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave96_meeteval_official_narrow_dry_run_coordination_card",
        "coordination_section_count": str(len(rows)),
        "official_dry_run_case_count": official_dry_run_case_count,
        "execution_receipt_status": "wave96_meeteval_official_narrow_dry_run_coordination_complete",
        "blocker": "official_benchmark_claims_still_blocked",
        "fill_note": (
            "Documented MeetEval official narrow dry-run refresh boundary after Wave96 closure; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave96_meeteval_official_narrow_dry_run_coordination_complete",
        "coordination_scope": "wave96_meeteval_official_narrow_dry_run",
        "wave96_closure_status": str(wave96_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave96 closure, demo wave96, Wave90 official coordination, Wave17 character-level refresh, official execution."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave96 MeetEval Official Narrow Dry-Run Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave96 MeetEval Official Narrow Dry-Run Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave96 MeetEval Official Narrow Dry-Run Coordination Receipt",
        "",
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave96_meeteval_official_narrow_dry_run_coordination_card.csv"
    card_json = tables_dir / "wave96_meeteval_official_narrow_dry_run_coordination_card.json"
    card_md = figures_dir / "wave96_meeteval_official_narrow_dry_run_coordination_card.md"
    fill_csv = tables_dir / "wave96_meeteval_official_narrow_dry_run_coordination_writeback.csv"
    fill_json = tables_dir / "wave96_meeteval_official_narrow_dry_run_coordination_writeback.json"
    fill_md = figures_dir / "wave96_meeteval_official_narrow_dry_run_coordination_writeback.md"
    receipt_json = tables_dir / "wave96_meeteval_official_narrow_dry_run_coordination_receipt.json"
    receipt_md = figures_dir / "wave96_meeteval_official_narrow_dry_run_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    wave96_receipt = load_json_dict("results/tables/wave96_exploration_baseline_closure_receipt.json")
    demo_wave96_fill = load_json_dict("results/tables/demo_wave96_presentation_writeback.json")
    ...
    assert_writeback_preconditions(
        wave96_receipt, demo_wave96_fill, prior_official_receipt, character_refresh_receipt
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave96_meeteval_official_narrow_dry_run_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave96_meeteval_official_narrow_dry_run_coordination_receipt.json")
        if str(existing.get("execution_status", "")) == "wave96_meeteval_official_narrow_dry_run_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave96_receipt, prior_official_receipt, character_refresh_receipt, official_count
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave96 MeetEval official narrow dry-run refresh after Wave96 closure."
    )
