...
from .demo_wave104_presentation_writeback import build_extended_polish_rows as build_wave104_polish_rows
...
def assert_extension_preconditions(
    ...
    wave104_fill: dict[str, Any],
) -> None:
    if str(wave105_receipt.get("execution_status", "")) != "wave105_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave105 exploration baseline closure must be complete before wave105 demo extension"
        )
    if str(wave104_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave104 presentation writeback must be filled before wave105 extension")
    if str(wave104_fill.get("storyboard_receipt_status", "")) != "wave104_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave104 storyboard receipt must be wave104_presentation_extension_complete before wave105 extension"
        )


def build_wave105_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave105",
        "headline": "Wave105 exploration+baseline closure after Wave104 MidOverlap chain boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave104_polish_rows()
    if any(row["section_id"] == "frontier_wave105" for row in rows):
        return rows
    return rows + [build_wave105_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave105_presentation_extension_complete",
        "storyboard_scope": "wave105_storyboard_extension",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave105_presentation_extension_complete",
        "walkthrough_scope": "wave105_walkthrough_extension",
        "expected_inputs": "Wave104 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave105_presentation_polish_extension",
        "storyboard_receipt_status": "wave105_presentation_extension_complete",
        "walkthrough_receipt_status": "wave105_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave105_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave105_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave105_presentation_writeback.md"
...
def run_wave105_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave105_receipt = load_json_dict("results/tables/wave105_exploration_baseline_closure_receipt.json")
    wave104_fill = load_json_dict("results/tables/demo_wave104_presentation_writeback.json")
    assert_extension_preconditions(wave105_receipt, wave104_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave105_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave105_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave105 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave105 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave105_presentation_writeback(force=args.force)
