...
def assert_writeback_preconditions(
    ...
    wave80_refresh_receipt: dict[str, Any],
    ...
    if str(wave86_receipt.get("execution_status", "")) != "wave86_exploration_baseline_closure_complete":
        raise RuntimeError("Wave86 closure must be complete before MidOverlap diagnostic refresh coordination")
    if str(demo_wave86_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave86 presentation writeback must be filled before MidOverlap coordination")
    if str(demo_wave86_fill.get("storyboard_receipt_status", "")) != "wave86_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave86 storyboard receipt must be wave86_presentation_extension_complete before MidOverlap coordination"
        )
    if str(midoverlap_receipt.get("execution_status", "")) != "speaker_profile_midoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Speaker profile MidOverlap diagnostic coordination must be complete before Wave86 refresh"
        )
    if (
        str(wave80_refresh_receipt.get("execution_status", ""))
        != "wave80_speaker_profile_midoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave80 MidOverlap diagnostic refresh must be complete before Wave86 refresh coordination"
        )
    if (
        ...
        raise RuntimeError(
            "Wave85 LightOverlap diagnostic refresh must be complete before MidOverlap refresh coordination"
        )
    ...
    return [
        {
            ...
            "headline": "MidOverlap separation-harm diagnostic scope refresh after Wave86 closure",
        ...
        {
            "section_id": "wave80_refresh",
            "headline": "Wave80 MidOverlap diagnostic refresh precedes Wave86 refresh",
        ...
        },
        {
            ...
            "headline": "Wave85 LightOverlap diagnostic refresh precedes MidOverlap refresh",
        ...
        },
        {
            ...
            "headline": "Wave86 closure defers overlap embedding execution to diagnostic boundaries only",
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave86_speaker_profile_midoverlap_diagnostic_coordination_card",
        "coordination_section_count": str(len(rows)),
        "diagnostic_case_scope": "MidOverlap",
        "candidate_case_scope": "HeavyOverlap",
        "execution_receipt_status": "wave86_speaker_profile_midoverlap_diagnostic_coordination_complete",
        "blocker": "attribution_claims_still_blocked",
        "fill_note": (
            "Documented MidOverlap diagnostic refresh after Wave86 closure; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave86_speaker_profile_midoverlap_diagnostic_coordination_complete",
        ...
        "wave80_refresh_status": str(wave80_refresh_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave86 closure, demo wave80, Wave9 MidOverlap, Wave74 refresh, and Wave85 LightOverlap refresh."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave86 Speaker Profile MidOverlap Diagnostic Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave86 Speaker Profile MidOverlap Diagnostic Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave86 Speaker Profile MidOverlap Diagnostic Coordination Receipt",
        "",
        "| execution_status | wave80_refresh_status | lightoverlap_refresh_status |",
        "| --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['wave80_refresh_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave86_speaker_profile_midoverlap_diagnostic_coordination_card.csv"
    card_json = tables_dir / "wave86_speaker_profile_midoverlap_diagnostic_coordination_card.json"
    card_md = figures_dir / "wave86_speaker_profile_midoverlap_diagnostic_coordination_card.md"
    fill_csv = tables_dir / "wave86_speaker_profile_midoverlap_diagnostic_coordination_writeback.csv"
    fill_json = tables_dir / "wave86_speaker_profile_midoverlap_diagnostic_coordination_writeback.json"
    fill_md = figures_dir / "wave86_speaker_profile_midoverlap_diagnostic_coordination_writeback.md"
    receipt_json = tables_dir / "wave86_speaker_profile_midoverlap_diagnostic_coordination_receipt.json"
    receipt_md = figures_dir / "wave86_speaker_profile_midoverlap_diagnostic_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    wave80_refresh_receipt = load_json_dict(
    ...
    assert_writeback_preconditions(
        ...
        demo_wave86_fill,
        midoverlap_receipt,
        wave80_refresh_receipt,
        lightoverlap_refresh_receipt,
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave86_speaker_profile_midoverlap_diagnostic_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict(
            "results/tables/wave86_speaker_profile_midoverlap_diagnostic_coordination_receipt.json"
        )
        if str(existing.get("execution_status", "")) == "wave86_speaker_profile_midoverlap_diagnostic_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave86_receipt, midoverlap_receipt, wave80_refresh_receipt, lightoverlap_refresh_receipt
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave80 speaker profile MidOverlap diagnostic refresh after Wave86 closure."
    )
