...
RECEIPT_COLUMNS = [
    "execution_status",
    "coordination_scope",
    "wave147_closure_status",
    "heavyoverlap_prior_coordination_status",
    "wave141_refresh_status",
...
def assert_writeback_preconditions(
    wave147_receipt: dict[str, Any],
    demo_wave147_fill: dict[str, Any],
    heavyoverlap_receipt: dict[str, Any],
    wave141_refresh_receipt: dict[str, Any],
    midoverlap_refresh_receipt: dict[str, Any],
) -> None:
    if str(wave147_receipt.get("execution_status", "")) != "wave147_exploration_baseline_closure_complete":
        raise RuntimeError("Wave147 closure must be complete before HeavyOverlap diagnostic refresh coordination")
    if str(demo_wave147_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave147 presentation writeback must be filled before HeavyOverlap coordination")
    if str(demo_wave147_fill.get("storyboard_receipt_status", "")) != "wave147_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave147 storyboard receipt must be wave147_presentation_extension_complete before HeavyOverlap coordination"
        )
    if str(heavyoverlap_receipt.get("execution_status", "")) != "speaker_profile_heavyoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Speaker profile HeavyOverlap diagnostic coordination must be complete before Wave147 refresh"
        )
    if (
        str(wave141_refresh_receipt.get("execution_status", ""))
        != "wave141_speaker_profile_heavyoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave141 HeavyOverlap diagnostic refresh must be complete before Wave147 refresh coordination"
        )
    if (
        str(midoverlap_refresh_receipt.get("execution_status", ""))
        != "wave146_speaker_profile_midoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave146 MidOverlap diagnostic refresh must be complete before HeavyOverlap refresh coordination"
        )
    ...
    return [
        {
            ...
            "headline": "HeavyOverlap separation-benefit diagnostic scope refresh after Wave147 closure",
        ...
        {
            "section_id": "wave141_refresh",
            "headline": "Wave141 HeavyOverlap diagnostic refresh precedes Wave147 refresh",
        ...
        },
        {
            ...
            "headline": "Wave146 MidOverlap diagnostic refresh precedes HeavyOverlap refresh",
            "artifact_anchor": "results/figures/wave146_speaker_profile_midoverlap_diagnostic_coordination_card.md",
            "coordination_note": "Overlap diagnostic chain progresses MidOverlap → HeavyOverlap through coordinated refreshes.",
            "result_label": "experimental/frontier",
        },
        {
            "section_id": "wave147_boundary",
            "headline": "Wave147 closure defers overlap embedding execution to diagnostic boundaries only",
            "artifact_anchor": "results/figures/wave147_exploration_baseline_closure_card.md",
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave147_speaker_profile_heavyoverlap_diagnostic_coordination_card",
        "coordination_section_count": str(len(rows)),
        "diagnostic_case_scope": "HeavyOverlap",
        "candidate_case_scope": "OppositeOverlap",
        "execution_receipt_status": "wave147_speaker_profile_heavyoverlap_diagnostic_coordination_complete",
        "blocker": "attribution_claims_still_blocked",
        "fill_note": (
            "Documented HeavyOverlap diagnostic refresh after Wave147 closure; "
...
def build_receipt_row(
    ...
    return {
        ...
        "coordination_scope": "wave147_speaker_profile_heavyoverlap_diagnostic",
        "wave147_closure_status": str(wave147_receipt.get("execution_status", "")),
        "heavyoverlap_prior_coordination_status": str(heavyoverlap_receipt.get("execution_status", "")),
        "wave141_refresh_status": str(wave141_refresh_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave147 closure, demo wave147, Wave12 HeavyOverlap, Wave141 refresh, and Wave146 MidOverlap refresh."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave147 Speaker Profile HeavyOverlap Diagnostic Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave147 Speaker Profile HeavyOverlap Diagnostic Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave147 Speaker Profile HeavyOverlap Diagnostic Coordination Receipt",
        "",
        "| execution_status | wave141_refresh_status | midoverlap_refresh_status |",
        "| --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['wave141_refresh_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave147_speaker_profile_heavyoverlap_diagnostic_coordination_card.csv"
    card_json = tables_dir / "wave147_speaker_profile_heavyoverlap_diagnostic_coordination_card.json"
    card_md = figures_dir / "wave147_speaker_profile_heavyoverlap_diagnostic_coordination_card.md"
    fill_csv = tables_dir / "wave147_speaker_profile_heavyoverlap_diagnostic_coordination_writeback.csv"
    fill_json = tables_dir / "wave147_speaker_profile_heavyoverlap_diagnostic_coordination_writeback.json"
    fill_md = figures_dir / "wave147_speaker_profile_heavyoverlap_diagnostic_coordination_writeback.md"
    receipt_json = tables_dir / "wave147_speaker_profile_heavyoverlap_diagnostic_coordination_receipt.json"
    receipt_md = figures_dir / "wave147_speaker_profile_heavyoverlap_diagnostic_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    wave147_receipt = load_json_dict("results/tables/wave147_exploration_baseline_closure_receipt.json")
    demo_wave147_fill = load_json_dict("results/tables/demo_wave147_presentation_writeback.json")
    heavyoverlap_receipt = load_json_dict(
        "results/tables/speaker_profile_heavyoverlap_diagnostic_coordination_receipt.json"
    )
    wave141_refresh_receipt = load_json_dict(
    ...
    midoverlap_refresh_receipt = load_json_dict(
        "results/tables/wave146_speaker_profile_midoverlap_diagnostic_coordination_receipt.json"
    )
    assert_writeback_preconditions(
        wave147_receipt,
        demo_wave147_fill,
        heavyoverlap_receipt,
        wave141_refresh_receipt,
        midoverlap_refresh_receipt,
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave147_speaker_profile_heavyoverlap_diagnostic_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict(
            "results/tables/wave147_speaker_profile_heavyoverlap_diagnostic_coordination_receipt.json"
        )
        if str(existing.get("execution_status", "")) == "wave147_speaker_profile_heavyoverlap_diagnostic_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave147_receipt, heavyoverlap_receipt, wave141_refresh_receipt, midoverlap_refresh_receipt
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave147 speaker profile HeavyOverlap diagnostic refresh after Wave147 closure."
    )
