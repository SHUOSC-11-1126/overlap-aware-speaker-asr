...
from .demo_wave87_presentation_writeback import build_extended_polish_rows as build_wave87_polish_rows
...
def assert_extension_preconditions(
    wave88_receipt: dict[str, Any],
    wave87_fill: dict[str, Any],
) -> None:
    if str(wave88_receipt.get("execution_status", "")) != "wave88_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave88 exploration baseline closure must be complete before wave88 demo extension"
        )
    if str(wave87_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave87 presentation writeback must be filled before wave88 extension")
    if str(wave87_fill.get("storyboard_receipt_status", "")) != "wave87_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave87 storyboard receipt must be wave87_presentation_extension_complete before wave88 extension"
        )


def build_wave88_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave88",
        "headline": "Wave88 exploration+baseline closure and OppositeOverlap speaker-profile diagnostic refresh speaker-profile diagnostic refresh boundary",
        "artifact_anchor": "results/figures/wave88_exploration_baseline_closure_card.md",
        "presentation_note": (
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave87_polish_rows()
    if any(row["section_id"] == "frontier_wave88" for row in rows):
        return rows
    return rows + [build_wave88_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave88_presentation_extension_complete",
        "storyboard_scope": "wave88_storyboard_extension",
        "expected_inputs": "Wave88 exploration+baseline closure card.",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave88_presentation_extension_complete",
        "walkthrough_scope": "wave88_walkthrough_extension",
        "expected_inputs": "Wave88 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave88_presentation_polish_extension",
        "storyboard_receipt_status": "wave88_presentation_extension_complete",
        "walkthrough_receipt_status": "wave88_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave88_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave88_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave88_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave88 Presentation Writeback",
                "",
...
def run_wave88_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave88_receipt = load_json_dict("results/tables/wave88_exploration_baseline_closure_receipt.json")
    wave87_fill = load_json_dict("results/tables/demo_wave87_presentation_writeback.json")
    assert_extension_preconditions(wave88_receipt, wave87_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave88_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave88_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave88 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave88 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave88_presentation_writeback(force=args.force)
