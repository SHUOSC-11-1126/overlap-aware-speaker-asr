...
def assert_writeback_preconditions(
    ...
    if str(wave150_receipt.get("execution_status", "")) != "wave150_exploration_baseline_closure_complete":
        raise RuntimeError("Wave150 closure must be complete before MeetEval official narrow dry-run refresh coordination")
    if str(demo_wave150_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave150 presentation writeback must be filled before MeetEval official coordination")
    if str(demo_wave150_fill.get("storyboard_receipt_status", "")) != "wave150_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave150 storyboard receipt must be wave150_presentation_extension_complete before MeetEval official coordination"
        )
    if (
        str(prior_official_receipt.get("execution_status", ""))
        != "wave144_meeteval_official_narrow_dry_run_coordination_complete"
    ):
        raise RuntimeError(
            "Wave144 MeetEval official narrow dry-run coordination must be complete before Wave150 refresh"
        )
...
def build_coordination_rows() -> list[dict[str, str]]:
    return [
        ...
        {
            "section_id": "wave144_prior_coordination",
            "headline": "Wave144 official MeetEval narrow dry-run refresh remains the prerequisite chain",
            "artifact_anchor": "results/figures/wave144_meeteval_official_narrow_dry_run_coordination_card.md",
            "coordination_note": "Wave150 refresh does not replace Wave144 official MeetEval boundary documentation.",
            "result_label": "experimental/frontier",
        },
        {
        ...
        {
            "section_id": "wave150_boundary",
            "headline": "Wave150 closure keeps official MeetEval coordination separate from gold baseline CER",
...
def build_fill_row(rows: list[dict[str, str]], official_dry_run_case_count: str) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave150_meeteval_official_narrow_dry_run_coordination_card",
        "coordination_section_count": str(len(rows)),
        "official_dry_run_case_count": official_dry_run_case_count,
        "execution_receipt_status": "wave150_meeteval_official_narrow_dry_run_coordination_complete",
        "blocker": "official_benchmark_claims_still_blocked",
        "fill_note": (
            "Documented MeetEval official narrow dry-run refresh boundary after Wave150 closure; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave150_meeteval_official_narrow_dry_run_coordination_complete",
        "coordination_scope": "wave150_meeteval_official_narrow_dry_run",
        ...
        "expected_inputs": (
            "Wave150 closure, demo wave150, Wave144 official coordination, Wave17 character-level refresh, official execution."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave150 MeetEval Official Narrow Dry-Run Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave150 MeetEval Official Narrow Dry-Run Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave150 MeetEval Official Narrow Dry-Run Coordination Receipt",
        "",
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave150_meeteval_official_narrow_dry_run_coordination_card.csv"
    card_json = tables_dir / "wave150_meeteval_official_narrow_dry_run_coordination_card.json"
    card_md = figures_dir / "wave150_meeteval_official_narrow_dry_run_coordination_card.md"
    fill_csv = tables_dir / "wave150_meeteval_official_narrow_dry_run_coordination_writeback.csv"
    fill_json = tables_dir / "wave150_meeteval_official_narrow_dry_run_coordination_writeback.json"
    fill_md = figures_dir / "wave150_meeteval_official_narrow_dry_run_coordination_writeback.md"
    receipt_json = tables_dir / "wave150_meeteval_official_narrow_dry_run_coordination_receipt.json"
    receipt_md = figures_dir / "wave150_meeteval_official_narrow_dry_run_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    prior_official_receipt = load_json_dict(
        "results/tables/wave144_meeteval_official_narrow_dry_run_coordination_receipt.json"
    )
    ...
    assert_writeback_preconditions(
        wave150_receipt, demo_wave150_fill, prior_official_receipt, character_refresh_receipt
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave150_meeteval_official_narrow_dry_run_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave150_meeteval_official_narrow_dry_run_coordination_receipt.json")
        if str(existing.get("execution_status", "")) == "wave150_meeteval_official_narrow_dry_run_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave150_receipt, prior_official_receipt, character_refresh_receipt, official_count
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave150 MeetEval official narrow dry-run refresh after Wave150 closure."
    )
