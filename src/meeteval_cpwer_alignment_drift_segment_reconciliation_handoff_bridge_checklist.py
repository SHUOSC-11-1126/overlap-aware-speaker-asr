...
def load_reconciliation_handoff_row() -> dict[str, str]:
    handoff_path = (
...
def build_bridge_checklist_rows(handoff_row: dict[str, str]) -> list[dict[str, str]]:
    ...
    handoff_status = str(handoff_row.get("handoff_status", "reconciliation_handoff_ready"))
    return [
        {
            "checklist_order": "1",
            "case_id": case_id,
            ...
            "checklist_goal": (
                f"Verify the reconciliation handoff bridge for {case_id} before reopening the reconciliation scaffold bridge."
            ),
            "bridge_note": (
                f"Reconciliation handoff remains {handoff_status}; confirm diagnostic context before advancing the scaffold bridge."
            ),
...
def build_bridge_checklist_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        ...
        "",
        "This generated checklist turns the reconciliation handoff into a row-by-row bridge verification path. "
...
def write_outputs(rows: list[dict[str, str]]) -> tuple[Path, Path, Path]:
    ...
    csv_path = tables_dir / "meeteval_cpwer_alignment_drift_segment_reconciliation_handoff_bridge_checklist.csv"
    json_path = tables_dir / "meeteval_cpwer_alignment_drift_segment_reconciliation_handoff_bridge_checklist.json"
    md_path = figures_dir / "meeteval_cpwer_alignment_drift_segment_reconciliation_handoff_bridge_checklist.md"
...
def main() -> None:
    ...
    print(
        "Wrote MeetEval cpWER alignment drift segment reconciliation handoff bridge checklist CSV: "
        f"{csv_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote MeetEval cpWER alignment drift segment reconciliation handoff bridge checklist JSON: "
        f"{json_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote MeetEval cpWER alignment drift segment reconciliation handoff bridge checklist note: "
