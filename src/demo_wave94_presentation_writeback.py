...
from .demo_wave93_presentation_writeback import build_extended_polish_rows as build_wave93_polish_rows
...
def assert_extension_preconditions(
    ...
    wave93_fill: dict[str, Any],
) -> None:
    if str(wave94_receipt.get("execution_status", "")) != "wave94_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave94 exploration baseline closure must be complete before wave94 demo extension"
        )
    if str(wave93_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave93 presentation writeback must be filled before wave94 extension")
    if str(wave93_fill.get("storyboard_receipt_status", "")) != "wave93_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave93 storyboard receipt must be wave93_presentation_extension_complete before wave94 extension"
        )


def build_wave94_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave94",
        "headline": "Wave94 exploration+baseline closure and OppositeOverlap speaker-profile diagnostic refresh speaker-profile diagnostic refresh boundary",
        "artifact_anchor": "results/figures/wave94_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave81 closure card only; OppositeOverlap speaker-profile diagnostic refresh remains "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave93_polish_rows()
    if any(row["section_id"] == "frontier_wave94" for row in rows):
        return rows
    return rows + [build_wave94_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave94_presentation_extension_complete",
        "storyboard_scope": "wave94_storyboard_extension",
        "expected_inputs": "Wave94 exploration+baseline closure card.",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave94_presentation_extension_complete",
        "walkthrough_scope": "wave94_walkthrough_extension",
        "expected_inputs": "Wave94 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave94_presentation_polish_extension",
        "storyboard_receipt_status": "wave94_presentation_extension_complete",
        "walkthrough_receipt_status": "wave94_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave94_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave94_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave94_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave94 Presentation Writeback",
                "",
...
def run_wave94_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave94_receipt = load_json_dict("results/tables/wave94_exploration_baseline_closure_receipt.json")
    wave93_fill = load_json_dict("results/tables/demo_wave93_presentation_writeback.json")
    assert_extension_preconditions(wave94_receipt, wave93_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave94_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave94_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave94 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave94 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave94_presentation_writeback(force=args.force)
