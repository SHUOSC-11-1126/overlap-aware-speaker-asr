...
RECEIPT_COLUMNS = [
    ...
    "phase5_gate_coordination_status",
...
def assert_closure_preconditions(
    ...
    if str(wave14_receipt.get("execution_status", "")) != "wave14_exploration_baseline_closure_complete":
        raise RuntimeError("Wave14 closure receipt must be complete before Wave15 exploration baseline closure")
    if str(phase5_receipt.get("execution_status", "")) != "cascade_benchmark_phase5_gate_coordination_complete":
        raise RuntimeError("Phase5 gate coordination must be complete before Wave15 closure")
    if str(meeteval_receipt.get("execution_status", "")) != "meeteval_official_narrow_dry_run_coordination_complete":
        raise RuntimeError(
            "MeetEval official narrow dry-run coordination must be complete before Wave15 closure"
        )
    if str(demo_wave14_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave14 presentation writeback must be filled before Wave15 closure")
    if str(demo_wave14_fill.get("storyboard_receipt_status", "")) != "wave14_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave14 storyboard receipt must be wave14_presentation_extension_complete before Wave15 closure"
        )
...
def build_closure_rows() -> list[dict[str, str]]:
    return [
        {
            "section_id": "wave14_rollup",
            "headline": "Wave14 closed: phase5 gate + MeetEval official dry-run + demo + exploration closure",
        ...
        {
            "section_id": "phase5_gate",
            "headline": "phase5_cross_dataset_refresh gate coordinated after phase4 chain",
            "artifact_anchor": "results/figures/cascade_benchmark_phase5_gate_coordination_card.md",
            "coordination_note": "Cross-dataset refresh not executed; timing-backed sessions still blocked.",
            "result_label": "experimental/frontier",
        },
        {
            "section_id": "meeteval_official",
            "headline": "MeetEval official cpWER narrow dry-run coordination complete on 5/5 gold cases",
            "artifact_anchor": "results/figures/meeteval_official_narrow_dry_run_coordination_card.md",
            "coordination_note": "External benchmark claims still blocked pending controlled timing.",
            "result_label": "experimental/frontier",
        },
        {
        ...
        },
        {
            ...
            "headline": "External validation narrow slice is the next open experimental coordination gate",
            "artifact_anchor": "results/tables/external_validation_status.json",
            "coordination_note": "Wave15 closure does not record external dataset execution sessions.",
            "result_label": "experimental/frontier",
        },
    ]
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave15_exploration_baseline_closure_card",
        "coordination_section_count": str(len(rows)),
        "execution_receipt_status": "wave15_exploration_baseline_closure_complete",
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": (
            "Wave15 exploration+baseline closure rollup after Wave14 chain; stable gold baseline preserved."
        ),
    }
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave15_exploration_baseline_closure_complete",
        "coordination_scope": "wave15_exploration_baseline",
        "wave14_closure_status": str(wave14_receipt.get("execution_status", "")),
        "phase5_gate_coordination_status": str(phase5_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave14 closure, phase5 gate, MeetEval official dry-run, demo wave14, frontier summary."
        ),
        "writeback_note": (
            "探索+基线 Wave15 closure writeback; qualitative/demo and experimental/frontier labels preserved."
        ),
    }
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave15 Exploration+Baseline Closure Card",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave15 Exploration+Baseline Closure Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave15 Exploration+Baseline Closure Receipt",
        "",
        "| execution_status | wave14_closure_status | frontier_go_count |",
        "| --- | --- | ---: |",
        (
            f"| {row['execution_status']} | {row['wave14_closure_status']} | {row['frontier_go_count']} |"
        ),
    ]
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave15_exploration_baseline_closure_card.csv"
    card_json = tables_dir / "wave15_exploration_baseline_closure_card.json"
    card_md = figures_dir / "wave15_exploration_baseline_closure_card.md"
    fill_csv = tables_dir / "wave15_exploration_baseline_closure_writeback.csv"
    fill_json = tables_dir / "wave15_exploration_baseline_closure_writeback.json"
    fill_md = figures_dir / "wave15_exploration_baseline_closure_writeback.md"
    receipt_json = tables_dir / "wave15_exploration_baseline_closure_receipt.json"
    receipt_md = figures_dir / "wave15_exploration_baseline_closure_receipt.md"
...
def run_closure_writeback(force: bool = False) -> dict[str, str]:
    ...
    phase5_receipt = load_json_dict("results/tables/cascade_benchmark_phase5_gate_coordination_receipt.json")
    ...
    assert_closure_preconditions(
        wave14_receipt, phase5_receipt, meeteval_receipt, demo_wave14_fill, frontier_summary
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave15_exploration_baseline_closure_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave15_exploration_baseline_closure_receipt.json")
        if str(existing.get("execution_status", "")) == "wave15_exploration_baseline_closure_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave14_receipt, phase5_receipt, meeteval_receipt, frontier_summary)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Write Wave15 exploration+baseline closure after Wave14 chain.")
