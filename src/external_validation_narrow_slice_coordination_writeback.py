...
def assert_writeback_preconditions(
    ...
    if str(wave11_receipt.get("execution_status", "")) != "wave11_exploration_baseline_closure_complete":
        raise RuntimeError("Wave11 closure must be complete before external validation narrow slice coordination")
    if str(demo_wave11_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave11 presentation writeback must be filled before external validation coordination")
    if str(demo_wave11_fill.get("storyboard_receipt_status", "")) != "wave11_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave11 storyboard receipt must be wave11_presentation_extension_complete before external coordination"
        )
    ...
    return [
        {
            ...
            "headline": "AISHELL-4 stub slice narrow ASR eval receipt filled",
        ...
        {
            "section_id": "staging_boundary",
            "headline": "Staging execution status documents excerpt fetch without full dataset claim",
            "artifact_anchor": "results/tables/external_validation_slice_staging_execution_status.json",
            "coordination_note": "Coordination artifact only; does not claim full AISHELL-4 benchmark run.",
            "result_label": "external/sanity-check",
        },
        {
            "section_id": "wave11_boundary",
            "headline": "Wave11 closure keeps external validation separate from gold baseline CER",
        ...
        },
        {
            ...
            "headline": "Narrow slice coordination does not upgrade external sanity-check to gold evaluation",
...
def build_fill_row(rows: list[dict[str, str]], slice_id: str) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "external_validation_narrow_slice_coordination_card",
        "coordination_section_count": str(len(rows)),
        "narrow_slice_id": slice_id,
        "execution_receipt_status": "external_validation_narrow_slice_coordination_complete",
        "blocker": "gold_benchmark_claims_still_blocked",
        "fill_note": (
            "Documented external validation narrow slice boundary after Wave11 closure; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "external_validation_narrow_slice_coordination_complete",
        "coordination_scope": "wave11_external_validation_narrow_slice",
        ...
        "expected_inputs": (
            "Wave11 closure, demo wave11 writeback, narrow ASR eval receipt, and external validation board."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# External Validation Narrow Slice Coordination Card (external/sanity-check)",
        "",
        "Narrow slice boundary coordination — not a gold benchmark completion claim.",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# External Validation Narrow Slice Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# External Validation Narrow Slice Coordination Receipt",
        "",
...
def write_outputs(
    ...
    card_csv = tables_dir / "external_validation_narrow_slice_coordination_card.csv"
    card_json = tables_dir / "external_validation_narrow_slice_coordination_card.json"
    card_md = figures_dir / "external_validation_narrow_slice_coordination_card.md"
    fill_csv = tables_dir / "external_validation_narrow_slice_coordination_writeback.csv"
    fill_json = tables_dir / "external_validation_narrow_slice_coordination_writeback.json"
    fill_md = figures_dir / "external_validation_narrow_slice_coordination_writeback.md"
    receipt_json = tables_dir / "external_validation_narrow_slice_coordination_receipt.json"
    receipt_md = figures_dir / "external_validation_narrow_slice_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    assert_writeback_preconditions(wave11_receipt, demo_wave11_fill, narrow_eval_receipt, external_summary)

    receipt_path = PROJECT_ROOT / "results/tables/external_validation_narrow_slice_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/external_validation_narrow_slice_coordination_receipt.json")
        if str(existing.get("execution_status", "")) == "external_validation_narrow_slice_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave11_receipt, narrow_eval_receipt, external_summary)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write external validation narrow slice coordination after Wave11 closure."
    )
