...
from .demo_wave12_presentation_writeback import build_extended_polish_rows as build_wave12_polish_rows
...
def assert_extension_preconditions(
    ...
    wave12_fill: dict[str, Any],
) -> None:
    if str(wave13_receipt.get("execution_status", "")) != "wave13_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave13 exploration baseline closure must be complete before wave13 demo extension"
        )
    if str(wave12_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave12 presentation writeback must be filled before wave13 extension")
    if str(wave12_fill.get("storyboard_receipt_status", "")) != "wave12_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave12 storyboard receipt must be wave12_presentation_extension_complete before wave13 extension"
        )


def build_wave13_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave13",
        "headline": "Wave13 exploration+baseline closure and LLM critic narrow dry-run boundary",
        "artifact_anchor": "results/figures/wave13_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave13 closure card only; HeavyOverlap diagnostic and LLM critic dry-run remain "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave12_polish_rows()
    if any(row["section_id"] == "frontier_wave13" for row in rows):
        return rows
    return rows + [build_wave13_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave13_presentation_extension_complete",
        "storyboard_scope": "wave13_storyboard_extension",
        "expected_inputs": "Wave13 exploration+baseline closure card.",
        "expected_outputs": "Wave13 storyboard extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave13 storyboard extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave13_presentation_extension_complete",
        "walkthrough_scope": "wave13_walkthrough_extension",
        "expected_inputs": "Wave13 exploration+baseline closure card.",
        "expected_outputs": "Wave13 walkthrough extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave13 walkthrough extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave13_presentation_polish_extension",
        "storyboard_receipt_status": "wave13_presentation_extension_complete",
        "walkthrough_receipt_status": "wave13_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave13 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave13_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave13_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave13_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave13 Presentation Writeback",
                "",
...
def run_wave13_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave13_receipt = load_json_dict("results/tables/wave13_exploration_baseline_closure_receipt.json")
    wave12_fill = load_json_dict("results/tables/demo_wave12_presentation_writeback.json")
    assert_extension_preconditions(wave13_receipt, wave12_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave13_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave13_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave13 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave13 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave13_presentation_writeback(force=args.force)
