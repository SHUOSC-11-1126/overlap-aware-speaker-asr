...
RECEIPT_COLUMNS = [
    ...
    "wave133_lightoverlap_refresh_status",
...
def assert_writeback_preconditions(
    ...
    wave133_lightoverlap_receipt: dict[str, Any],
    ...
    if str(wave139_receipt.get("execution_status", "")) != "wave139_exploration_baseline_closure_complete":
        raise RuntimeError("Wave139 closure must be complete before LightOverlap diagnostic refresh coordination")
    if str(demo_wave139_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave139 presentation writeback must be filled before LightOverlap coordination")
    if str(demo_wave139_fill.get("storyboard_receipt_status", "")) != "wave139_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave139 storyboard receipt must be wave139_presentation_extension_complete before LightOverlap coordination"
        )
    if (
        str(wave133_lightoverlap_receipt.get("execution_status", ""))
        != "wave133_speaker_profile_lightoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave133 LightOverlap diagnostic refresh must be complete before Wave139 refresh coordination"
        )
    if str(lightoverlap_receipt.get("execution_status", "")) != "speaker_profile_lightoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Speaker profile LightOverlap diagnostic coordination must be complete before Wave139 refresh"
        )
    ...
    return [
        {
            ...
            "headline": "LightOverlap separation-harm diagnostic scope refresh after Wave139 closure",
        ...
        {
            "section_id": "wave133_refresh",
            "headline": "Wave133 LightOverlap diagnostic refresh precedes Wave139 refresh",
            "artifact_anchor": "results/figures/wave133_speaker_profile_lightoverlap_diagnostic_coordination_card.md",
            "coordination_note": "Prior wave refresh provides chain for cyclic overlap diagnostic coordination.",
            "result_label": "experimental/frontier",
        },
        {
            "section_id": "wave139_boundary",
            "headline": "Wave139 closure defers overlap embedding execution to diagnostic boundaries only",
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave139_speaker_profile_lightoverlap_diagnostic_coordination_card",
        "coordination_section_count": str(len(rows)),
        "diagnostic_case_scope": "LightOverlap",
        "candidate_case_scope": "MidOverlap",
        "execution_receipt_status": "wave139_speaker_profile_lightoverlap_diagnostic_coordination_complete",
        "blocker": "attribution_claims_still_blocked",
        "fill_note": (
            "Documented LightOverlap diagnostic refresh after Wave139 closure; "
...
def build_receipt_row(
    ...
    return {
        ...
        "coordination_scope": "wave139_speaker_profile_lightoverlap_diagnostic",
        "wave139_closure_status": str(wave139_receipt.get("execution_status", "")),
        "wave133_lightoverlap_refresh_status": str(wave133_lightoverlap_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave139 closure, demo wave139 writeback, Wave133 LightOverlap refresh, and Wave8 LightOverlap coordination."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave139 Speaker Profile LightOverlap Diagnostic Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave139 Speaker Profile LightOverlap Diagnostic Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        ...
        "| --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['wave133_lightoverlap_refresh_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave139_speaker_profile_lightoverlap_diagnostic_coordination_card.csv"
    card_json = tables_dir / "wave139_speaker_profile_lightoverlap_diagnostic_coordination_card.json"
    card_md = figures_dir / "wave139_speaker_profile_lightoverlap_diagnostic_coordination_card.md"
    fill_csv = tables_dir / "wave139_speaker_profile_lightoverlap_diagnostic_coordination_writeback.csv"
    fill_json = tables_dir / "wave139_speaker_profile_lightoverlap_diagnostic_coordination_writeback.json"
    fill_md = figures_dir / "wave139_speaker_profile_lightoverlap_diagnostic_coordination_writeback.md"
    receipt_json = tables_dir / "wave139_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    receipt_md = figures_dir / "wave139_speaker_profile_lightoverlap_diagnostic_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    wave133_lightoverlap_receipt = load_json_dict(
        "results/tables/wave133_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    )
    ...
    assert_writeback_preconditions(
        wave139_receipt, demo_wave139_fill, wave133_lightoverlap_receipt, lightoverlap_receipt
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave139_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict(
            "results/tables/wave139_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
        )
        if str(existing.get("execution_status", "")) == "wave139_speaker_profile_lightoverlap_diagnostic_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave139_receipt, wave133_lightoverlap_receipt, lightoverlap_receipt)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave139 speaker profile LightOverlap diagnostic refresh after Wave139 closure."
    )
