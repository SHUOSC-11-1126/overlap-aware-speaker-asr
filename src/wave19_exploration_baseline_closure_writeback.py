...
def assert_closure_preconditions(
    ...
    if str(wave18_receipt.get("execution_status", "")) != "wave18_exploration_baseline_closure_complete":
        raise RuntimeError("Wave18 closure receipt must be complete before Wave19 exploration baseline closure")
    if str(heavyoverlap_receipt.get("execution_status", "")) != "wave18_speaker_profile_heavyoverlap_diagnostic_coordination_complete":
        raise RuntimeError("Wave18 HeavyOverlap diagnostic coordination must be complete before Wave19 closure")
    if str(llm_critic_receipt.get("execution_status", "")) != "wave18_llm_critic_narrow_dry_run_coordination_complete":
        raise RuntimeError("Wave18 LLM critic narrow dry-run coordination must be complete before Wave19 closure")
    if str(demo_wave18_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave18 presentation writeback must be filled before Wave19 closure")
    if str(demo_wave18_fill.get("storyboard_receipt_status", "")) != "wave18_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave18 storyboard receipt must be wave18_presentation_extension_complete before Wave19 closure"
        )
...
def build_closure_rows() -> list[dict[str, str]]:
    return [
        {
            "section_id": "wave18_rollup",
            "headline": "Wave18 closed: HeavyOverlap diagnostic refresh + LLM critic + demo + exploration closure",
        ...
        {
            "section_id": "heavyoverlap_diagnostic",
            "headline": "HeavyOverlap separation-benefit diagnostic refresh coordinated after Wave18 closure",
        ...
        {
            "section_id": "llm_critic_narrow_dry_run",
            "headline": "LLM critic narrow dry-run refresh coordinated on 5/5 gold cases",
            "artifact_anchor": "results/figures/wave18_llm_critic_narrow_dry_run_coordination_card.md",
            "coordination_note": "qualitative/demo dry-run only; verified repair claims still blocked.",
            "result_label": "qualitative/demo",
        },
        {
            "section_id": "demo_wave18",
            "headline": "Demo presentation polish extended to 19 sections under qualitative/demo labeling",
            "artifact_anchor": "results/figures/demo_wave18_presentation_writeback.md",
        ...
        },
        {
            ...
            "coordination_note": "Wave19 closure does not record overlap-case embedding execution sessions.",
            "result_label": "experimental/frontier",
        },
    ]
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave19_exploration_baseline_closure_card",
        "coordination_section_count": str(len(rows)),
        "execution_receipt_status": "wave19_exploration_baseline_closure_complete",
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": (
            "Wave19 exploration+baseline closure rollup after Wave18 chain; stable gold baseline preserved."
        ),
    }
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave19_exploration_baseline_closure_complete",
        "coordination_scope": "wave19_exploration_baseline",
        ...
        "demo_wave18_status": str(demo_wave18_fill.get("storyboard_receipt_status", "")),
        "frontier_go_count": str(frontier_summary.get("go_count", "")),
        "expected_inputs": (
            "Wave18 closure, HeavyOverlap diagnostic, LLM critic narrow dry-run, demo wave18, frontier summary."
        ),
        "writeback_note": (
            "探索+基线 Wave19 closure writeback; qualitative/demo and experimental/frontier labels preserved."
        ),
    }
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave19 Exploration+Baseline Closure Card",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave19 Exploration+Baseline Closure Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave19 Exploration+Baseline Closure Receipt",
        "",
        "| execution_status | wave18_closure_status | frontier_go_count |",
        "| --- | --- | ---: |",
        (
            f"| {row['execution_status']} | {row['wave18_closure_status']} | {row['frontier_go_count']} |"
        ),
    ]
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave19_exploration_baseline_closure_card.csv"
    card_json = tables_dir / "wave19_exploration_baseline_closure_card.json"
    card_md = figures_dir / "wave19_exploration_baseline_closure_card.md"
    fill_csv = tables_dir / "wave19_exploration_baseline_closure_writeback.csv"
    fill_json = tables_dir / "wave19_exploration_baseline_closure_writeback.json"
    fill_md = figures_dir / "wave19_exploration_baseline_closure_writeback.md"
    receipt_json = tables_dir / "wave19_exploration_baseline_closure_receipt.json"
    receipt_md = figures_dir / "wave19_exploration_baseline_closure_receipt.md"
...
def run_closure_writeback(force: bool = False) -> dict[str, str]:
    ...
    llm_critic_receipt = load_json_dict(
        "results/tables/wave18_llm_critic_narrow_dry_run_coordination_receipt.json"
    )
    ...
    assert_closure_preconditions(
        wave18_receipt, heavyoverlap_receipt, llm_critic_receipt, demo_wave18_fill, frontier_summary
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave19_exploration_baseline_closure_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave19_exploration_baseline_closure_receipt.json")
        if str(existing.get("execution_status", "")) == "wave19_exploration_baseline_closure_complete":
            return {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Write Wave19 exploration+baseline closure after Wave18 chain.")
