...
from .demo_wave155_presentation_writeback import build_extended_polish_rows as build_wave156_polish_rows
...
def assert_extension_preconditions(
    ...
    wave155_fill: dict[str, Any],
) -> None:
    if str(wave156_receipt.get("execution_status", "")) != "wave156_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave155 exploration baseline closure must be complete before wave156 demo extension"
        )
    if str(wave155_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave155 presentation writeback must be filled before wave156 extension")
    if str(wave155_fill.get("storyboard_receipt_status", "")) != "wave155_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave155 storyboard receipt must be wave155_presentation_extension_complete before wave156 extension"
        )


def build_wave156_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave156",
        "headline": "Wave156 exploration+baseline closure after Wave155 external validation chain boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave156_polish_rows()
    if any(row["section_id"] == "frontier_wave156" for row in rows):
        return rows
    return rows + [build_wave156_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave156_presentation_extension_complete",
        "storyboard_scope": "wave156_storyboard_extension",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave156_presentation_extension_complete",
        "walkthrough_scope": "wave156_walkthrough_extension",
        "expected_inputs": "Wave104 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave156_presentation_polish_extension",
        "storyboard_receipt_status": "wave156_presentation_extension_complete",
        "walkthrough_receipt_status": "wave156_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave156 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave156_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave156_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave156_presentation_writeback.md"
...
def run_wave156_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave156_receipt = load_json_dict("results/tables/wave156_exploration_baseline_closure_receipt.json")
    wave155_fill = load_json_dict("results/tables/demo_wave155_presentation_writeback.json")
    assert_extension_preconditions(wave156_receipt, wave155_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave156_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave156_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave156 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave156 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave156_presentation_writeback(force=args.force)
