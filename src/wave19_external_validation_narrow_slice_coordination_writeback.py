...
def assert_writeback_preconditions(
    ...
    if str(wave19_receipt.get("execution_status", "")) != "wave19_exploration_baseline_closure_complete":
        raise RuntimeError("Wave19 closure must be complete before external validation narrow slice coordination")
    if str(oppositeoverlap_receipt.get("execution_status", "")) != "wave19_speaker_profile_oppositeoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Wave19 OppositeOverlap diagnostic coordination must be complete before external validation coordination"
        )
    if str(demo_wave19_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave19 presentation writeback must be filled before external validation coordination")
    if str(demo_wave19_fill.get("storyboard_receipt_status", "")) != "wave19_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave19 storyboard receipt must be wave19_presentation_extension_complete before external coordination"
        )
    ...
    if str(prior_coord_receipt.get("execution_status", "")) != "wave15_external_validation_narrow_slice_coordination_complete":
        raise RuntimeError(
            "Wave15 external validation narrow slice coordination must be complete before Wave19 refresh"
        )
...
def build_coordination_rows() -> list[dict[str, str]]:
    return [
        ...
        {
            "section_id": "wave15_prior_coordination",
            "headline": "Wave15 external validation narrow slice coordination remains the prerequisite chain",
            "artifact_anchor": "results/figures/wave15_external_validation_narrow_slice_coordination_card.md",
            "coordination_note": "Wave19 refresh does not replace Wave15 external/sanity-check boundary.",
            "result_label": "external/sanity-check",
        },
        {
            "section_id": "oppositeoverlap_prior",
            "headline": "Wave19 OppositeOverlap diagnostic coordination precedes external validation refresh",
        ...
        {
            "section_id": "wave19_boundary",
            "headline": "Wave19 closure keeps external validation separate from gold baseline CER",
...
def build_fill_row(rows: list[dict[str, str]], slice_id: str) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave19_external_validation_narrow_slice_coordination_card",
        "coordination_section_count": str(len(rows)),
        "narrow_slice_id": slice_id,
        "execution_receipt_status": "wave19_external_validation_narrow_slice_coordination_complete",
        "blocker": "gold_benchmark_claims_still_blocked",
        "fill_note": (
            "Documented external validation narrow slice refresh boundary after Wave19 OppositeOverlap chain; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave19_external_validation_narrow_slice_coordination_complete",
        "coordination_scope": "wave19_external_validation_narrow_slice",
        ...
        "expected_inputs": (
            "Wave19 closure, OppositeOverlap diagnostic, demo wave19, narrow ASR eval receipt, external board."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave19 External Validation Narrow Slice Coordination Card (external/sanity-check)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave19 External Validation Narrow Slice Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave19 External Validation Narrow Slice Coordination Receipt",
        "",
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave19_external_validation_narrow_slice_coordination_card.csv"
    card_json = tables_dir / "wave19_external_validation_narrow_slice_coordination_card.json"
    card_md = figures_dir / "wave19_external_validation_narrow_slice_coordination_card.md"
    fill_csv = tables_dir / "wave19_external_validation_narrow_slice_coordination_writeback.csv"
    fill_json = tables_dir / "wave19_external_validation_narrow_slice_coordination_writeback.json"
    fill_md = figures_dir / "wave19_external_validation_narrow_slice_coordination_writeback.md"
    receipt_json = tables_dir / "wave19_external_validation_narrow_slice_coordination_receipt.json"
    receipt_md = figures_dir / "wave19_external_validation_narrow_slice_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    receipt_path = PROJECT_ROOT / "results/tables/wave19_external_validation_narrow_slice_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave19_external_validation_narrow_slice_coordination_receipt.json")
        if str(existing.get("execution_status", "")) == "wave19_external_validation_narrow_slice_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave19_receipt, oppositeoverlap_receipt, narrow_eval_receipt, external_summary)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave19 external validation narrow slice coordination after OppositeOverlap chain."
    )
