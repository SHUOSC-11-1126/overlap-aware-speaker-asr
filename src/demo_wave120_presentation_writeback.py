...
from .demo_wave119_presentation_writeback import build_extended_polish_rows as build_wave119_polish_rows
...
def assert_extension_preconditions(
    ...
    wave119_fill: dict[str, Any],
) -> None:
    if str(wave120_receipt.get("execution_status", "")) != "wave120_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave120 exploration baseline closure must be complete before wave120 demo extension"
        )
    if str(wave119_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave119 presentation writeback must be filled before wave120 extension")
    if str(wave119_fill.get("storyboard_receipt_status", "")) != "wave119_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave119 storyboard receipt must be wave119_presentation_extension_complete before wave120 extension"
        )


def build_wave120_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave120",
        "headline": "Wave119 exploration+baseline closure after Wave119 external validation chain boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave119_polish_rows()
    if any(row["section_id"] == "frontier_wave120" for row in rows):
        return rows
    return rows + [build_wave120_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave120_presentation_extension_complete",
        "storyboard_scope": "wave120_storyboard_extension",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave120_presentation_extension_complete",
        "walkthrough_scope": "wave120_walkthrough_extension",
        "expected_inputs": "Wave104 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave120_presentation_polish_extension",
        "storyboard_receipt_status": "wave120_presentation_extension_complete",
        "walkthrough_receipt_status": "wave120_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave119 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave120_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave120_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave120_presentation_writeback.md"
...
def run_wave120_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave120_receipt = load_json_dict("results/tables/wave120_exploration_baseline_closure_receipt.json")
    wave119_fill = load_json_dict("results/tables/demo_wave119_presentation_writeback.json")
    assert_extension_preconditions(wave120_receipt, wave119_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave120_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave120_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave120 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave120 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave120_presentation_writeback(force=args.force)
