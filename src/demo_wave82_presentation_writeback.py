...
from .demo_wave81_presentation_writeback import build_extended_polish_rows as build_wave81_polish_rows
...
def assert_extension_preconditions(
    ...
    wave81_fill: dict[str, Any],
) -> None:
    if str(wave82_receipt.get("execution_status", "")) != "wave82_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave82 exploration baseline closure must be complete before wave82 demo extension"
        )
    if str(wave81_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave81 presentation writeback must be filled before wave82 extension")
    if str(wave81_fill.get("storyboard_receipt_status", "")) != "wave81_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave81 storyboard receipt must be wave81_presentation_extension_complete before wave82 extension"
        )


def build_wave82_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave82",
        "headline": "Wave82 exploration+baseline closure and OppositeOverlap speaker-profile diagnostic refresh boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave81_polish_rows()
    if any(row["section_id"] == "frontier_wave82" for row in rows):
        return rows
    return rows + [build_wave82_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave82_presentation_extension_complete",
        "storyboard_scope": "wave82_storyboard_extension",
        "expected_inputs": "Wave82 exploration+baseline closure card.",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave82_presentation_extension_complete",
        "walkthrough_scope": "wave82_walkthrough_extension",
        "expected_inputs": "Wave82 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave82_presentation_polish_extension",
        "storyboard_receipt_status": "wave82_presentation_extension_complete",
        "walkthrough_receipt_status": "wave82_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave82_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave82_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave82_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave82 Presentation Writeback",
                "",
...
def run_wave82_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave82_receipt = load_json_dict("results/tables/wave82_exploration_baseline_closure_receipt.json")
    wave81_fill = load_json_dict("results/tables/demo_wave81_presentation_writeback.json")
    assert_extension_preconditions(wave82_receipt, wave81_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave82_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave82_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave82 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave82 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave82_presentation_writeback(force=args.force)
