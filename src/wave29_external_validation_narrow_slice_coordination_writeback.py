...
RECEIPT_COLUMNS = [
    "execution_status",
    "coordination_scope",
    "wave29_closure_status",
    "midoverlap_coordination_status",
    "wave25_refresh_status",
...
def assert_writeback_preconditions(
    wave29_receipt: dict[str, Any],
    midoverlap_receipt: dict[str, Any],
    demo_wave29_fill: dict[str, Any],
    narrow_eval_receipt: dict[str, Any],
    external_summary: dict[str, Any],
    prior_coord_receipt: dict[str, Any],
) -> None:
    if str(wave29_receipt.get("execution_status", "")) != "wave29_exploration_baseline_closure_complete":
        raise RuntimeError("Wave29 closure must be complete before external validation narrow slice coordination")
    if (
        str(midoverlap_receipt.get("execution_status", ""))
        != "wave28_speaker_profile_midoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave28 MidOverlap diagnostic coordination must be complete before external validation coordination"
        )
    if str(demo_wave29_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave29 presentation writeback must be filled before external validation coordination")
    if str(demo_wave29_fill.get("storyboard_receipt_status", "")) != "wave29_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave29 storyboard receipt must be wave29_presentation_extension_complete before external coordination"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Wave25 external validation narrow slice coordination must be complete before Wave29 refresh"
        )
...
def build_coordination_rows() -> list[dict[str, str]]:
    return [
        ...
        {
            "section_id": "wave25_prior_coordination",
            "headline": "Wave25 external validation narrow slice coordination precedes Wave29 refresh",
            "artifact_anchor": "results/figures/wave25_external_validation_narrow_slice_coordination_card.md",
            "coordination_note": "Wave29 refresh does not replace Wave25 external/sanity-check boundary.",
            "result_label": "external/sanity-check",
        },
        {
            "section_id": "midoverlap_prior",
            "headline": "Wave28 MidOverlap diagnostic coordination precedes external validation refresh",
        ...
        },
        {
            ...
            "headline": "Wave29 closure keeps external validation separate from gold baseline CER",
            "artifact_anchor": "results/figures/wave29_exploration_baseline_closure_card.md",
...
def build_fill_row(rows: list[dict[str, str]], slice_id: str) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave29_external_validation_narrow_slice_coordination_card",
        "coordination_section_count": str(len(rows)),
        "narrow_slice_id": slice_id,
        "execution_receipt_status": "wave29_external_validation_narrow_slice_coordination_complete",
        "blocker": "gold_benchmark_claims_still_blocked",
        "fill_note": (
            "Documented external validation narrow slice refresh boundary after Wave28 MidOverlap chain; "
...
def build_receipt_row(
    ...
    return {
        ...
        "coordination_scope": "wave29_external_validation_narrow_slice",
        "wave29_closure_status": str(wave29_receipt.get("execution_status", "")),
        "midoverlap_coordination_status": str(midoverlap_receipt.get("execution_status", "")),
        "wave25_refresh_status": str(prior_coord_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave29 closure, MidOverlap diagnostic, demo wave29, narrow ASR eval receipt, external board."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave29 External Validation Narrow Slice Coordination Card (external/sanity-check)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave29 External Validation Narrow Slice Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave29 External Validation Narrow Slice Coordination Receipt",
        "",
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave29_external_validation_narrow_slice_coordination_card.csv"
    card_json = tables_dir / "wave29_external_validation_narrow_slice_coordination_card.json"
    card_md = figures_dir / "wave29_external_validation_narrow_slice_coordination_card.md"
    fill_csv = tables_dir / "wave29_external_validation_narrow_slice_coordination_writeback.csv"
    fill_json = tables_dir / "wave29_external_validation_narrow_slice_coordination_writeback.json"
    fill_md = figures_dir / "wave29_external_validation_narrow_slice_coordination_writeback.md"
    receipt_json = tables_dir / "wave29_external_validation_narrow_slice_coordination_receipt.json"
    receipt_md = figures_dir / "wave29_external_validation_narrow_slice_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    wave29_receipt = load_json_dict("results/tables/wave29_exploration_baseline_closure_receipt.json")
    midoverlap_receipt = load_json_dict(
        "results/tables/wave28_speaker_profile_midoverlap_diagnostic_coordination_receipt.json"
    )
    demo_wave29_fill = load_json_dict("results/tables/demo_wave29_presentation_writeback.json")
    ...
    prior_coord_receipt = load_json_dict(
        "results/tables/wave25_external_validation_narrow_slice_coordination_receipt.json"
    )
    ...
    receipt_path = PROJECT_ROOT / "results/tables/wave29_external_validation_narrow_slice_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave29_external_validation_narrow_slice_coordination_receipt.json")
        if str(existing.get("execution_status", "")) == "wave29_external_validation_narrow_slice_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave29_receipt, midoverlap_receipt, prior_coord_receipt, narrow_eval_receipt, external_summary
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave29 external validation narrow slice coordination after MidOverlap chain."
    )
