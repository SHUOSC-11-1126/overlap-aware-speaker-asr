...
from .demo_wave157_presentation_writeback import build_extended_polish_rows as build_wave158_polish_rows
...
def assert_extension_preconditions(
    ...
    wave157_fill: dict[str, Any],
) -> None:
    if str(wave158_receipt.get("execution_status", "")) != "wave158_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave158 exploration baseline closure must be complete before wave158 demo extension"
        )
    if str(wave157_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave157 presentation writeback must be filled before wave158 extension")
    if str(wave157_fill.get("storyboard_receipt_status", "")) != "wave157_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave157 storyboard receipt must be wave157_presentation_extension_complete before wave158 extension"
        )


def build_wave158_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave158",
        "headline": "Wave158 exploration+baseline closure after Wave157 LightOverlap chain boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave158_polish_rows()
    if any(row["section_id"] == "frontier_wave158" for row in rows):
        return rows
    return rows + [build_wave158_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave158_presentation_extension_complete",
        "storyboard_scope": "wave158_storyboard_extension",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave158_presentation_extension_complete",
        "walkthrough_scope": "wave158_walkthrough_extension",
        "expected_inputs": "Wave104 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave158_presentation_polish_extension",
        "storyboard_receipt_status": "wave158_presentation_extension_complete",
        "walkthrough_receipt_status": "wave158_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave158 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave158_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave158_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave158_presentation_writeback.md"
...
def run_wave158_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave158_receipt = load_json_dict("results/tables/wave158_exploration_baseline_closure_receipt.json")
    wave157_fill = load_json_dict("results/tables/demo_wave157_presentation_writeback.json")
    assert_extension_preconditions(wave158_receipt, wave157_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave158_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave158_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave158 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave158 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave158_presentation_writeback(force=args.force)
