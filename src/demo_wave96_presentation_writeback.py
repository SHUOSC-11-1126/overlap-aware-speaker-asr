...
from .demo_wave95_presentation_writeback import build_extended_polish_rows as build_wave95_polish_rows
...
def assert_extension_preconditions(
    ...
    wave95_fill: dict[str, Any],
) -> None:
    if str(wave96_receipt.get("execution_status", "")) != "wave96_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave96 exploration baseline closure must be complete before wave96 demo extension"
        )
    if str(wave95_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave95 presentation writeback must be filled before wave96 extension")
    if str(wave95_fill.get("storyboard_receipt_status", "")) != "wave95_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave95 storyboard receipt must be wave95_presentation_extension_complete before wave96 extension"
        )


def build_wave96_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave96",
        "headline": "Wave96 exploration+baseline closure after Wave95 external validation chain boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave95_polish_rows()
    if any(row["section_id"] == "frontier_wave96" for row in rows):
        return rows
    return rows + [build_wave96_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave96_presentation_extension_complete",
        "storyboard_scope": "wave96_storyboard_extension",
        "expected_inputs": "Wave96 exploration+baseline closure card.",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave96_presentation_extension_complete",
        "walkthrough_scope": "wave96_walkthrough_extension",
        "expected_inputs": "Wave96 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave96_presentation_polish_extension",
        "storyboard_receipt_status": "wave96_presentation_extension_complete",
        "walkthrough_receipt_status": "wave96_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave96_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave96_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave96_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave96 Presentation Writeback",
                "",
...
def run_wave96_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave96_receipt = load_json_dict("results/tables/wave96_exploration_baseline_closure_receipt.json")
    wave95_fill = load_json_dict("results/tables/demo_wave95_presentation_writeback.json")
    assert_extension_preconditions(wave96_receipt, wave95_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave96_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave96_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave96 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave96 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave96_presentation_writeback(force=args.force)
