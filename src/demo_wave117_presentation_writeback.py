...
from .demo_wave116_presentation_writeback import build_extended_polish_rows as build_wave116_polish_rows
...
def assert_extension_preconditions(
    wave117_receipt: dict[str, Any],
    wave116_fill: dict[str, Any],
) -> None:
    if str(wave117_receipt.get("execution_status", "")) != "wave117_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave117 exploration baseline closure must be complete before wave117 demo extension"
        )
    if str(wave116_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave116 presentation writeback must be filled before wave117 extension")
    if str(wave116_fill.get("storyboard_receipt_status", "")) != "wave116_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave116 storyboard receipt must be wave116_presentation_extension_complete before wave117 extension"
        )


def build_wave117_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave117",
        "headline": "Wave116 exploration+baseline closure after Wave116 MidOverlap chain boundary",
        "artifact_anchor": "results/figures/wave117_exploration_baseline_closure_card.md",
        "presentation_note": (
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave116_polish_rows()
    if any(row["section_id"] == "frontier_wave117" for row in rows):
        return rows
    return rows + [build_wave117_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave117_presentation_extension_complete",
        "storyboard_scope": "wave117_storyboard_extension",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave117_presentation_extension_complete",
        "walkthrough_scope": "wave117_walkthrough_extension",
        "expected_inputs": "Wave104 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave117_presentation_polish_extension",
        "storyboard_receipt_status": "wave117_presentation_extension_complete",
        "walkthrough_receipt_status": "wave117_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave116 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave117_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave117_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave117_presentation_writeback.md"
...
def run_wave117_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave117_receipt = load_json_dict("results/tables/wave117_exploration_baseline_closure_receipt.json")
    wave116_fill = load_json_dict("results/tables/demo_wave116_presentation_writeback.json")
    assert_extension_preconditions(wave117_receipt, wave116_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave117_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave117_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave117 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave117 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave117_presentation_writeback(force=args.force)
