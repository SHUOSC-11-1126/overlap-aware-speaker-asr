...
RECEIPT_COLUMNS = [
    "execution_status",
    "coordination_scope",
    "wave136_closure_status",
    "oppositeoverlap_prior_coordination_status",
    "wave130_refresh_status",
...
def assert_writeback_preconditions(
    wave136_receipt: dict[str, Any],
    demo_wave136_fill: dict[str, Any],
    oppositeoverlap_receipt: dict[str, Any],
    wave130_refresh_receipt: dict[str, Any],
    heavyoverlap_refresh_receipt: dict[str, Any],
) -> None:
    if str(wave136_receipt.get("execution_status", "")) != "wave136_exploration_baseline_closure_complete":
        raise RuntimeError("Wave136 closure must be complete before OppositeOverlap diagnostic refresh coordination")
    if str(demo_wave136_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave136 presentation writeback must be filled before OppositeOverlap coordination")
    if str(demo_wave136_fill.get("storyboard_receipt_status", "")) != "wave136_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave136 storyboard receipt must be wave136_presentation_extension_complete before OppositeOverlap coordination"
        )
    if str(oppositeoverlap_receipt.get("execution_status", "")) != "speaker_profile_oppositeoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Speaker profile OppositeOverlap diagnostic coordination must be complete before Wave136 refresh"
        )
    if (
        str(wave130_refresh_receipt.get("execution_status", ""))
        != "wave130_speaker_profile_oppositeoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave130 OppositeOverlap diagnostic refresh must be complete before Wave136 refresh coordination"
        )
    if (
        ...
        raise RuntimeError(
            "Wave135 HeavyOverlap diagnostic refresh must be complete before OppositeOverlap refresh coordination"
        )
    ...
    return [
        {
            ...
            "headline": "OppositeOverlap separation-benefit diagnostic scope refresh after Wave136 closure",
        ...
        {
            "section_id": "wave130_refresh",
            "headline": "Wave130 OppositeOverlap diagnostic refresh precedes Wave136 refresh",
            "artifact_anchor": "results/figures/wave130_speaker_profile_oppositeoverlap_diagnostic_coordination_card.md",
        ...
        },
        {
            ...
            "headline": "Wave135 HeavyOverlap diagnostic refresh precedes OppositeOverlap refresh",
        ...
        {
            "section_id": "wave136_boundary",
            "headline": "Wave136 closure defers overlap embedding execution to diagnostic boundaries only",
            "artifact_anchor": "results/figures/wave136_exploration_baseline_closure_card.md",
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave136_speaker_profile_oppositeoverlap_diagnostic_coordination_card",
        "coordination_section_count": str(len(rows)),
        "diagnostic_case_scope": "OppositeOverlap",
        "candidate_case_scope": "none_remaining",
        "execution_receipt_status": "wave136_speaker_profile_oppositeoverlap_diagnostic_coordination_complete",
        "blocker": "attribution_claims_still_blocked",
        "fill_note": (
            "Documented OppositeOverlap diagnostic refresh after Wave136 closure; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave136_speaker_profile_oppositeoverlap_diagnostic_coordination_complete",
        "coordination_scope": "wave136_speaker_profile_oppositeoverlap_diagnostic",
        "wave136_closure_status": str(wave136_receipt.get("execution_status", "")),
        "oppositeoverlap_prior_coordination_status": str(oppositeoverlap_receipt.get("execution_status", "")),
        "wave130_refresh_status": str(wave130_refresh_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave136 closure, demo wave136, Wave13 OppositeOverlap, Wave130 refresh, and Wave135 HeavyOverlap refresh."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave136 Speaker Profile OppositeOverlap Diagnostic Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave136 Speaker Profile OppositeOverlap Diagnostic Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave136 Speaker Profile OppositeOverlap Diagnostic Coordination Receipt",
        "",
        "| execution_status | wave130_refresh_status | heavyoverlap_refresh_status |",
        "| --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['wave130_refresh_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave136_speaker_profile_oppositeoverlap_diagnostic_coordination_card.csv"
    card_json = tables_dir / "wave136_speaker_profile_oppositeoverlap_diagnostic_coordination_card.json"
    card_md = figures_dir / "wave136_speaker_profile_oppositeoverlap_diagnostic_coordination_card.md"
    fill_csv = tables_dir / "wave136_speaker_profile_oppositeoverlap_diagnostic_coordination_writeback.csv"
    fill_json = tables_dir / "wave136_speaker_profile_oppositeoverlap_diagnostic_coordination_writeback.json"
    fill_md = figures_dir / "wave136_speaker_profile_oppositeoverlap_diagnostic_coordination_writeback.md"
    receipt_json = tables_dir / "wave136_speaker_profile_oppositeoverlap_diagnostic_coordination_receipt.json"
    receipt_md = figures_dir / "wave136_speaker_profile_oppositeoverlap_diagnostic_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    wave136_receipt = load_json_dict("results/tables/wave136_exploration_baseline_closure_receipt.json")
    demo_wave136_fill = load_json_dict("results/tables/demo_wave136_presentation_writeback.json")
    ...
    wave130_refresh_receipt = load_json_dict(
    ...
    assert_writeback_preconditions(
        ...
        wave130_refresh_receipt,
        heavyoverlap_refresh_receipt,
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave136_speaker_profile_oppositeoverlap_diagnostic_coordination_receipt.json"
    if receipt_path.exists() and not force:
        ...
        if str(existing.get("execution_status", "")) == "wave136_speaker_profile_oppositeoverlap_diagnostic_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave136_receipt, oppositeoverlap_receipt, wave130_refresh_receipt, heavyoverlap_refresh_receipt
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave136 speaker profile OppositeOverlap diagnostic refresh after Wave136 closure."
    )
