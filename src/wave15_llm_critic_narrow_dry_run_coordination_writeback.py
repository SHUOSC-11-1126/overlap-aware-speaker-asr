...
def assert_writeback_preconditions(
    ...
    if str(wave15_receipt.get("execution_status", "")) != "wave15_exploration_baseline_closure_complete":
        raise RuntimeError("Wave15 closure must be complete before LLM critic narrow dry-run coordination")
    if str(external_receipt.get("execution_status", "")) != "wave15_external_validation_narrow_slice_coordination_complete":
        raise RuntimeError(
            "Wave15 external validation coordination must be complete before LLM critic coordination"
        )
    if str(demo_wave15_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave15 presentation writeback must be filled before LLM critic coordination")
    if str(demo_wave15_fill.get("storyboard_receipt_status", "")) != "wave15_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave15 storyboard receipt must be wave15_presentation_extension_complete before LLM critic coordination"
        )
    ...
    if str(prior_llm_receipt.get("execution_status", "")) != "llm_critic_narrow_dry_run_coordination_complete":
        raise RuntimeError(
            "Wave12 LLM critic narrow dry-run coordination must be complete before Wave15 refresh"
        )
...
def build_coordination_rows() -> list[dict[str, str]]:
    return [
        ...
        {
            "section_id": "external_validation_prior",
            "headline": "Wave15 external validation coordination precedes critic narrow dry-run refresh",
            "artifact_anchor": "results/figures/wave15_external_validation_narrow_slice_coordination_card.md",
            "coordination_note": "external/sanity-check chain provides slice context only.",
            "result_label": "external/sanity-check",
        },
        {
            "section_id": "wave15_boundary",
            "headline": "Wave15 closure keeps LLM critic dry-run separate from gold baseline CER",
...
def build_fill_row(rows: list[dict[str, str]], narrow_dry_run_case_count: str) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave15_llm_critic_narrow_dry_run_coordination_card",
        "coordination_section_count": str(len(rows)),
        "narrow_dry_run_case_count": narrow_dry_run_case_count,
        "execution_receipt_status": "wave15_llm_critic_narrow_dry_run_coordination_complete",
        "blocker": "verified_repair_claims_still_blocked",
        "fill_note": (
            "Documented LLM critic narrow dry-run refresh boundary after Wave15 external validation chain; "
...
def build_receipt_row(
    ...
    return {
        ...
        "coordination_scope": "wave15_llm_critic_narrow_dry_run",
        ...
        "expected_inputs": (
            "Wave15 closure, external validation coordination, demo wave15, LLM critic review pass status."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave15 LLM Critic Narrow Dry-Run Coordination Card (qualitative/demo)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave15 LLM Critic Narrow Dry-Run Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave15 LLM Critic Narrow Dry-Run Coordination Receipt",
        "",
        "| execution_status | external_validation_coordination_status | llm_critic_board_state | blocker |",
        "| --- | --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['external_validation_coordination_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave15_llm_critic_narrow_dry_run_coordination_card.csv"
    card_json = tables_dir / "wave15_llm_critic_narrow_dry_run_coordination_card.json"
    card_md = figures_dir / "wave15_llm_critic_narrow_dry_run_coordination_card.md"
    fill_csv = tables_dir / "wave15_llm_critic_narrow_dry_run_coordination_writeback.csv"
    fill_json = tables_dir / "wave15_llm_critic_narrow_dry_run_coordination_writeback.json"
    fill_md = figures_dir / "wave15_llm_critic_narrow_dry_run_coordination_writeback.md"
    receipt_json = tables_dir / "wave15_llm_critic_narrow_dry_run_coordination_receipt.json"
    receipt_md = figures_dir / "wave15_llm_critic_narrow_dry_run_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    prior_llm_receipt = load_json_dict("results/tables/llm_critic_narrow_dry_run_coordination_receipt.json")
    assert_writeback_preconditions(
        wave15_receipt, external_receipt, demo_wave15_fill, llm_summary, prior_llm_receipt
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave15_llm_critic_narrow_dry_run_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave15_llm_critic_narrow_dry_run_coordination_receipt.json")
        if str(existing.get("execution_status", "")) == "wave15_llm_critic_narrow_dry_run_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave15_receipt, external_receipt, llm_summary)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave15 LLM critic narrow dry-run coordination after external validation chain."
    )
