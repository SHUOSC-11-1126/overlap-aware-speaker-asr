...
def assert_closure_preconditions(
    ...
    if str(wave122_receipt.get("execution_status", "")) != "wave122_exploration_baseline_closure_complete":
        raise RuntimeError("Wave122 closure receipt must be complete before Wave123 exploration baseline closure")
    if (
        str(midoverlap_receipt.get("execution_status", ""))
        != "wave122_speaker_profile_midoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError("Wave122 MidOverlap coordination must be complete before Wave123 closure")
    if str(demo_wave122_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave122 presentation writeback must be filled before Wave123 closure")
    if str(demo_wave122_fill.get("storyboard_receipt_status", "")) != "wave122_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave122 storyboard receipt must be wave122_presentation_extension_complete before Wave123 closure"
        )
...
def build_closure_rows() -> list[dict[str, str]]:
    return [
        {
            "section_id": "wave122_rollup",
            "headline": "Wave122 closed: MidOverlap diagnostic refresh + demo + exploration closure",
        ...
        },
        {
            ...
            "headline": "MidOverlap coordination refresh coordinated after Wave122 closure",
        ...
        {
            "section_id": "demo_wave122",
            "headline": "Demo presentation polish extended to 123 sections under qualitative/demo labeling",
            "artifact_anchor": "results/figures/demo_wave122_presentation_writeback.md",
            "coordination_note": "No live demo or benchmark timing claims.",
            "result_label": "qualitative/demo",
        },
        {
            "section_id": "frontier_board",
            "headline": "Frontier go/no-go board remains all_ready after Wave122 chain",
        ...
        },
        {
            ...
            "coordination_note": "Wave123 closure does not record overlap-case embedding execution sessions.",
            "result_label": "experimental/frontier",
        },
    ]
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave123_exploration_baseline_closure_card",
        "coordination_section_count": str(len(rows)),
        "execution_receipt_status": "wave123_exploration_baseline_closure_complete",
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": (
            "Wave123 exploration+baseline closure rollup after Wave122 chain; stable gold baseline preserved."
        ),
    }
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave123_exploration_baseline_closure_complete",
        "coordination_scope": "wave123_exploration_baseline",
        ...
        "demo_wave122_status": str(demo_wave122_fill.get("storyboard_receipt_status", "")),
        "frontier_go_count": str(frontier_summary.get("go_count", "")),
        "expected_inputs": (
            "Wave79 closure, LightOverlap, demo wave79, frontier summary."
        ),
        "writeback_note": (
            "探索+基线 Wave123 closure writeback; qualitative/demo and experimental/frontier labels preserved."
        ),
    }
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave123 Exploration+Baseline Closure Card",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave123 Exploration+Baseline Closure Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave123 Exploration+Baseline Closure Receipt",
        "",
        "| execution_status | wave122_closure_status | frontier_go_count |",
        "| --- | --- | ---: |",
        (
            f"| {row['execution_status']} | {row['wave122_closure_status']} | {row['frontier_go_count']} |"
        ),
    ]
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave123_exploration_baseline_closure_card.csv"
    card_json = tables_dir / "wave123_exploration_baseline_closure_card.json"
    card_md = figures_dir / "wave123_exploration_baseline_closure_card.md"
    fill_csv = tables_dir / "wave123_exploration_baseline_closure_writeback.csv"
    fill_json = tables_dir / "wave123_exploration_baseline_closure_writeback.json"
    fill_md = figures_dir / "wave123_exploration_baseline_closure_writeback.md"
    receipt_json = tables_dir / "wave123_exploration_baseline_closure_receipt.json"
    receipt_md = figures_dir / "wave123_exploration_baseline_closure_receipt.md"
...
def run_closure_writeback(force: bool = False) -> dict[str, str]:
    ...
    assert_closure_preconditions(wave122_receipt, midoverlap_receipt, demo_wave122_fill, frontier_summary)

    receipt_path = PROJECT_ROOT / "results/tables/wave123_exploration_baseline_closure_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave123_exploration_baseline_closure_receipt.json")
        if str(existing.get("execution_status", "")) == "wave123_exploration_baseline_closure_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave122_receipt, midoverlap_receipt, demo_wave122_fill, frontier_summary)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Write Wave123 exploration+baseline closure after Wave122 chain.")
