...
from .demo_wave16_presentation_writeback import build_extended_polish_rows as build_wave16_polish_rows
...
def assert_extension_preconditions(
    ...
    wave16_fill: dict[str, Any],
) -> None:
    if str(wave17_receipt.get("execution_status", "")) != "wave17_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave17 exploration baseline closure must be complete before wave17 demo extension"
        )
    if str(wave16_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave16 presentation writeback must be filled before wave17 extension")
    if str(wave16_fill.get("storyboard_receipt_status", "")) != "wave16_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave16 storyboard receipt must be wave16_presentation_extension_complete before wave17 extension"
        )


def build_wave17_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave17",
        "headline": "Wave17 exploration+baseline closure and MidOverlap diagnostic refresh boundary",
        "artifact_anchor": "results/figures/wave17_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave17 closure card only; MidOverlap diagnostic and MeetEval cpWER dry-run remain "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave16_polish_rows()
    if any(row["section_id"] == "frontier_wave17" for row in rows):
        return rows
    return rows + [build_wave17_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave17_presentation_extension_complete",
        "storyboard_scope": "wave17_storyboard_extension",
        "expected_inputs": "Wave17 exploration+baseline closure card.",
        "expected_outputs": "Wave17 storyboard extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave17 storyboard extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave17_presentation_extension_complete",
        "walkthrough_scope": "wave17_walkthrough_extension",
        "expected_inputs": "Wave17 exploration+baseline closure card.",
        "expected_outputs": "Wave17 walkthrough extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave17 walkthrough extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave17_presentation_polish_extension",
        "storyboard_receipt_status": "wave17_presentation_extension_complete",
        "walkthrough_receipt_status": "wave17_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave17 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave17_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave17_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave17_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave17 Presentation Writeback",
                "",
...
def run_wave17_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave17_receipt = load_json_dict("results/tables/wave17_exploration_baseline_closure_receipt.json")
    wave16_fill = load_json_dict("results/tables/demo_wave16_presentation_writeback.json")
    assert_extension_preconditions(wave17_receipt, wave16_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave17_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave17_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave17 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave17 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave17_presentation_writeback(force=args.force)
