...
from .demo_wave10_presentation_writeback import build_extended_polish_rows as build_wave10_polish_rows
...
def assert_extension_preconditions(
    ...
    wave10_fill: dict[str, Any],
) -> None:
    if str(wave11_receipt.get("execution_status", "")) != "wave11_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave11 exploration baseline closure must be complete before wave11 demo extension"
        )
    if str(wave10_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave10 presentation writeback must be filled before wave11 extension")
    if str(wave10_fill.get("storyboard_receipt_status", "")) != "wave10_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave10 storyboard receipt must be wave10_presentation_extension_complete before wave11 extension"
        )


def build_wave11_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave11",
        "headline": "Wave11 exploration+baseline closure and controlled benchmark timing boundary",
        "artifact_anchor": "results/figures/wave11_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave11 closure card only; phase2 gate and MeetEval dry-run remain "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave10_polish_rows()
    if any(row["section_id"] == "frontier_wave11" for row in rows):
        return rows
    return rows + [build_wave11_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave11_presentation_extension_complete",
        "storyboard_scope": "wave11_storyboard_extension",
        "expected_inputs": "Wave11 exploration+baseline closure card.",
        "expected_outputs": "Wave11 storyboard extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave11 storyboard extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave11_presentation_extension_complete",
        "walkthrough_scope": "wave11_walkthrough_extension",
        "expected_inputs": "Wave11 exploration+baseline closure card.",
        "expected_outputs": "Wave11 walkthrough extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave11 walkthrough extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave11_presentation_polish_extension",
        "storyboard_receipt_status": "wave11_presentation_extension_complete",
        "walkthrough_receipt_status": "wave11_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave11 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave11_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave11_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave11_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave11 Presentation Writeback",
                "",
...
def run_wave11_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave11_receipt = load_json_dict("results/tables/wave11_exploration_baseline_closure_receipt.json")
    wave10_fill = load_json_dict("results/tables/demo_wave10_presentation_writeback.json")
    assert_extension_preconditions(wave11_receipt, wave10_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave11_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave11_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave11 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave11 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave11_presentation_writeback(force=args.force)
