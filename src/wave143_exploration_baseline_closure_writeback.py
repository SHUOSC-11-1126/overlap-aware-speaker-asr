...
def assert_closure_preconditions(
    ...
    if str(wave142_receipt.get("execution_status", "")) != "wave142_exploration_baseline_closure_complete":
        raise RuntimeError("Wave142 closure receipt must be complete before Wave143 exploration baseline closure")
    if (
        str(oppositeoverlap_receipt.get("execution_status", ""))
        != "wave142_speaker_profile_oppositeoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError("Wave142 OppositeOverlap coordination must be complete before Wave143 closure")
    if str(demo_wave142_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave142 presentation writeback must be filled before Wave143 closure")
    if str(demo_wave142_fill.get("storyboard_receipt_status", "")) != "wave142_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave142 storyboard receipt must be wave142_presentation_extension_complete before Wave143 closure"
        )
...
def build_closure_rows() -> list[dict[str, str]]:
    return [
        {
            "section_id": "wave142_rollup",
            "headline": "Wave142 closed: OppositeOverlap diagnostic refresh + demo + exploration closure",
        ...
        },
        {
            ...
            "headline": "OppositeOverlap coordination refresh coordinated after Wave142 closure",
        ...
        {
            "section_id": "demo_wave142",
            "headline": "Demo presentation polish extended to 143 sections under qualitative/demo labeling",
            "artifact_anchor": "results/figures/demo_wave142_presentation_writeback.md",
            "coordination_note": "No live demo or benchmark timing claims.",
            "result_label": "qualitative/demo",
        },
        {
            "section_id": "frontier_board",
            "headline": "Frontier go/no-go board remains all_ready after Wave142 chain",
        ...
        },
        {
            ...
            "coordination_note": "Wave143 closure does not record external benchmark execution sessions.",
            "result_label": "experimental/frontier",
        },
    ]
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave143_exploration_baseline_closure_card",
        "coordination_section_count": str(len(rows)),
        "execution_receipt_status": "wave143_exploration_baseline_closure_complete",
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": (
            "Wave143 exploration+baseline closure rollup after Wave142 chain; stable gold baseline preserved."
        ),
    }
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave143_exploration_baseline_closure_complete",
        "coordination_scope": "wave143_exploration_baseline",
        ...
        "demo_wave142_status": str(demo_wave142_fill.get("storyboard_receipt_status", "")),
        "frontier_go_count": str(frontier_summary.get("go_count", "")),
        "expected_inputs": (
            "Wave142 closure, OppositeOverlap, demo wave142, frontier summary."
        ),
        "writeback_note": (
            "探索+基线 Wave143 closure writeback; qualitative/demo and experimental/frontier labels preserved."
        ),
    }
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave143 Exploration+Baseline Closure Card",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave143 Exploration+Baseline Closure Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave143 Exploration+Baseline Closure Receipt",
        "",
        "| execution_status | wave142_closure_status | frontier_go_count |",
        "| --- | --- | ---: |",
        (
            f"| {row['execution_status']} | {row['wave142_closure_status']} | {row['frontier_go_count']} |"
        ),
    ]
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave143_exploration_baseline_closure_card.csv"
    card_json = tables_dir / "wave143_exploration_baseline_closure_card.json"
    card_md = figures_dir / "wave143_exploration_baseline_closure_card.md"
    fill_csv = tables_dir / "wave143_exploration_baseline_closure_writeback.csv"
    fill_json = tables_dir / "wave143_exploration_baseline_closure_writeback.json"
    fill_md = figures_dir / "wave143_exploration_baseline_closure_writeback.md"
    receipt_json = tables_dir / "wave143_exploration_baseline_closure_receipt.json"
    receipt_md = figures_dir / "wave143_exploration_baseline_closure_receipt.md"
...
def run_closure_writeback(force: bool = False) -> dict[str, str]:
    ...
    assert_closure_preconditions(wave142_receipt, oppositeoverlap_receipt, demo_wave142_fill, frontier_summary)

    receipt_path = PROJECT_ROOT / "results/tables/wave143_exploration_baseline_closure_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave143_exploration_baseline_closure_receipt.json")
        if str(existing.get("execution_status", "")) == "wave143_exploration_baseline_closure_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave142_receipt, oppositeoverlap_receipt, demo_wave142_fill, frontier_summary)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Write Wave143 exploration+baseline closure after Wave142 chain.")
