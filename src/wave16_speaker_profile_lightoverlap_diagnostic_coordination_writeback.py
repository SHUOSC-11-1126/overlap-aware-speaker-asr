...
def assert_writeback_preconditions(
    ...
    if str(wave16_receipt.get("execution_status", "")) != "wave16_exploration_baseline_closure_complete":
        raise RuntimeError("Wave16 closure must be complete before LightOverlap diagnostic refresh coordination")
    if str(demo_wave16_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave16 presentation writeback must be filled before LightOverlap coordination")
    if str(demo_wave16_fill.get("storyboard_receipt_status", "")) != "wave16_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave16 storyboard receipt must be wave16_presentation_extension_complete before LightOverlap coordination"
        )
    if str(lightoverlap_receipt.get("execution_status", "")) != "speaker_profile_lightoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Speaker profile LightOverlap diagnostic coordination must be complete before Wave16 refresh"
        )
    ...
    return [
        {
            ...
            "headline": "LightOverlap separation-harm diagnostic scope refresh after Wave16 closure",
        ...
        },
        {
            ...
            "coordination_note": "Wave16 refresh does not replace original LightOverlap boundary documentation.",
            "result_label": "experimental/frontier",
        },
        {
            "section_id": "overlap_chain_status",
            "headline": "MidOverlap through OppositeOverlap diagnostic scopes coordinated in later waves",
            "artifact_anchor": "results/figures/speaker_profile_oppositeoverlap_diagnostic_coordination_card.md",
            "coordination_note": "Full gold-case diagnostic chain documented; embedding execution still blocked.",
            "result_label": "experimental/frontier",
        },
        {
            "section_id": "wave16_boundary",
            "headline": "Wave16 closure defers overlap embedding execution to diagnostic boundaries only",
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave16_speaker_profile_lightoverlap_diagnostic_coordination_card",
        "coordination_section_count": str(len(rows)),
        "diagnostic_case_scope": "LightOverlap",
        "candidate_case_scope": "MidOverlap",
        "execution_receipt_status": "wave16_speaker_profile_lightoverlap_diagnostic_coordination_complete",
        "blocker": "attribution_claims_still_blocked",
        "fill_note": (
            "Documented LightOverlap diagnostic refresh after Wave16 closure; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave16_speaker_profile_lightoverlap_diagnostic_coordination_complete",
        "coordination_scope": "wave16_speaker_profile_lightoverlap_diagnostic",
        ...
        "expected_inputs": (
            "Wave16 closure, demo wave16 writeback, and Wave8 LightOverlap diagnostic coordination."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave16 Speaker Profile LightOverlap Diagnostic Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave16 Speaker Profile LightOverlap Diagnostic Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave16 Speaker Profile LightOverlap Diagnostic Coordination Receipt",
        "",
        "| execution_status | lightoverlap_prior_coordination_status | separation_harm_signal |",
        "| --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['lightoverlap_prior_coordination_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave16_speaker_profile_lightoverlap_diagnostic_coordination_card.csv"
    card_json = tables_dir / "wave16_speaker_profile_lightoverlap_diagnostic_coordination_card.json"
    card_md = figures_dir / "wave16_speaker_profile_lightoverlap_diagnostic_coordination_card.md"
    fill_csv = tables_dir / "wave16_speaker_profile_lightoverlap_diagnostic_coordination_writeback.csv"
    fill_json = tables_dir / "wave16_speaker_profile_lightoverlap_diagnostic_coordination_writeback.json"
    fill_md = figures_dir / "wave16_speaker_profile_lightoverlap_diagnostic_coordination_writeback.md"
    receipt_json = tables_dir / "wave16_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    receipt_md = figures_dir / "wave16_speaker_profile_lightoverlap_diagnostic_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    assert_writeback_preconditions(wave16_receipt, demo_wave16_fill, lightoverlap_receipt)

    receipt_path = PROJECT_ROOT / "results/tables/wave16_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict(
            "results/tables/wave16_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
        )
        if str(existing.get("execution_status", "")) == "wave16_speaker_profile_lightoverlap_diagnostic_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave16_receipt, lightoverlap_receipt)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave16 speaker profile LightOverlap diagnostic refresh after Wave16 closure."
    )
