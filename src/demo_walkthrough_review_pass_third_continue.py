...
def select_next_step(steps: list[dict[str, str]], completed_step_ids: set[str]) -> dict[str, str]:
    for step in steps:
        ...
        if step_id and step_id not in completed_step_ids:
            return step
    return steps[4] if len(steps) > 4 else {"step_id": "5", "focus": "Next-step framing"}


def build_continue_row(next_step: dict[str, str], completed_count: int) -> dict[str, str]:
    step_id = str(next_step.get("step_id", "5"))
    focus = str(next_step.get("focus", "Next-step framing"))
    return {
        "review_order": "5",
        "step_id": step_id,
...
def build_continue_lines(row: dict[str, str]) -> list[str]:
    lines = [
        ...
        "",
        "This generated note records the fifth qualitative walkthrough review pass in step order. "
...
def write_outputs(
    ...
    continue_csv_path = tables_dir / "demo_walkthrough_review_pass_third_continue.csv"
    continue_json_path = tables_dir / "demo_walkthrough_review_pass_third_continue.json"
    continue_md_path = figures_dir / "demo_walkthrough_review_pass_third_continue.md"
    fifth_csv_path = tables_dir / "demo_walkthrough_review_pass_fifth.csv"
    fifth_json_path = tables_dir / "demo_walkthrough_review_pass_fifth.json"
    fifth_md_path = figures_dir / "demo_walkthrough_review_pass_fifth.md"
    receipt_json_path = tables_dir / "demo_walkthrough_review_pass_third_continue_receipt.json"
    receipt_md_path = figures_dir / "demo_walkthrough_review_pass_third_continue_receipt.md"
    ...
    fifth_review_row = dict(review_row)
    fifth_review_row["review_order"] = "5"
    with fifth_csv_path.open("w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=list(fifth_review_row.keys()))
        writer.writeheader()
        writer.writerow(fifth_review_row)
    fifth_json_path.write_text(json.dumps(fifth_review_row, ensure_ascii=False, indent=2), encoding="utf-8")
    fifth_md_path.write_text("\n".join(build_review_lines(fifth_review_row)) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    for receipt in receipt_rows:
        ...
        receipt["writeback_note"] = (
            f"Fifth qualitative walkthrough review documented for step {review_row['step_id']}; "
    ...
    print(f"Wrote demo walkthrough review pass third continue CSV: {continue_csv_path.relative_to(PROJECT_ROOT)}")
    print(f"Wrote demo walkthrough review pass third continue JSON: {continue_json_path.relative_to(PROJECT_ROOT)}")
    print(f"Wrote demo walkthrough review pass third continue note: {continue_md_path.relative_to(PROJECT_ROOT)}")
    print(f"Wrote demo walkthrough review pass fifth CSV: {fifth_csv_path.relative_to(PROJECT_ROOT)}")
    print(f"Wrote demo walkthrough review pass fifth JSON: {fifth_json_path.relative_to(PROJECT_ROOT)}")
    print(f"Wrote demo walkthrough review pass fifth note: {fifth_md_path.relative_to(PROJECT_ROOT)}")
    print(f"Wrote demo walkthrough review pass third continue receipt JSON: {receipt_json_path.relative_to(PROJECT_ROOT)}")
    print(f"Wrote demo walkthrough review pass third continue receipt note: {receipt_md_path.relative_to(PROJECT_ROOT)}")
