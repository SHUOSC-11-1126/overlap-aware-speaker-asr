...
def assert_writeback_preconditions(
    ...
    if str(wave17_receipt.get("execution_status", "")) != "wave17_exploration_baseline_closure_complete":
        raise RuntimeError("Wave17 closure must be complete before MidOverlap diagnostic refresh coordination")
    if str(demo_wave17_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave17 presentation writeback must be filled before MidOverlap coordination")
    if str(demo_wave17_fill.get("storyboard_receipt_status", "")) != "wave17_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave17 storyboard receipt must be wave17_presentation_extension_complete before MidOverlap coordination"
        )
    if str(midoverlap_receipt.get("execution_status", "")) != "speaker_profile_midoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Speaker profile MidOverlap diagnostic coordination must be complete before Wave17 refresh"
        )
    if str(lightoverlap_refresh_receipt.get("execution_status", "")) != "wave16_speaker_profile_lightoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Wave16 LightOverlap diagnostic refresh must be complete before MidOverlap refresh coordination"
        )
    ...
    return [
        {
            ...
            "headline": "MidOverlap separation-harm diagnostic scope refresh after Wave17 closure",
        ...
        },
        {
            ...
            "coordination_note": "Wave17 refresh does not replace original MidOverlap boundary documentation.",
            "result_label": "experimental/frontier",
        },
        {
            "section_id": "lightoverlap_refresh",
            "headline": "Wave16 LightOverlap diagnostic refresh precedes MidOverlap refresh",
        ...
        },
        {
            ...
            "headline": "Wave17 closure defers overlap embedding execution to diagnostic boundaries only",
        ...
        },
        {
            ...
            "headline": "MidOverlap refresh does not upgrade diagnostic scope to attribution claims",
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave17_speaker_profile_midoverlap_diagnostic_coordination_card",
        "coordination_section_count": str(len(rows)),
        "diagnostic_case_scope": "MidOverlap",
        "candidate_case_scope": "HeavyOverlap",
        "execution_receipt_status": "wave17_speaker_profile_midoverlap_diagnostic_coordination_complete",
        "blocker": "attribution_claims_still_blocked",
        "fill_note": (
            "Documented MidOverlap diagnostic refresh after Wave17 closure; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave17_speaker_profile_midoverlap_diagnostic_coordination_complete",
        "coordination_scope": "wave17_speaker_profile_midoverlap_diagnostic",
        ...
        "expected_inputs": (
            "Wave17 closure, demo wave17 writeback, Wave9 MidOverlap diagnostic, and Wave16 LightOverlap refresh."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave17 Speaker Profile MidOverlap Diagnostic Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave17 Speaker Profile MidOverlap Diagnostic Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave17 Speaker Profile MidOverlap Diagnostic Coordination Receipt",
        "",
        "| execution_status | midoverlap_prior_coordination_status | separation_harm_signal |",
        "| --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['midoverlap_prior_coordination_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave17_speaker_profile_midoverlap_diagnostic_coordination_card.csv"
    card_json = tables_dir / "wave17_speaker_profile_midoverlap_diagnostic_coordination_card.json"
    card_md = figures_dir / "wave17_speaker_profile_midoverlap_diagnostic_coordination_card.md"
    fill_csv = tables_dir / "wave17_speaker_profile_midoverlap_diagnostic_coordination_writeback.csv"
    fill_json = tables_dir / "wave17_speaker_profile_midoverlap_diagnostic_coordination_writeback.json"
    fill_md = figures_dir / "wave17_speaker_profile_midoverlap_diagnostic_coordination_writeback.md"
    receipt_json = tables_dir / "wave17_speaker_profile_midoverlap_diagnostic_coordination_receipt.json"
    receipt_md = figures_dir / "wave17_speaker_profile_midoverlap_diagnostic_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    assert_writeback_preconditions(
        wave17_receipt, demo_wave17_fill, midoverlap_receipt, lightoverlap_refresh_receipt
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave17_speaker_profile_midoverlap_diagnostic_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict(
            "results/tables/wave17_speaker_profile_midoverlap_diagnostic_coordination_receipt.json"
        )
        if str(existing.get("execution_status", "")) == "wave17_speaker_profile_midoverlap_diagnostic_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave17_receipt, midoverlap_receipt, lightoverlap_refresh_receipt)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave17 speaker profile MidOverlap diagnostic refresh after Wave17 closure."
    )
