...
from .demo_wave97_presentation_writeback import build_extended_polish_rows as build_wave97_polish_rows
...
def assert_extension_preconditions(
    ...
    wave97_fill: dict[str, Any],
) -> None:
    if str(wave98_receipt.get("execution_status", "")) != "wave98_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave98 exploration baseline closure must be complete before wave98 demo extension"
        )
    if str(wave97_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave97 presentation writeback must be filled before wave98 extension")
    if str(wave97_fill.get("storyboard_receipt_status", "")) != "wave97_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave97 storyboard receipt must be wave97_presentation_extension_complete before wave98 extension"
        )


def build_wave98_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave98",
        "headline": "Wave98 exploration+baseline closure after Wave97 LightOverlap chain boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave97_polish_rows()
    if any(row["section_id"] == "frontier_wave98" for row in rows):
        return rows
    return rows + [build_wave98_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave98_presentation_extension_complete",
        "storyboard_scope": "wave98_storyboard_extension",
        "expected_inputs": "Wave98 exploration+baseline closure card.",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave98_presentation_extension_complete",
        "walkthrough_scope": "wave98_walkthrough_extension",
        "expected_inputs": "Wave98 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave98_presentation_polish_extension",
        "storyboard_receipt_status": "wave98_presentation_extension_complete",
        "walkthrough_receipt_status": "wave98_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave98_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave98_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave98_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave98 Presentation Writeback",
                "",
...
def run_wave98_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave98_receipt = load_json_dict("results/tables/wave98_exploration_baseline_closure_receipt.json")
    wave97_fill = load_json_dict("results/tables/demo_wave97_presentation_writeback.json")
    assert_extension_preconditions(wave98_receipt, wave97_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave98_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave98_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave98 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave98 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave98_presentation_writeback(force=args.force)
