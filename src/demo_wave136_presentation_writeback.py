...
from .demo_wave135_presentation_writeback import build_extended_polish_rows as build_wave135_polish_rows
...
def assert_extension_preconditions(
    ...
    wave135_fill: dict[str, Any],
) -> None:
    if str(wave136_receipt.get("execution_status", "")) != "wave136_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave135 exploration baseline closure must be complete before wave136 demo extension"
        )
    if str(wave135_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave135 presentation writeback must be filled before wave136 extension")
    if str(wave135_fill.get("storyboard_receipt_status", "")) != "wave135_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave135 storyboard receipt must be wave135_presentation_extension_complete before wave136 extension"
        )


def build_wave136_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave136",
        "headline": "Wave135 exploration+baseline closure after Wave135 LightOverlap chain boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave135_polish_rows()
    if any(row["section_id"] == "frontier_wave136" for row in rows):
        return rows
    return rows + [build_wave136_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave136_presentation_extension_complete",
        "storyboard_scope": "wave136_storyboard_extension",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave136_presentation_extension_complete",
        "walkthrough_scope": "wave136_walkthrough_extension",
        "expected_inputs": "Wave104 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave136_presentation_polish_extension",
        "storyboard_receipt_status": "wave136_presentation_extension_complete",
        "walkthrough_receipt_status": "wave136_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave135 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave136_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave136_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave136_presentation_writeback.md"
...
def run_wave136_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave136_receipt = load_json_dict("results/tables/wave136_exploration_baseline_closure_receipt.json")
    wave135_fill = load_json_dict("results/tables/demo_wave135_presentation_writeback.json")
    assert_extension_preconditions(wave136_receipt, wave135_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave136_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave136_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave135 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave136 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave136_presentation_writeback(force=args.force)
