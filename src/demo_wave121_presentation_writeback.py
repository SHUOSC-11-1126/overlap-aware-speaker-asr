...
from .demo_wave120_presentation_writeback import build_extended_polish_rows as build_wave120_polish_rows
...
def assert_extension_preconditions(
    ...
    wave120_fill: dict[str, Any],
) -> None:
    if str(wave121_receipt.get("execution_status", "")) != "wave121_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave121 exploration baseline closure must be complete before wave121 demo extension"
        )
    if str(wave120_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave120 presentation writeback must be filled before wave121 extension")
    if str(wave120_fill.get("storyboard_receipt_status", "")) != "wave120_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave120 storyboard receipt must be wave120_presentation_extension_complete before wave121 extension"
        )


def build_wave121_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave121",
        "headline": "Wave120 exploration+baseline closure after Wave120 MeetEval chain boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave120_polish_rows()
    if any(row["section_id"] == "frontier_wave121" for row in rows):
        return rows
    return rows + [build_wave121_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave121_presentation_extension_complete",
        "storyboard_scope": "wave121_storyboard_extension",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave121_presentation_extension_complete",
        "walkthrough_scope": "wave121_walkthrough_extension",
        "expected_inputs": "Wave104 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave121_presentation_polish_extension",
        "storyboard_receipt_status": "wave121_presentation_extension_complete",
        "walkthrough_receipt_status": "wave121_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave121_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave121_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave121_presentation_writeback.md"
...
def run_wave121_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave121_receipt = load_json_dict("results/tables/wave121_exploration_baseline_closure_receipt.json")
    wave120_fill = load_json_dict("results/tables/demo_wave120_presentation_writeback.json")
    assert_extension_preconditions(wave121_receipt, wave120_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave121_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave121_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave121 section.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave121_presentation_writeback(force=args.force)
