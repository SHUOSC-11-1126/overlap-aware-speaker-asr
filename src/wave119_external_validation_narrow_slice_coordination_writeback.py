...
RECEIPT_COLUMNS = [
    "execution_status",
    "coordination_scope",
    "wave119_closure_status",
    "oppositeoverlap_coordination_status",
    "wave113_refresh_status",
...
def assert_writeback_preconditions(
    wave119_receipt: dict[str, Any],
    oppositeoverlap_receipt: dict[str, Any],
    demo_wave119_fill: dict[str, Any],
    narrow_eval_receipt: dict[str, Any],
    external_summary: dict[str, Any],
    prior_coord_receipt: dict[str, Any],
) -> None:
    if str(wave119_receipt.get("execution_status", "")) != "wave119_exploration_baseline_closure_complete":
    ...
    if str(demo_wave119_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave119 presentation writeback must be filled before external validation coordination")
    if str(demo_wave119_fill.get("storyboard_receipt_status", "")) != "wave119_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave119 storyboard receipt must be wave119_presentation_extension_complete before external coordination"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Wave113 external validation narrow slice coordination must be complete before Wave119 refresh"
        )
...
def build_coordination_rows() -> list[dict[str, str]]:
    return [
        ...
        {
            "section_id": "wave113_prior_coordination",
            "headline": "Wave113 external validation narrow slice coordination precedes Wave119 refresh",
            "artifact_anchor": "results/figures/wave113_external_validation_narrow_slice_coordination_card.md",
        ...
        {
            "section_id": "wave119_boundary",
            "headline": "Wave119 closure keeps external validation separate from gold baseline CER",
            "artifact_anchor": "results/figures/wave119_exploration_baseline_closure_card.md",
...
def build_fill_row(rows: list[dict[str, str]], slice_id: str) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave119_external_validation_narrow_slice_coordination_card",
        "coordination_section_count": str(len(rows)),
        "narrow_slice_id": slice_id,
        "execution_receipt_status": "wave119_external_validation_narrow_slice_coordination_complete",
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave119_external_validation_narrow_slice_coordination_complete",
        "coordination_scope": "wave119_external_validation_narrow_slice",
        "wave119_closure_status": str(wave119_receipt.get("execution_status", "")),
        "oppositeoverlap_coordination_status": str(oppositeoverlap_receipt.get("execution_status", "")),
        "wave113_refresh_status": str(prior_coord_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave119 closure, OppositeOverlap diagnostic, demo wave119, narrow ASR eval receipt, external board."
        ),
        "writeback_note": (
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave119_external_validation_narrow_slice_coordination_card.csv"
    card_json = tables_dir / "wave119_external_validation_narrow_slice_coordination_card.json"
    card_md = figures_dir / "wave119_external_validation_narrow_slice_coordination_card.md"
    fill_csv = tables_dir / "wave119_external_validation_narrow_slice_coordination_writeback.csv"
    fill_json = tables_dir / "wave119_external_validation_narrow_slice_coordination_writeback.json"
    fill_md = figures_dir / "wave119_external_validation_narrow_slice_coordination_writeback.md"
    receipt_json = tables_dir / "wave119_external_validation_narrow_slice_coordination_receipt.json"
    receipt_md = figures_dir / "wave119_external_validation_narrow_slice_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    wave119_receipt = load_json_dict("results/tables/wave119_exploration_baseline_closure_receipt.json")
    ...
    demo_wave119_fill = load_json_dict("results/tables/demo_wave119_presentation_writeback.json")
    ...
    prior_coord_receipt = load_json_dict(
        "results/tables/wave113_external_validation_narrow_slice_coordination_receipt.json"
    )
    ...
    receipt_path = PROJECT_ROOT / "results/tables/wave119_external_validation_narrow_slice_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave119_external_validation_narrow_slice_coordination_receipt.json")
        if str(existing.get("execution_status", "")) == "wave119_external_validation_narrow_slice_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave119_receipt, oppositeoverlap_receipt, prior_coord_receipt, narrow_eval_receipt, external_summary
    )
