...
def assert_writeback_preconditions(
    ...
    if str(wave20_receipt.get("execution_status", "")) != "wave20_exploration_baseline_closure_complete":
        raise RuntimeError("Wave20 closure must be complete before MeetEval official narrow dry-run refresh coordination")
    if str(demo_wave20_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave20 presentation writeback must be filled before MeetEval official coordination")
    if str(demo_wave20_fill.get("storyboard_receipt_status", "")) != "wave20_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave20 storyboard receipt must be wave20_presentation_extension_complete before MeetEval official coordination"
        )
    if str(prior_official_receipt.get("execution_status", "")) != "meeteval_official_narrow_dry_run_coordination_complete":
        raise RuntimeError(
            "Wave14 MeetEval official narrow dry-run coordination must be complete before Wave20 refresh"
        )
...
def build_coordination_rows() -> list[dict[str, str]]:
    return [
        ...
        {
            "section_id": "wave14_prior_coordination",
            "headline": "Wave14 official MeetEval narrow dry-run coordination remains the prerequisite chain",
            "artifact_anchor": "results/figures/meeteval_official_narrow_dry_run_coordination_card.md",
            "coordination_note": "Wave20 refresh does not replace original official MeetEval boundary documentation.",
            "result_label": "experimental/frontier",
        },
        {
        ...
        },
        {
            ...
            "headline": "Wave20 closure keeps official MeetEval coordination separate from gold baseline CER",
...
def build_fill_row(rows: list[dict[str, str]], official_dry_run_case_count: str) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave20_meeteval_official_narrow_dry_run_coordination_card",
        "coordination_section_count": str(len(rows)),
        "official_dry_run_case_count": official_dry_run_case_count,
        "execution_receipt_status": "wave20_meeteval_official_narrow_dry_run_coordination_complete",
        "blocker": "official_benchmark_claims_still_blocked",
        "fill_note": (
            "Documented MeetEval official narrow dry-run refresh boundary after Wave20 closure; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave20_meeteval_official_narrow_dry_run_coordination_complete",
        "coordination_scope": "wave20_meeteval_official_narrow_dry_run",
        ...
        "expected_inputs": (
            "Wave20 closure, demo wave20, Wave14 official coordination, Wave17 character-level refresh, official execution."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave20 MeetEval Official Narrow Dry-Run Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave20 MeetEval Official Narrow Dry-Run Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave20 MeetEval Official Narrow Dry-Run Coordination Receipt",
        "",
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave20_meeteval_official_narrow_dry_run_coordination_card.csv"
    card_json = tables_dir / "wave20_meeteval_official_narrow_dry_run_coordination_card.json"
    card_md = figures_dir / "wave20_meeteval_official_narrow_dry_run_coordination_card.md"
    fill_csv = tables_dir / "wave20_meeteval_official_narrow_dry_run_coordination_writeback.csv"
    fill_json = tables_dir / "wave20_meeteval_official_narrow_dry_run_coordination_writeback.json"
    fill_md = figures_dir / "wave20_meeteval_official_narrow_dry_run_coordination_writeback.md"
    receipt_json = tables_dir / "wave20_meeteval_official_narrow_dry_run_coordination_receipt.json"
    receipt_md = figures_dir / "wave20_meeteval_official_narrow_dry_run_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    assert_writeback_preconditions(
        wave20_receipt, demo_wave20_fill, prior_official_receipt, character_refresh_receipt
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave20_meeteval_official_narrow_dry_run_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave20_meeteval_official_narrow_dry_run_coordination_receipt.json")
        if str(existing.get("execution_status", "")) == "wave20_meeteval_official_narrow_dry_run_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave20_receipt, prior_official_receipt, character_refresh_receipt, official_count
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave20 MeetEval official narrow dry-run refresh after Wave20 closure."
    )
