...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a synthetic overlap benchmark from snippets.")
    parser.add_argument("--num-per-tier", type=int, default=5, help="Samples to generate per tier.")
...
def tier_overlap_ratio(tier: str, index: int) -> float:
    ...
    if isinstance(value, tuple):
        ...
        if len(value) == 1:
            return float(low)
        step = 0 if index == 0 else index / max(1, 4)
...
def main() -> None:
    ...
    base_audio_dir = PROJECT_ROOT / "resources" / "synthetic_overlap" / "audio"
    base_ref_dir = PROJECT_ROOT / "resources" / "synthetic_overlap" / "references"
    ...
    print(f"Wrote synthetic manifest: {manifest_path.relative_to(PROJECT_ROOT)}")
    for tier in TIER_ORDER:
