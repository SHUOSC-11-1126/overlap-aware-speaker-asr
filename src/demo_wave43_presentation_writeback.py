...
from .demo_wave42_presentation_writeback import build_extended_polish_rows as build_wave42_polish_rows
...
def assert_extension_preconditions(
    ...
    wave42_fill: dict[str, Any],
) -> None:
    if str(wave43_receipt.get("execution_status", "")) != "wave43_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave43 exploration baseline closure must be complete before wave43 demo extension"
        )
    if str(wave42_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave42 presentation writeback must be filled before wave43 extension")
    if str(wave42_fill.get("storyboard_receipt_status", "")) != "wave42_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave42 storyboard receipt must be wave42_presentation_extension_complete before wave43 extension"
        )


def build_wave43_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave43",
        "headline": "Wave43 exploration+baseline closure and LightOverlap diagnostic refresh boundary",
        "artifact_anchor": "results/figures/wave43_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave43 closure card only; LightOverlap speaker-profile diagnostic refresh remains "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave42_polish_rows()
    if any(row["section_id"] == "frontier_wave43" for row in rows):
        return rows
    return rows + [build_wave43_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave43_presentation_extension_complete",
        "storyboard_scope": "wave43_storyboard_extension",
        "expected_inputs": "Wave43 exploration+baseline closure card.",
        "expected_outputs": "Wave43 storyboard extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave43 storyboard extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave43_presentation_extension_complete",
        "walkthrough_scope": "wave43_walkthrough_extension",
        "expected_inputs": "Wave43 exploration+baseline closure card.",
        "expected_outputs": "Wave43 walkthrough extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave43 walkthrough extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave43_presentation_polish_extension",
        "storyboard_receipt_status": "wave43_presentation_extension_complete",
        "walkthrough_receipt_status": "wave43_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave43 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave43_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave43_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave43_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave43 Presentation Writeback",
                "",
...
def run_wave43_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave43_receipt = load_json_dict("results/tables/wave43_exploration_baseline_closure_receipt.json")
    wave42_fill = load_json_dict("results/tables/demo_wave42_presentation_writeback.json")
    assert_extension_preconditions(wave43_receipt, wave42_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave43_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave43_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave43 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave43 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave43_presentation_writeback(force=args.force)
