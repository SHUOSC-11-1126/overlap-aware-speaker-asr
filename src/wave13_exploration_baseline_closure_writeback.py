...
def assert_closure_preconditions(
    ...
    if str(wave12_receipt.get("execution_status", "")) != "wave12_exploration_baseline_closure_complete":
        raise RuntimeError("Wave12 closure receipt must be complete before Wave13 exploration baseline closure")
    if str(llm_critic_receipt.get("execution_status", "")) != "llm_critic_narrow_dry_run_coordination_complete":
        raise RuntimeError("LLM critic narrow dry-run coordination must be complete before Wave13 closure")
    if str(heavyoverlap_receipt.get("execution_status", "")) != "speaker_profile_heavyoverlap_diagnostic_coordination_complete":
        raise RuntimeError("Speaker profile HeavyOverlap diagnostic coordination must be complete before Wave13 closure")
    if str(demo_wave12_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave12 presentation writeback must be filled before Wave13 closure")
...
def build_closure_rows() -> list[dict[str, str]]:
    return [
        {
            "section_id": "wave12_rollup",
            "headline": "Wave12 closed: LLM critic dry-run + HeavyOverlap diagnostic + demo + exploration closure",
        ...
        },
        {
            ...
            "headline": "LLM critic qualitative narrow dry-run coordinated on 5/5 gold cases",
            "artifact_anchor": "results/figures/llm_critic_narrow_dry_run_coordination_card.md",
            "coordination_note": "Verified repair claims still blocked.",
            "result_label": "qualitative/demo",
        },
        {
            "section_id": "heavyoverlap_diagnostic",
            "headline": "HeavyOverlap separation-benefit diagnostic scope coordinated",
            "artifact_anchor": "results/figures/speaker_profile_heavyoverlap_diagnostic_coordination_card.md",
            "coordination_note": "OppositeOverlap remains deferred; attribution still blocked.",
            "result_label": "experimental/frontier",
        },
        {
        ...
        },
        {
            ...
            "coordination_note": "Wave13 closure does not record controlled hardware benchmark sessions.",
            "result_label": "experimental/frontier",
        },
    ]
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave13_exploration_baseline_closure_card",
        "coordination_section_count": str(len(rows)),
        "execution_receipt_status": "wave13_exploration_baseline_closure_complete",
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": (
            "Wave13 exploration+baseline closure rollup after Wave12 chain; stable gold baseline preserved."
        ),
    }
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave13_exploration_baseline_closure_complete",
        "coordination_scope": "wave13_exploration_baseline",
        ...
        "expected_inputs": (
            "Wave12 closure, LLM critic coordination, HeavyOverlap diagnostic, demo wave12, frontier summary."
        ),
        "writeback_note": (
            "探索+基线 Wave13 closure writeback; qualitative/demo and experimental/frontier labels preserved."
        ),
    }
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave13 Exploration+Baseline Closure Card",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave13 Exploration+Baseline Closure Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave13 Exploration+Baseline Closure Receipt",
        "",
        "| execution_status | wave12_closure_status | frontier_go_count |",
        "| --- | --- | ---: |",
        (
            f"| {row['execution_status']} | {row['wave12_closure_status']} | {row['frontier_go_count']} |"
        ),
    ]
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave13_exploration_baseline_closure_card.csv"
    card_json = tables_dir / "wave13_exploration_baseline_closure_card.json"
    card_md = figures_dir / "wave13_exploration_baseline_closure_card.md"
    fill_csv = tables_dir / "wave13_exploration_baseline_closure_writeback.csv"
    fill_json = tables_dir / "wave13_exploration_baseline_closure_writeback.json"
    fill_md = figures_dir / "wave13_exploration_baseline_closure_writeback.md"
    receipt_json = tables_dir / "wave13_exploration_baseline_closure_receipt.json"
    receipt_md = figures_dir / "wave13_exploration_baseline_closure_receipt.md"
...
def run_closure_writeback(force: bool = False) -> dict[str, str]:
    ...
    llm_critic_receipt = load_json_dict("results/tables/llm_critic_narrow_dry_run_coordination_receipt.json")
    ...
    assert_closure_preconditions(
        wave12_receipt, llm_critic_receipt, heavyoverlap_receipt, demo_wave12_fill, frontier_summary
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave13_exploration_baseline_closure_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave13_exploration_baseline_closure_receipt.json")
        if str(existing.get("execution_status", "")) == "wave13_exploration_baseline_closure_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave12_receipt, llm_critic_receipt, heavyoverlap_receipt, frontier_summary)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Write Wave13 exploration+baseline closure after Wave12 chain.")
