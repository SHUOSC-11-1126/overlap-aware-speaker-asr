...
RECEIPT_COLUMNS = [
    ...
    "wave43_lightoverlap_refresh_status",
...
def assert_writeback_preconditions(
    ...
    wave43_lightoverlap_receipt: dict[str, Any],
    lightoverlap_receipt: dict[str, Any],
) -> None:
    if str(wave49_receipt.get("execution_status", "")) != "wave49_exploration_baseline_closure_complete":
        raise RuntimeError("Wave49 closure must be complete before LightOverlap diagnostic refresh coordination")
    if str(demo_wave49_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave49 presentation writeback must be filled before LightOverlap coordination")
    if str(demo_wave49_fill.get("storyboard_receipt_status", "")) != "wave49_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave49 storyboard receipt must be wave49_presentation_extension_complete before LightOverlap coordination"
        )
    if (
        str(wave43_lightoverlap_receipt.get("execution_status", ""))
        != "wave43_speaker_profile_lightoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave43 LightOverlap diagnostic refresh must be complete before Wave49 refresh coordination"
        )
    if str(lightoverlap_receipt.get("execution_status", "")) != "speaker_profile_lightoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Speaker profile LightOverlap diagnostic coordination must be complete before Wave49 refresh"
        )
    ...
    return [
        {
            ...
            "headline": "LightOverlap separation-harm diagnostic scope refresh after Wave49 closure",
        ...
        },
        {
            ...
            "coordination_note": "Wave49 refresh does not replace original LightOverlap boundary documentation.",
            "result_label": "experimental/frontier",
        },
        {
            "section_id": "wave43_refresh",
            "headline": "Wave43 LightOverlap diagnostic refresh precedes Wave49 refresh",
        ...
        {
            "section_id": "wave49_boundary",
            "headline": "Wave49 closure defers overlap embedding execution to diagnostic boundaries only",
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave49_speaker_profile_lightoverlap_diagnostic_coordination_card",
        "coordination_section_count": str(len(rows)),
        "diagnostic_case_scope": "LightOverlap",
        "candidate_case_scope": "MidOverlap",
        "execution_receipt_status": "wave49_speaker_profile_lightoverlap_diagnostic_coordination_complete",
        "blocker": "attribution_claims_still_blocked",
        "fill_note": (
            "Documented LightOverlap diagnostic refresh after Wave49 closure; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave49_speaker_profile_lightoverlap_diagnostic_coordination_complete",
        "coordination_scope": "wave49_speaker_profile_lightoverlap_diagnostic",
        "wave49_closure_status": str(wave49_receipt.get("execution_status", "")),
        "wave43_lightoverlap_refresh_status": str(wave43_lightoverlap_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave49 closure, demo wave49 writeback, Wave43 LightOverlap refresh, and Wave8 LightOverlap coordination."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave49 Speaker Profile LightOverlap Diagnostic Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave49 Speaker Profile LightOverlap Diagnostic Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave49 Speaker Profile LightOverlap Diagnostic Coordination Receipt",
        "",
        "| execution_status | wave43_lightoverlap_refresh_status | separation_harm_signal |",
        "| --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['wave43_lightoverlap_refresh_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave49_speaker_profile_lightoverlap_diagnostic_coordination_card.csv"
    card_json = tables_dir / "wave49_speaker_profile_lightoverlap_diagnostic_coordination_card.json"
    card_md = figures_dir / "wave49_speaker_profile_lightoverlap_diagnostic_coordination_card.md"
    fill_csv = tables_dir / "wave49_speaker_profile_lightoverlap_diagnostic_coordination_writeback.csv"
    fill_json = tables_dir / "wave49_speaker_profile_lightoverlap_diagnostic_coordination_writeback.json"
    fill_md = figures_dir / "wave49_speaker_profile_lightoverlap_diagnostic_coordination_writeback.md"
    receipt_json = tables_dir / "wave49_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    receipt_md = figures_dir / "wave49_speaker_profile_lightoverlap_diagnostic_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    wave43_lightoverlap_receipt = load_json_dict(
    ...
    assert_writeback_preconditions(
        wave49_receipt, demo_wave49_fill, wave43_lightoverlap_receipt, lightoverlap_receipt
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave49_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict(
            "results/tables/wave49_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
        )
        if str(existing.get("execution_status", "")) == "wave49_speaker_profile_lightoverlap_diagnostic_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave49_receipt, wave43_lightoverlap_receipt, lightoverlap_receipt)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave49 speaker profile LightOverlap diagnostic refresh after Wave49 closure."
    )
