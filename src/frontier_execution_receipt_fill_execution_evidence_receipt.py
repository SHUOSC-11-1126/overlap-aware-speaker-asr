...
def build_evidence_receipt_row(operator_brief: dict[str, str]) -> dict[str, str]:
    ...
    return {
        ...
        "receipt_followup": (
            "Archive the fill evidence note and advance to the next frontier receipt in the handoff table."
        ),
        "receipt_note": (
...
def build_evidence_receipt_lines(row: dict[str, str]) -> list[str]:
    lines = [
        ...
        "",
        "This generated receipt shows what the current fill execution run must write back before the next contributor advances the stack. "
...
def write_outputs(row: dict[str, str]) -> tuple[Path, Path, Path]:
    ...
    csv_path = tables_dir / "frontier_execution_receipt_fill_execution_evidence_receipt.csv"
    json_path = tables_dir / "frontier_execution_receipt_fill_execution_evidence_receipt.json"
    md_path = figures_dir / "frontier_execution_receipt_fill_execution_evidence_receipt.md"
...
def main() -> None:
    ...
    if not row:
        print("Operator brief not found; evidence receipt not written.")
        return
    csv_path, json_path, md_path = write_outputs(row)
    print(
        "Wrote frontier execution receipt fill execution evidence receipt CSV: "
        f"{csv_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote frontier execution receipt fill execution evidence receipt JSON: "
        f"{json_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote frontier execution receipt fill execution evidence receipt note: "
