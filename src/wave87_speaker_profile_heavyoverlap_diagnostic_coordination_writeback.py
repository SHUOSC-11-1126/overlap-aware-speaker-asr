...
RECEIPT_COLUMNS = [
    "execution_status",
    "coordination_scope",
    "wave87_closure_status",
    "heavyoverlap_prior_coordination_status",
    "wave81_refresh_status",
...
def assert_writeback_preconditions(
    wave87_receipt: dict[str, Any],
    demo_wave87_fill: dict[str, Any],
    heavyoverlap_receipt: dict[str, Any],
    wave81_refresh_receipt: dict[str, Any],
    midoverlap_refresh_receipt: dict[str, Any],
) -> None:
    if str(wave87_receipt.get("execution_status", "")) != "wave87_exploration_baseline_closure_complete":
        raise RuntimeError("Wave87 closure must be complete before HeavyOverlap diagnostic refresh coordination")
    if str(demo_wave87_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave87 presentation writeback must be filled before HeavyOverlap coordination")
    if str(demo_wave87_fill.get("storyboard_receipt_status", "")) != "wave87_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave87 storyboard receipt must be wave87_presentation_extension_complete before HeavyOverlap coordination"
        )
    ...
    if str(wave81_refresh_receipt.get("execution_status", "")) != "wave81_speaker_profile_heavyoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Wave81 HeavyOverlap diagnostic refresh must be complete before Wave87 refresh coordination"
        )
    if (
        ...
        raise RuntimeError(
            "Wave86 MidOverlap diagnostic refresh must be complete before HeavyOverlap refresh coordination"
        )
    ...
    return [
        {
            ...
            "headline": "HeavyOverlap separation-benefit diagnostic scope refresh after Wave87 closure",
        ...
        {
            "section_id": "wave81_refresh",
            "headline": "Wave81 HeavyOverlap diagnostic refresh precedes Wave87 refresh",
        ...
        },
        {
            ...
            "headline": "Wave86 MidOverlap diagnostic refresh precedes HeavyOverlap refresh",
        ...
        },
        {
            ...
            "headline": "Wave87 closure defers overlap embedding execution to diagnostic boundaries only",
            "artifact_anchor": "results/figures/wave87_exploration_baseline_closure_card.md",
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave87_speaker_profile_heavyoverlap_diagnostic_coordination_card",
        "coordination_section_count": str(len(rows)),
        "diagnostic_case_scope": "HeavyOverlap",
        "candidate_case_scope": "OppositeOverlap",
        "execution_receipt_status": "wave87_speaker_profile_heavyoverlap_diagnostic_coordination_complete",
        "blocker": "attribution_claims_still_blocked",
        "fill_note": (
            "Documented HeavyOverlap diagnostic refresh after Wave87 closure; "
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave87_speaker_profile_heavyoverlap_diagnostic_coordination_complete",
        "coordination_scope": "wave81_speaker_profile_heavyoverlap_diagnostic",
        "wave87_closure_status": str(wave87_receipt.get("execution_status", "")),
        "heavyoverlap_prior_coordination_status": str(heavyoverlap_receipt.get("execution_status", "")),
        "wave81_refresh_status": str(wave81_refresh_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave87 closure, demo wave81, Wave12 HeavyOverlap, Wave75 refresh, and Wave86 MidOverlap refresh."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave87 Speaker Profile HeavyOverlap Diagnostic Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave87 Speaker Profile HeavyOverlap Diagnostic Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave87 Speaker Profile HeavyOverlap Diagnostic Coordination Receipt",
        "",
        "| execution_status | wave81_refresh_status | midoverlap_refresh_status |",
        "| --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['wave81_refresh_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave87_speaker_profile_heavyoverlap_diagnostic_coordination_card.csv"
    card_json = tables_dir / "wave87_speaker_profile_heavyoverlap_diagnostic_coordination_card.json"
    card_md = figures_dir / "wave87_speaker_profile_heavyoverlap_diagnostic_coordination_card.md"
    fill_csv = tables_dir / "wave87_speaker_profile_heavyoverlap_diagnostic_coordination_writeback.csv"
    fill_json = tables_dir / "wave87_speaker_profile_heavyoverlap_diagnostic_coordination_writeback.json"
    fill_md = figures_dir / "wave87_speaker_profile_heavyoverlap_diagnostic_coordination_writeback.md"
    receipt_json = tables_dir / "wave87_speaker_profile_heavyoverlap_diagnostic_coordination_receipt.json"
    receipt_md = figures_dir / "wave87_speaker_profile_heavyoverlap_diagnostic_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    wave87_receipt = load_json_dict("results/tables/wave87_exploration_baseline_closure_receipt.json")
    demo_wave87_fill = load_json_dict("results/tables/demo_wave87_presentation_writeback.json")
    heavyoverlap_receipt = load_json_dict(
        "results/tables/speaker_profile_heavyoverlap_diagnostic_coordination_receipt.json"
    )
    wave81_refresh_receipt = load_json_dict(
    ...
    assert_writeback_preconditions(
        ...
        wave81_refresh_receipt,
        midoverlap_refresh_receipt,
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave87_speaker_profile_heavyoverlap_diagnostic_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict(
            "results/tables/wave87_speaker_profile_heavyoverlap_diagnostic_coordination_receipt.json"
        )
        if str(existing.get("execution_status", "")) == "wave87_speaker_profile_heavyoverlap_diagnostic_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave87_receipt, heavyoverlap_receipt, wave81_refresh_receipt, midoverlap_refresh_receipt
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave81 speaker profile HeavyOverlap diagnostic refresh after Wave87 closure."
    )
