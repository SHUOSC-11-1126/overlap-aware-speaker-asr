...
from .demo_wave43_presentation_writeback import build_extended_polish_rows as build_wave43_polish_rows
...
def assert_extension_preconditions(
    ...
    wave43_fill: dict[str, Any],
) -> None:
    if str(wave44_receipt.get("execution_status", "")) != "wave44_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave44 exploration baseline closure must be complete before wave44 demo extension"
        )
    if str(wave43_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave43 presentation writeback must be filled before wave44 extension")
    if str(wave43_fill.get("storyboard_receipt_status", "")) != "wave43_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave43 storyboard receipt must be wave43_presentation_extension_complete before wave44 extension"
        )


def build_wave44_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave44",
        "headline": "Wave44 exploration+baseline closure and MidOverlap diagnostic refresh boundary",
        "artifact_anchor": "results/figures/wave44_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave44 closure card only; MidOverlap speaker-profile diagnostic refresh remains "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave43_polish_rows()
    if any(row["section_id"] == "frontier_wave44" for row in rows):
        return rows
    return rows + [build_wave44_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave44_presentation_extension_complete",
        "storyboard_scope": "wave44_storyboard_extension",
        "expected_inputs": "Wave44 exploration+baseline closure card.",
        "expected_outputs": "Wave44 storyboard extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave44 storyboard extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave44_presentation_extension_complete",
        "walkthrough_scope": "wave44_walkthrough_extension",
        "expected_inputs": "Wave44 exploration+baseline closure card.",
        "expected_outputs": "Wave44 walkthrough extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave44 walkthrough extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave44_presentation_polish_extension",
        "storyboard_receipt_status": "wave44_presentation_extension_complete",
        "walkthrough_receipt_status": "wave44_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave44 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave44_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave44_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave44_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave44 Presentation Writeback",
                "",
...
def run_wave44_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave44_receipt = load_json_dict("results/tables/wave44_exploration_baseline_closure_receipt.json")
    wave43_fill = load_json_dict("results/tables/demo_wave43_presentation_writeback.json")
    assert_extension_preconditions(wave44_receipt, wave43_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave44_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave44_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave44 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave44 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave44_presentation_writeback(force=args.force)
