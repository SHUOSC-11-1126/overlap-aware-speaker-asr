...
from .demo_wave6_presentation_writeback import build_extended_polish_rows as build_wave6_polish_rows
...
def assert_extension_preconditions(
    ...
    wave6_fill: dict[str, Any],
) -> None:
    if str(wave7_receipt.get("execution_status", "")) != "wave7_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave7 exploration baseline closure must be complete before wave7 demo extension"
        )
    if str(demo_summary.get("overall_state", "")) != "presentation_wave6_extension_complete":
        raise RuntimeError(
            f"Demo go/no-go must be presentation_wave6_extension_complete; got {demo_summary.get('overall_state', 'missing')!r}"
        )
    if str(wave6_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave6 presentation writeback must be filled before wave7 extension")


def build_wave7_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave7",
        "headline": "Wave7 exploration+baseline closure and next frontier gates",
        "artifact_anchor": "results/figures/wave7_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave7 closure card only; speaker profile and benchmark timing remain "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave6_polish_rows()
    if any(row["section_id"] == "frontier_wave7" for row in rows):
        return rows
    return rows + [build_wave7_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave7_presentation_extension_complete",
        "storyboard_scope": "wave7_storyboard_extension",
        "expected_inputs": "Wave7 exploration+baseline closure card.",
        "expected_outputs": "Wave7 storyboard extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave7 storyboard extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave7_presentation_extension_complete",
        "walkthrough_scope": "wave7_walkthrough_extension",
        "expected_inputs": "Wave7 exploration+baseline closure card.",
        "expected_outputs": "Wave7 walkthrough extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave7 walkthrough extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave7_presentation_polish_extension",
        "storyboard_receipt_status": "wave7_presentation_extension_complete",
        "walkthrough_receipt_status": "wave7_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave7 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave7_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave7_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave7_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave7 Presentation Writeback",
                "",
...
def run_wave7_presentation_writeback(force: bool = False) -> dict[str, str]:
    ...
    wave6_fill = load_json_dict("results/tables/demo_wave6_presentation_writeback.json")
    assert_extension_preconditions(wave7_receipt, demo_summary, wave6_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave7_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave7_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave7 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave7 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave7_presentation_writeback(force=args.force)
