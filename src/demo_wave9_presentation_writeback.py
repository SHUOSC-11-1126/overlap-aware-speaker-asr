...
from .demo_wave8_presentation_writeback import build_extended_polish_rows as build_wave8_polish_rows
...
def assert_extension_preconditions(
    ...
    wave8_fill: dict[str, Any],
) -> None:
    if str(wave9_receipt.get("execution_status", "")) != "wave9_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave9 exploration baseline closure must be complete before wave9 demo extension"
        )
    if str(wave8_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave8 presentation writeback must be filled before wave9 extension")
    if str(wave8_fill.get("storyboard_receipt_status", "")) != "wave8_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave8 storyboard receipt must be wave8_presentation_extension_complete before wave9 extension"
        )


def build_wave9_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave9",
        "headline": "Wave9 exploration+baseline closure and evidence receipt boundary rollup",
        "artifact_anchor": "results/figures/wave9_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave9 closure card only; MidOverlap diagnostics and benchmark phase1 gate remain "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave8_polish_rows()
    if any(row["section_id"] == "frontier_wave9" for row in rows):
        return rows
    return rows + [build_wave9_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave9_presentation_extension_complete",
        "storyboard_scope": "wave9_storyboard_extension",
        "expected_inputs": "Wave9 exploration+baseline closure card.",
        "expected_outputs": "Wave9 storyboard extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave9 storyboard extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave9_presentation_extension_complete",
        "walkthrough_scope": "wave9_walkthrough_extension",
        "expected_inputs": "Wave9 exploration+baseline closure card.",
        "expected_outputs": "Wave9 walkthrough extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave9 walkthrough extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave9_presentation_polish_extension",
        "storyboard_receipt_status": "wave9_presentation_extension_complete",
        "walkthrough_receipt_status": "wave9_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave9 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave9_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave9_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave9_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave9 Presentation Writeback",
                "",
...
def run_wave9_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave9_receipt = load_json_dict("results/tables/wave9_exploration_baseline_closure_receipt.json")
    wave8_fill = load_json_dict("results/tables/demo_wave8_presentation_writeback.json")
    assert_extension_preconditions(wave9_receipt, wave8_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave9_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave9_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave9 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave9 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave9_presentation_writeback(force=args.force)
