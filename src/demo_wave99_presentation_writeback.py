...
from .demo_wave98_presentation_writeback import build_extended_polish_rows as build_wave98_polish_rows
...
def assert_extension_preconditions(
    wave99_receipt: dict[str, Any],
    wave98_fill: dict[str, Any],
) -> None:
    if str(wave99_receipt.get("execution_status", "")) != "wave99_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave99 exploration baseline closure must be complete before wave99 demo extension"
        )
    if str(wave98_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave98 presentation writeback must be filled before wave99 extension")
    if str(wave98_fill.get("storyboard_receipt_status", "")) != "wave98_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave98 storyboard receipt must be wave98_presentation_extension_complete before wave99 extension"
        )


def build_wave99_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave99",
        "headline": "Wave99 exploration+baseline closure after Wave98 MidOverlap chain boundary",
        "artifact_anchor": "results/figures/wave99_exploration_baseline_closure_card.md",
        "presentation_note": (
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave98_polish_rows()
    if any(row["section_id"] == "frontier_wave99" for row in rows):
        return rows
    return rows + [build_wave99_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave99_presentation_extension_complete",
        "storyboard_scope": "wave99_storyboard_extension",
        "expected_inputs": "Wave99 exploration+baseline closure card.",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave99_presentation_extension_complete",
        "walkthrough_scope": "wave99_walkthrough_extension",
        "expected_inputs": "Wave99 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave99_presentation_polish_extension",
        "storyboard_receipt_status": "wave99_presentation_extension_complete",
        "walkthrough_receipt_status": "wave99_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave99_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave99_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave99_presentation_writeback.md"
...
def run_wave99_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave99_receipt = load_json_dict("results/tables/wave99_exploration_baseline_closure_receipt.json")
    wave98_fill = load_json_dict("results/tables/demo_wave98_presentation_writeback.json")
    assert_extension_preconditions(wave99_receipt, wave98_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave99_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave99_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave99 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave99 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave99_presentation_writeback(force=args.force)
